...
// ---------------------------------------------------------------------------
// 假 Tavern：内存文件树 + 工作区四方法（形状照 workspace.js）
// ---------------------------------------------------------------------------

function createFakeTavern() {
  const files = new Map() // relPath -> string
  const dirs = new Set()
  const requests = [] // 假服务收到的每个请求 {method,url,host,origin} —— A9 逐条断言用
  let putCount = 0
  let armedAt = null
  let failRelN = null
  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      try {
        // ★ 刻意复刻 secureTavernApi（tavern-loader/src/api-security.js:119-123）的同源规则：
        // 变更方法（非 GET/HEAD/OPTIONS）必须带 Origin 且其 host === Host，缺失/跨源 ⇒ 403。
        // 响应形状照真安全层：{ok:false, code:'TAVERN_API_ORIGIN_FORBIDDEN'}（平铺 code）。
