...
**向量检索**（★ 2026-09-26：BM25 档已退役）、策略步与 rerank 拦截、echo（life 衰减 + 回响续命），
检索结果经 `system-prompt/assemble` 瀑布注入 RP 会话（`<memoryEcho>` 那一格也由它装）；
既是**独立的检索工具**，也是这套酒馆 agent 的**记忆库检索端**。
...
| | |
|---|---|
| **检索** | 每轮装配时按会话取词 → 向量检索 → 策略步（base / important / diversity）→ rerank 拦截 → 注入 system 段 `anima:memory`（`<memoryEcho>` 那一格是同一批命中的紧凑视图） |
| **检索工具** | `anima_query`：模型可**主动**查历史（返回命中条数、正文、以及**为什么是 0 命中**的一行诊断） |
| **入库** | 把**摘要目录**里的摘要写进记忆库（向量的**一次调用**）：压缩后自动入库，也可手动触发 |
...
| **向量面板的那半边** | 读 `dsh-memory-archive` 写来的**请求单**执行动作（立即入库 / 重建 / 删向量库），并把**状态快照**与**回执**写回去给它显示（★ 2026-09-26：`delete-bm25` 已随 BM25 退役摘除） |

**向量生成的时机**（明确只有两处）：① **自动压缩入库时**（摘要进库 ⇒ 向量写进去）；
...
数据根默认**指向 ST 侧现役 Anima 的数据**（`vectors/` + `data/sessions/`；★ BM25 根已随退役摘除）——
...
- **检索核心**：向量检索、策略步、rerank 拦截（BM25 双轨已于 2026-09-26 摘除）
- **echo（回响）机制**：本地字面召回
