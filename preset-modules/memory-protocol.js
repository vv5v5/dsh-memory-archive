 ...
 *
 * ## 草稿（填进 preset 的 `config.text` 即可；措辞按你的口径改）
 * ```
 * 此外，当这一轮要承接的事需要前史细节（更早的人、地点、物品、说过的话、约定）而你不确定时，
 * 先调用一次 anima_query 去查找；查到什么就按角色的记忆用，没查到就照常演，不要在正文里解释检索。
 * 只是当前场景内的事、或纯 OOC / 元讨论 / 聊设定时，不要调用；插件不存在时跳过。
 * 一轮最多一次；检索到的是你自己的回忆，只呈现角色当时知道的部分 —— 角色不该知道的，
 * 即使查到了也不许拿来用。
 * ```
