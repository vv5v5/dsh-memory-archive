...
//   ② **回档时自动恢复**（用户明确要"自动跟随"）：把上面那几份写回**该楼层那一份快照**。
//      恢复**之前**先给"当下"再存一份快照（可撤销），并**如实播报**（面板 + 日志）。
...
// ---------------------------------------------------------------------------
// ③ 一次「楼同步」的编排（判回档 → （必要时）恢复 → 记录）
//
// 两个调用时机（同一份编排，⛔ 不两处各写一遍）：
//   · `mode:'watch'` —— 挂 `system-prompt/assemble`（**每轮至多一次**，排到本轮之外再跑）：
//     只判回档、只做恢复。这一脚的意义是**让模型本轮读到的笔记就是那一楼的**。
//   · `mode:'turn'`  —— 挂 `session/event` 的 `turn/end` / `compaction/end`：判回档 + 恢复 + **记录**
//     （"每轮模型的改动都留一份"）。
//   ⚠️ 两处都跑不重复：watch 恢复过 ⇒ turn 那一脚看到的是 `same`（零写盘）。
// ---------------------------------------------------------------------------

/**
 * ★★ **恢复那一脚**（`kind` = `'rollback'`（自动跟随）/ `'manual'`（面板「回到这一楼」）——
 * 两边**共用这一份**，⛔ 绝不两处各写一遍：同一个恢复动作，只是"谁点的"不同）。
 *
 * 三步（顺序不许换）：
...
function runRestore(o) {
  ...
  if (failed.length === 0 && plan.blocked === null) {
    ...
    if (preWrote
      ...
      writeIndex(memoryDir, { schemaVersion: FLOOR_SCHEMA_VERSION, updatedAt: at, last, floors })
      wrote = true
    }
  } else if (preWrote) {
    writeIndex(memoryDir, { schemaVersion: FLOOR_SCHEMA_VERSION, updatedAt: at, last: index.last, floors })
  }
  return {
...
export function syncFloor(o) {
  ...
  if (decision.kind === 'rollback') {
    return runRestore({
      memoryDir, index, target: decision.target, kind: 'rollback', head,
      source: decision.source, why: decision.why, prevNodeId: decision.prevNodeId,
      legacyAtHead: decision.legacyAtHead,
      seq: decision.seq, at, stamp: o?.stamp, zones: o?.zones,
    })
  }
  ...
  return {
    kind: decision.kind, at, wrote, record, pre: null, restore: null, notes,
    // 判据那一层的如实补充（回执/面板据此说清"为什么没动 / 用的是哪一份"）。
...
export default {
  ...
  floorKeyOf, isLegacyFloor, isRestorableFloor,
  ...
  readFloorTargets, hasAnyFloorFile, writeRestoredFile, restoreFloor, syncFloor,
}
