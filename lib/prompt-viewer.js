...
export function buildConversation(events) {
  ...
  for (const ev of list) {
    ...
    switch (ev?.type) {
      case 'request/header': {
        const s = summarizeHeader(ev, headers.length + 1)
...
export function createSessionSource(rawConfig) {
  ...
  async function loadParsed(sessionId) {
    ...
    for (const ev of events) {
      if (ev?.type === 'request/header') requests.push(summarizeHeader(ev, requests.length + 1))
    }
    ...
    const title = firstTextRow ? firstTextRow.text.replace(/\s+/g, ' ').trim().slice(0, 60) : ''
...
async function partApi(source, params) {
  ...
  if (part === 'system') {
    ...
    return { ok: true, part, sessionId: id, turn, ...carriedFields, chars: header.system.length, text: clipText(header.system) }
  }
  if (part === 'messages') {
    ...
    try {
      if (path === '/health') {
        ...
        body = {
          ok: false,
          error: 'not found',
          hint: 'GET /health | /api/sessions | /api/sessions/resolve?ids=<id,… ≤10> | /api/session?id= | /api/turns?id= | /api/messages?id=&from=&limit= | /api/tool?id=&turn=&name= | /api/part?id=&turn=&part=system|tools|messages|inventory',
        }
      }
    } catch (error) {
