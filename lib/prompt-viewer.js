 ...
 *                             → 20260918 查看器单：part=messages 的每行纯新增
 *                               `turn`（这条消息属于哪一楼；楼外/无楼 ⇒ null，不猜成 0）与
 *                               `blocks`（content[] 按块拆的 [{kind, chars, head}]，思维链 =
 *                               kind:'reasoning'，只给词头 —— ⛔ 全文不进响应，更不落盘）。
 *   GET /api/part?…&part=tools|inventory
 *                             → { ok:true, part, tools:[{ name, chars, text }] }（`text` = 该条定义的原始 JSON；超上限时 null）
 *
...
/**
 * 单个 content 块的元数据（20260918 查看器单，纯新增）：`{ kind, chars, head }`。
 * 真机实测（session-c37c466c）：思维链就在 assistant/message 的 `content[]` 里，形状是
 * `{type:'reasoning', text:'…'}` 块与 `{type:'text'}` 块并列 —— 本会话没有独立思维链事件，
 * 所以取思维链只能从内容块拆。kind ∈ 'reasoning' | 'text' | 其它块类型原样（没有 type ⇒ null）。
 * chars/head 只认块正文（`b.text`）；没有正文 ⇒ 0/''（如实，不编）。
 * ⛔ head 是词头（沿用 PREVIEW_MAX_CHARS / previewOf 的既有口径），不许为思维链开第二套更长口径。
 */
function blockMeta(b) {
  const body = b && typeof b.text === 'string' ? b.text : ''
  return {
    kind: b && typeof b.type === 'string' ? b.type : null,
    chars: body.length,
    head: previewOf(body),
  }
}

/** content（blocks 数组或纯字符串）→ 每块的 `{kind, chars, head}`；拿不到 ⇒ []（⛔ 不编块）。 */
function blocksOf(content) {
  if (typeof content === 'string') return content === '' ? [] : [blockMeta({ type: 'text', text: content })]
  if (!Array.isArray(content)) return []
  return content.map(blockMeta)
}

/**
 * ★★ 甲（2026-09-18）：**原样**留一份"这条消息在会话日志里长什么样"，
 * 供 `/api/part?part=messages&seq=` 在响应里现取（⛔ 不落我们自己的盘，也不改宿主那份）。
 *
 * ★ 为什么给的是"日志里的样子"而不是"线上的 wire JSON"：DSH **不落盘线上请求体**
 *   （`llm-deepseek/src/serialize.ts` 的产物只在宿主进程里活一次）⇒ 谁都拿不出 wire JSON。
 *   能给出的**唯一真实**的东西，就是会话日志里那条消息事件的对象。所以这里照抄它，
 *   并在界面上如实写清来源 —— ⛔ 绝不拿我们自己的拼装结果冒充"原样"。
 *
 * 形状（真机实测 `session-c37c466c`）：
 *   · `user/message`      ⇒ `{role:'user', content:[…], source:{kind,…}}`（source 是结构性判据，一并留）
 *   · `assistant/message` ⇒ `{role:'assistant', content:[…]}`（**思维链就在 content[] 的 reasoning 块里**）
 *   · `tool/result`       ⇒ 它的 message 对象
 *   · `compaction/summary`⇒ 不是消息 ⇒ 原样留事件的 `{summary, shadowedSeqs}`
 *
 * ⚠️ `content` 是**引用共享**（不复制），所以留下它几乎不额外占内存。
 * @param {'user'|'assistant'|'tool'|'compact'} kind
 * @param {any} obj - 日志里的对象（user 给 `ev.data`，其余给 `ev.data.message`，compact 给 `ev.data`）
 */
function rawMessageOf(kind, obj) {
  if (obj == null || typeof obj !== 'object') return null
  if (kind === 'compact') {
    return {
      _event: 'compaction/summary',
      summary: typeof obj.summary === 'string' ? obj.summary : null,
      shadowedSeqs: Array.isArray(obj.shadowedSeqs) ? obj.shadowedSeqs.slice() : null,
    }
  }
  return {
    role: typeof obj.role === 'string' ? obj.role : (kind === 'tool' ? 'tool' : kind),
    content: obj.content === undefined ? null : obj.content,
    ...
    switch (ev?.type) {
      ...
      case 'user/message':
        ...
        rows.push({
          ...
          blocks: blocksOf(ev.data?.content),
          // ★★ 2026-09-18 甲：**原样**留一份可在响应里现取的消息对象（见 rawMessageOf 的说明）。
          raw: rawMessageOf('user', ev.data),
        })
        break
      case 'assistant/message':
        rows.push({ seq, time, role: 'assistant', text: blocksText(ev.data?.message?.content), isToolResult: false, isCompacted: false, blocks: blocksOf(ev.data?.message?.content), raw: rawMessageOf('assistant', ev.data?.message) })
        break
      case 'tool/result':
        rows.push({ seq, time, role: 'tool', text: blocksText(ev.data?.message?.content), isToolResult: true, isCompacted: false, blocks: blocksOf(ev.data?.message?.content), raw: rawMessageOf('tool', ev.data?.message) })
        break
      case 'compaction/summary': {
        ...
        rows.push({ seq, time, role: 'user', text: blocksText(ev.data?.summary), isToolResult: false, isCompacted: true, blocks: blocksOf(ev.data?.summary), raw: rawMessageOf('compact', ev.data) })
        break
      }
      default:
        break
...
async function partApi(source, params) {
  ...
  if (part === 'messages') {
    ...
    const endSeq = entry.open ? Number.MAX_SAFE_INTEGER : entry.endSeq
    // ★★ 甲（2026-09-18）：`seq=` ⇒ **只回那一条的原样对象**（界面上"点开某条看它日志里的样子"）。
    //   为什么按条取而不是整包发：整楼 36 条 / 约 91 KB（真机 `session-c37c466c` 实测），
    //   整包又大又没人一次看完；而单条最大也只有 20 KB。⛔ 只读已缓存的解析结果，不重新解析日志。
    //   ⛔ 只认**本楼实际发出的历史**里的 seq（被压缩遮蔽的、别的楼的，一律如实报没有）。
    const seqRaw = params.get('seq')
    if (seqRaw !== null && seqRaw !== '') {
      const seqNum = Number(seqRaw)
      if (!Number.isInteger(seqNum)) return { ok: false, error: 'seq 必须是整数' }
      const hit = conv.rows.find((r) => r.seq === seqNum && r.seq <= endSeq)
      if (!hit) {
        return { ok: false, error: `第 ${turn} 楼实际发出的历史里没有 seq=${seqRaw} 的消息（可能被压缩摘要遮蔽，或不属于这一楼）` }
      }
      return {
        ...
        role: hit.role,
        chars: hit.chars,
        isToolResult: hit.isToolResult,
        isCompacted: hit.isCompacted,
        message: hit.raw ?? null,
        messageSource: '会话日志里那条消息对象（原样）。⛔ 不是线上 wire JSON —— DSH 不落盘请求体，那个谁也拿不到。',
      }
    }
    // ★★ 2026-09-20（用户口径修正，原话）：「但这一字段在对话历史里**需要给出并标红**」
    //   ⇒ ⛔ 不隐藏插件注入的 user 消息（本插件的后处理提示词、官方运行上下文快照都在内），
    //     照旧进 [对话历史]；由**面板**按 `sourcePlugin` 把它标红 + 写注释。
    //     （"不被记忆库收录"那一条**不受影响** —— 归档侧仍在 collect-scan 里按 kind 排除。）
    ...
    return {
      ...
      // ★ 2026-09-20：逐行带上 `sourceKind`/`sourcePlugin` —— 面板据此把**本插件注入的**那条
      //   在 [对话历史] 里**标红 + 写注释**（用户口径：「这一字段在对话历史里需要给出并标红」）。
      messages: inTurn.map((r) => ({
        ...
        // 20260918 查看器单（纯新增）：turn = 这条消息属于哪一楼（buildConversation 的行级归属）。
        // 0 = 楼外/尚未开楼的哨兵 ⇒ 响应里如实给 null（"没有楼"），⛔ 不许把"没有楼"猜成"第 0 楼"。
        turn: typeof r.turn === 'number' && Number.isInteger(r.turn) && r.turn > 0 ? r.turn : null,
        // 每块 {kind, chars, head}：思维链 = kind:'reasoning'，只给词头（⛔ 不把整段思维链塞进响应）。
        blocks: Array.isArray(r.blocks) ? r.blocks : [],
      })),
    }
  }
  // tools / inventory：该楼之前最近一条 header 的 per-tool 清单（已按字符数降序）。
  // ★ 2026-09-19（用户报障：「工具定义字段中看不到渲染过的原文，必须到原始 json 中找。复制的全文中没有 tool 定义具体内容」）：
  //   清单里原来只有 `{name, chars}` ⇒ 面板那一行点开才有正文、**复制整楼时拿不到任何工具正文**。
  //   现在把**每一条的正文**一并给出（`text` = 该条原始定义的 JSON；`rawTools` 超上限被丢时如实给 null，
  //   ⛔ 绝不截半份糊弄）。正文来自本楼 header 的 `rawTools`（`summarizeHeader` 留着的那份）。
  // `inventory` 是老名字（同一个清单，语义不变）⇒ 两个 part 名都给同一份带正文的清单。
  if (!header) return { ok: false, error: '该会话没有任何 request/header 记录（headerAvailable:false），tools 未知（不猜）' }
  const rawByName = new Map()
  if (Array.isArray(header.rawTools)) {
    for (const t of header.rawTools) {
      const nm = String(t?.name ?? t?.function?.name ?? '')
      if (nm !== '' && !rawByName.has(nm)) rawByName.set(nm, t)
    }
  }
  const toolsWithText = (Array.isArray(header.tools) ? header.tools : []).map((t) => {
    const raw = rawByName.get(String(t?.name))
    return { ...t, text: raw === undefined ? null : JSON.stringify(raw) }
  })
  return {
    ok: true,
    part,
    ...
    ...carriedFields,
    tools: toolsWithText,
    // 如实交代"为什么有的条没有正文"（超上限 / 没有原始定义），⛔ 不让前端猜
    toolsTextAvailable: Array.isArray(header.rawTools),
    toolsTextUnavailableReason: Array.isArray(header.rawTools)
      ? null
      : (Number(header.rawToolsChars) > 0
        ? `原始工具定义 ${header.rawToolsChars} 字符，超过驻留上限 ⇒ 内存里不留正文（点开单条仍可取）`
        : '这一楼没有原始工具定义'),
  }
}
...
async function messagesApi(source, params) {
  ...
  if (!found) return { ok: false, error: `会话不存在：${id}` }
  // ★ 2026-09-20（用户口径修正）：「这一字段在对话历史里**需要给出并标红**」——
  //   ⛔ 不再把它过滤掉：插件注入的 user 消息照旧是一行、照旧计数（计数里归 `injected`，不是 `player`），
  //   由**面板**按 `sourcePlugin` 标红 + 写注释。
  const rows = found.parsed.conversation.rows
  const skippedInjected = 0
  ...
  return {
    ok: true, sessionId: id, total, latestIndex: total === 0 ? null : total - 1, counts, skippedInjected,
