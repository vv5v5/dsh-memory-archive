  ...
  for (const ev of list) {
    ...
    switch (ev?.type) {
      ...
      case 'assistant/message':
        rows.push({ seq, time, role: 'assistant', text: blocksText(ev.data?.message?.content), isToolResult: false, isCompacted: false, blocks: blocksOf(ev.data?.message?.content) })
        break
      case 'tool/result':
        rows.push({ seq, time, role: 'tool', text: blocksText(ev.data?.message?.content), isToolResult: true, isCompacted: false, blocks: blocksOf(ev.data?.message?.content) })
        break
      case 'compaction/summary': {
        ...
        rows.push({ seq, time, role: 'user', text: blocksText(ev.data?.summary), isToolResult: false, isCompacted: true, blocks: blocksOf(ev.data?.summary) })
        break
      }
      default:
        break
