...
import { looksLikePromptEcho } from './summarize.js'
...
export function findShadowRegions(bySeq) {
  ...
  for (const seq of seqs) {
    ...
    const doc = {
      ...
      // ★ 2026-09-20 清洗②：把 `source.kind` 一并带进 region —— 归档侧据此**不把插件注入当成楼**
      sourceKind: typeof v.sourceKind === 'string' ? v.sourceKind : null,
    }
...
export function mapRegionToFloors(region, startFloor = 0) {
  ...
  for (const doc of (region && Array.isArray(region.docs) ? region.docs : []) || []) {
    ...
    // ★★ 2026-09-20 清洗②（用户口径：「也不被记忆库收录」）：`source.kind === 'plugin'` 的 user 消息
    //   **不是对话**（本插件注入的后处理提示词、官方运行上下文快照），⛔ 既不成楼、也不并进前一条楼
    //   —— 直接跳过（它的正文不该进归档，更不该进摘要与向量库）。判据是结构性的 kind，⛔ 不猜文本。
    //   ⚠️ `kind` 缺失（老日志）⇒ 照旧处理：那可能是真玩家消息，⛔ 不许因为"认不出"就把它丢掉。
    if (isPlainObject(doc) && doc.sourceKind === 'plugin') continue
 ...
 *           闸门是 lib/summarize.js 的 looksLikePromptEcho；`opts.instruction` 为空 = 拿不到
 *           当前生效的指令 ⇒ 只按结构判（不瞎猜，如实给不出 echo 判定）。
 *           命中时 planScan 会**大声播报**（warnings），绝不静默回落。）
 */
export function selectModelSummary(events, region, opts = {}) {
  ...
  if (instruction !== '' && looksLikePromptEcho(ev.text, instruction)) {
    return { kind: 'mechanical', reason: 'prompt-echo' }
  }
...
export async function planScan(input, deps) {
  ...
  for (const region of regions) {
    ...
    const sel = selectModelSummary(summaryEvents, region, { instruction: echoInstruction })
...
export async function collectOnce(ctx, opts = {}) {
  const { sessionId, target, auto, base, dshHome: dshHomeIn, collectSurfaces, expectedRevisions, maxFloorsPerRegion: maxFloors, instruction } = opts
  if (typeof sessionId !== 'string' || sessionId === '' || !isPlainObject(target) ||
      typeof target.characterId !== 'string' || target.characterId === '' ||
      typeof target.playthroughId !== 'string' || target.playthroughId === '') {
    throw mkErr('SCAN_INVALID', '需要 sessionId 与 target:{characterId,playthroughId}（非空字符串）')
...
async function runCollectRoute(ctx, req, send, log, collectSurfaces, auto, instruction) {
  try {
...
export async function handleCollectScan(ctx, req, send, log, collectSurfaces, instruction) {
  return runCollectRoute(ctx, req, send, log, collectSurfaces, false, instruction)
}

/** POST /magictarven/api/collect/auto —— 规划 + 落库 + 记台账（幂等）。`instruction` 同上。 */
export async function handleCollectAuto(ctx, req, send, log, collectSurfaces, instruction) {
  return runCollectRoute(ctx, req, send, log, collectSurfaces, true, instruction)
}
