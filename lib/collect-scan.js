...
async function runCollectRoute(ctx, req, send, log, collectSurfaces, auto) {
  try {
    ...
    const deps = {
      ...
      maxExistingFloor: (t) => tavernMaxExistingFloor(req, t),
      collect: {
        ...
        applyCollect: async (subPlan, opts) => {
          const backend = await importCollectBackend()
          const tavern = makeA1Tavern(backend, req)
          let plan = subPlan
          // 调用方的 expectedRevisions 覆盖（合并规则照抄 A1 handleCollectApply：只收 null/字符串）
          if (opts && isPlainObject(opts.expectedRevisions) && isPlainObject(subPlan) && isPlainObject(subPlan.expectedRevisions)) {
            const merged = { ...subPlan.expectedRevisions }
            for (const [k, v] of Object.entries(opts.expectedRevisions)) {
