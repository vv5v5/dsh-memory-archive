// ---------------------------------------------------------------------------
// last-floors —— 「**上下文很短**时注入最近几楼充当对话历史」的**纯函数核心**。
//
// ## 它解决什么
...
//
// ## 什么时候注入（★ 2026-09-19 用户口径改过）
// **不再**只看"首轮"：只要本会话**当前上下文的字数**小于阈值（`LAST_FLOORS_CONTEXT_CHARS`
// = 5000 字），就注入最近 N 楼；上下文长过阈值就停（不与真历史打架）。
//   · 字数 = 会话里**未被遮蔽**（没被压缩掉）的那些消息正文的总字数，**带外**算出来的缓存值；
//   · 算不出来时（刚重启/续跑，还没跑过一轮）**回退**到旧的"首轮"判据 —— 判不出就不注，
//     宁可少注一次，也不把已经进过模型的历史再喂一遍。
//
// ## 为什么只能做成「段」，不做成会话历史
...
// 宿主侧的装配（section 注册 / `system-prompt/assemble` 带外算字数 / 缓存）在 lib/index.js 的
// registerLastFloors。
//
// ## ★ 与「回响」的分工（别搞混）
// `<memoryEcho>` 是**按本轮内容检索**出的历史碎片（前身是本插件的 FTS5 关键词命中；
// 2026-09-26 起改由 `dsh-anima-rag` 的**语义命中**装）；本段是**按时间顺序**的最近几楼原文。
// 两者都以「已经发生过的事」为前提，前者靠命中，后者靠位置。
// ★ 2026-09-26（T3）：本插件那个 FTS5 版回响（`dma:echo`@54 + `lib/echo-index.js`
//   + `lib/echo-inject.js`）**已整块退役**，所以"分工"的说法如今是与 anima 那一格的分工。
//
// @license CC-BY-NC-4.0 (Attribution-NonCommercial 4.0 International)
// ---------------------------------------------------------------------------

/** 计划版本：行为变了就 +1（自检台钉住它）。★ 2 = 触发条件从"首轮"改成"上下文 < 5000 字"。 */
export const LAST_FLOORS_VERSION = 2
...
/** 默认保留几楼（2026-09-19 用户口径「注入前五楼」；实测中位数 272 字/楼、6 楼≈2000 字）。 */
export const DEFAULT_LAST_FLOORS_COUNT = 5

/**
 * 上下文阈值（字）。本会话**当前上下文**的字数 **< 这个数** 才注入 ——
 * 用户 2026-09-19 口径：「如果当前会话上下文数量少于 5000 字，自动注入前五楼的内容」。
 * ★ 2026-09-27（迁移局实测）：这是**默认值**，可在 config.json `lastFloors.contextChars` 覆写 ——
 *   「与 X 新开周目」的迁移局首轮上下文就有 ~25k 字（风格指令块 + 首轮长回复），5000 的窗
 *   永远 `context-full` ⇒ 播种了归档楼层也注不进去。覆写只在**更宽**时生效（见 readSwitch 夹紧）。
 */
export const LAST_FLOORS_CONTEXT_CHARS = 5000
...
export function readSwitch(configJson) {
  ...
  return {
    ...
    // ★ 2026-09-27：上下文阈值可覆写（迁移局首轮就有 ~25k 字，5000 的窗永远 context-full）。
    //   ⛔ 只许**放宽**：夹紧下限 = 默认 5000 —— 这个功能的语义是"薄上下文兜底"，
    //   配得比默认还小只会让它更早闭嘴，没有意义（还会让"为什么没注"更难排查）。
    contextChars: safeInt(lf.contextChars, LAST_FLOORS_CONTEXT_CHARS, LAST_FLOORS_CONTEXT_CHARS, 200000),
  }
}
...
/**
 * 前置门（纯函数）：**本会话现在值不值得考虑注入**。
 *   · 上下文算得出来 ⇒ 只在 **< LAST_FLOORS_CONTEXT_CHARS** 时为真（够长就停，不与真历史打架）；
 *   · 算不出来（刚重启/续跑，还没跑过一轮）⇒ **回退**到"是不是首轮"（判不出就不注）。
 *
 * @param {{ contextChars?: unknown, isFirstTurn?: unknown }} input
 * @returns {boolean}
 */
export function contextIsShort(input) {
  const i = input ?? {}
  const known = Number.isFinite(i.contextChars) && i.contextChars >= 0
  // ★ 2026-09-27：阈值可由调用方带 `contextLimit` 覆写（config `lastFloors.contextChars`）；
  //   不带 = 常量默认（老行为一字不变）。
  const limit = Number.isFinite(i.contextLimit) && i.contextLimit > 0 ? i.contextLimit : LAST_FLOORS_CONTEXT_CHARS
  if (known) return i.contextChars < limit
  return i.isFirstTurn === true
}
 ...
 *   contextChars - 本会话**当前上下文**的字数（带外算出来的缓存值；不是有限数 = 还没算出来）。
 *   isFirstTurn  - 仅当 contextChars **算不出来**时的回退判据。
 *   floors       - 现周目归档楼层（readArchiveFloors 的产物）。
 *   text         - renderRecentFloors 的产物（空串 = 渲染不出东西）。
 * @returns {{ inject: boolean, reason: 'switch-off'|'context-full'|'context-unknown'|'no-floors'|'empty-render'|'short-context'|'first-turn' }}
 *
 * 真值表（自检台逐行钉住）：
 *   enabled !== true                        ⇒ { false, 'switch-off'     }
 *   字数已知且 ≥ 5000                       ⇒ { false, 'context-full'   }（★ 上下文够长就不注）
 *   字数未知 且 不是首轮                    ⇒ { false, 'context-unknown'}（判不出 ⇒ 少注一次）
 *   楼层为空                                ⇒ { false, 'no-floors'      }
 *   text === ''                             ⇒ { false, 'empty-render'   }
 *   字数已知（且 < 5000）                   ⇒ { true,  'short-context'  }
 *   否则（未知但首轮）                      ⇒ { true,  'first-turn'     }
 */
export function decideInject(input) {
  const i = input ?? {}
  if (i.enabled !== true) return { inject: false, reason: 'switch-off' }
  // ★ 2026-09-27：阈值可覆写（与 contextIsShort 同一份 `contextLimit`），不带 = 常量默认。
  const limit = Number.isFinite(i.contextLimit) && i.contextLimit > 0 ? i.contextLimit : LAST_FLOORS_CONTEXT_CHARS
  const known = Number.isFinite(i.contextChars) && i.contextChars >= 0
  if (known && i.contextChars >= limit) return { inject: false, reason: 'context-full' }
  if (!known && i.isFirstTurn !== true) return { inject: false, reason: 'context-unknown' }
  ...
  return { inject: true, reason: known ? 'short-context' : 'first-turn' }
}
