 ...
 * 且每段的 `offset/chars` 能从那一段里切出该段。而装配瀑布里**原理上拿不到最终正文**：
 *
 * · **anima**（`dsh-anima-rag/lib/index.js:1029`）是 **`next()` 之后才改写** ——
 *   它的产出只有瀑布的**调用方**（DSH 自己）看得见，任何监听器都拿不到 ⇒ 排多后都没用；
 * · DSH 在瀑布返回**之后**还会做 `complete` 段覆盖（`core/system-prompt/src/index.ts:601-610`）；
 * · 实测：捕获在装配期算出的整段凭据 = 8931/10100，而日志里最终系统消息 = 10997/12074，
 *   差的就是 anima / 世界书在瀑布里补进去的内容；且 anima 一注入，它**之后**所有段的
 *   offset 全是错的（之前的"偶尔能对上"只是因为那一楼恰好没有下游注入）。
 *
 * ⇒ 只能等 DSH 把**最终正文**写进会话日志（`system/message` 事件）之后再定位。
 *
 * ## 定位口径（顺序匹配，⛔ 不猜）
