 ...
 *            total:number, renderedChars:number|null}}
 */
export function locateSections(sections, finalText, opts = {}) {
  ...
  if (text === '') {
    return { ok: false, reason: 'no-final-text', offsets, matched: 0, exact: 0, anchored: 0, total: 0, renderedChars: null }
  }
  ...
  if (total > 0 && exact === total) {
    return { ok: true, reason: 'all-matched', offsets, matched: exact, exact, anchored: 0, total, renderedChars: text.length }
  }
