  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      function RpMemoryFlow({ scrollBind }) {
        ...
        //   它上面那两个动作（「把档案退回第 N 楼」复用 `gotoFloor`（**动正文**）；
        //   「保持现状」走 `settlePending` ＝ **就地登记**（⛔ 正文一个字节都不写，只把账对齐））。
        ...
        // ★ 2026-09-24（回档改手动挡）：档顶那条横幅上「保持现状」的忙态/回执。
        ...
        /**
         * 楼层清单（`GET /playthrough/rp-memory/floors`）—— 宿主现算：每楼序号 / 时间 / 改了哪几份（+几字节）/
        ...
        const loadFloors = () => {
          ...
          ;(async () => {
            try {
              const data = await requestJson(HOST_API_BASE + '/playthrough/rp-memory/floors', controller.signal)
              if (controller.signal.aborted) return
        ...
        /**
         * 「回到这一楼」/「把档案退回第 N 楼」：**手动**触宿主那**同一个恢复函数**（★ 2026-09-24 起
        ...
        const gotoFloor = (nodeId, variantId) => {
          ...
          setGotoMsg({ status: 'busy', nodeId: nodeId, message: '', detail: null })
          ;(async () => {
            try {
              const data = await apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/restore', undefined, { nodeId: nodeId, variantId: variantId })
              setGotoMsg({
              ...
              const rest = p && p.restore && typeof p.restore === 'object' ? p.restore : null
              const msg = p && p.error && typeof p.error.message === 'string' && p.error.message !== ''
                ? p.error.message
                : errText(error)
              setGotoMsg({ status: 'err', nodeId: nodeId, message: msg, detail: rest })
            } finally {
              loadFloors()   // 恢复之后重取一次清单（当前那一楼、每楼的样子都可能变了）
            }
          })()
        }

        /**
         * ★★ 「保持现状」（`POST /playthrough/rp-memory/floors/pending/settle`，2026-09-24）——
         * 语义是 **就地登记**（用户口径：「跟 Tavern 一致 —— 回档之后那段历史就是没有了，记忆回档之后
         * 也直接按新的来」）：宿主把**当下盘上那 5 份**的现文记成"剧情现在站的这一楼"那一份快照、
         * 并把账（`last`）锚到它 ⇒ 之后判据不再认为"还在回档"，记录无缝续上。
         * ⛔ 那 5 份的**正文一个字节都不写**（"退回"那条路是 `gotoFloor`）、⛔ 不影响会话。
         * 带上**面板看到的那条** `pending` 的 `targetKey`：跟宿主当下那条不一致（这中间又发生了
         * 另一次回档、目标换了）⇒ 宿主回 409，面板如实红字 —— ⛔ 绝不"按老目标就地登记"。
         */
        const settlePending = () => {
          if (pendingMsg.status === 'busy') return
          if (pendingKey === '') return
          setPendingMsg({ status: 'busy', message: '' })
          ;(async () => {
            try {
              const data = await apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/pending/settle', undefined, { targetKey: pendingKey })
              setPendingMsg({ status: 'ok', message: typeof data.message === 'string' && data.message !== '' ? data.message : '已「保持现状」' })
            } catch (error) {
              const p = error && error.payload && typeof error.payload === 'object' ? error.payload : null
              const msg = p && p.error && typeof p.error.message === 'string' && p.error.message !== ''
                ? p.error.message
                : errText(error)
              setPendingMsg({ status: 'err', message: msg })
            } finally {
              loadFloors()   // 横幅的判据在宿主那边（那条 pending）⇒ 清完重取一次清单
            }
          })()
        }
        ...
        const floorAuto = floorData && floorData.lastAuto ? floorData.lastAuto : null
        // ── ★★ 2026-09-24「**回档改成手动挡**」：档顶那条**显著**横幅 ──────────────────────
        //   用户原话：「**不要做跟随楼层的功能，先做手动挡**，开 fork 时（回档）**前台弹提示，让用户手动
        //   回退剧情档案**。做**更显著的更改表示**」。判据**全在宿主**（那条 `pending` 是宿主记的、
        //   楼号与目标也是宿主算的）：面板只把它摊成人话、把两个动作发出去。
        //   · 「把档案退回第 N 楼」= 复用 `gotoFloor`（宿主那**同一个**恢复端点，⛔ 不另开一条路）——
        //     **动正文**：把笔记写回目标那一楼的样子；
        //   · 「保持现状」= `settlePending` ＝ **就地登记**（★ 2026-09-24 用户口径：跟 Tavern 一致，
        //     回档之后那段历史就是没有了 ⇒ 认下这个新起点）—— 宿主把**当下盘上那 5 份**记成
        //     "剧情现在站的这一楼"那一份快照、把账锚到它；⛔ **正文一个字节都不写**。
        ...
        /** 楼号的人话（认不出就如实说"未知楼层"，⛔ 不编一个 0 出来）。 */
        const floorSeat = (n) => (Number.isFinite(n) ? '第 ' + String(n) + ' 楼' : '未知楼层')
        /** 那条显著横幅：**色块（琥珀）+ 加粗 + ⚠ 图标**（用户明确否掉了"一行小灰字"那种做法）。
         *  文案必须带上「不影响会话」（用户在验收口径里点了名）。 */
        const floorPendingBanner = floorPending === null ? null : e('div', {
          ...
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', fontSize: 14, fontWeight: 700 } },
            ...
            '检测到回档：' + floorSeat(floorPending.fromSeq) + ' → ' + floorSeat(floorPending.toSeq)
              + '（剧情档案还停在 ' + floorSeat(floorPending.fromSeq) + '）'),
          e('div', { style: { fontSize: SMALL_FONT_SIZE, lineHeight: 1.6, opacity: 0.85 } },
            '剧情已经' + (floorPending.why === 'variant' ? '在同一楼换了变体（swipe 重 roll）' : '退回前面那一楼')
            + '，但剧情档案（模型维护的那 5 份笔记）还停在原地 —— 跟 Tavern 一样，回档之后那段历史就按新的算，'
            + '要不要把档案也退回去由你定。'
            + (floorPending.source === 'prev'
              ? '（' + floorSeat(floorPending.toSeq) + '这一支还没记过快照 ⇒ 要退回的是"进这一楼之前"的样子，用的是' + floorSeat(floorPending.targetSeq) + '那一份。）'
              : '')),
          e('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' } },
            e('button', {
              id: 'dma-rpmem-floor-pending-restore', className: 'dma-btn', style: btnStyle,
              disabled: gotoMsg.status === 'busy' || pendingMsg.status === 'busy',
              title: '把剧情档案（那 5 份笔记）恢复到' + floorSeat(floorPending.targetSeq)
                + '的样子 —— 恢复前先把你现在的样子另存一份，可撤销。⛔ 不影响会话；⛔ 不碰预置那几份，⛔ 死区那几段保留盘上现况。',
              onClick: () => gotoFloor(floorPending.targetNodeId, floorPending.targetVariantId),
            }, '把档案退回' + floorSeat(floorPending.targetSeq)),
            e('button', {
              id: 'dma-rpmem-floor-pending-settle', className: 'dma-btn', style: btnStyle,
              disabled: pendingMsg.status === 'busy' || gotoMsg.status === 'busy',
              title: '就地登记：认下现在这个起点 —— 把「现在这份档案」记成' + floorSeat(floorPending.toSeq)
                + '的样子，往后从这一楼接着记。⛔ 那 5 份的正文一个字节都不写（只动我们自己的清单和快照），⛔ 不影响会话。',
              onClick: settlePending,
            }, '保持现状')),
          e('div', { style: dimStyle },
            '两个按钮都不影响会话（会话与时间线一个字节都不碰）；「把档案退回」会改那 5 份笔记的正文'
            + '（写回目标那一楼的样子，改之前先另存一份、可撤销）；「保持现状」不改正文 —— '
            + '只把账就地对齐到当前这一楼，之后从这一楼接着记。'),
          ...
          }, pendingMsg.status === 'busy' ? '正在就地登记…' : pendingMsg.message),
        )
        ...
        const floorRow = (f) => e('div', {
          ...
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' } },
            e('span', { style: { fontFamily: MONO, fontWeight: f.current === true ? 700 : 400 } },
              '第 ' + String(Number.isFinite(f.seq) ? f.seq : '?') + ' 楼'),
            f.current === true ? e('span', { style: Object.assign({}, dimBadgeStyle, { fontWeight: 700 }) }, '◀ 当前这一楼') : null,
            // 同一楼 swipe 出来的**另一支**：如实标一句（⛔ 不把它当"当前这一楼"）
            f.current !== true && f.sameNode === true && f.legacy !== true
              ? e('span', { style: dimBadgeStyle }, '同一楼的另一支（swipe 掉的）') : null,
            e('span', { style: dimStyle }, (f.updatedAt || f.at) ? new Date(f.updatedAt || f.at).toLocaleString() : '—'),
            e('span', { style: { flex: 1, minWidth: 0, fontSize: SMALL_FONT_SIZE, color: 'GrayText' } },
              f.legacy === true
                ? '（老记录：只有楼层、不知道是哪一支）'
        ...
        const gotoLine = gotoMsg.status === 'idle' ? null : e('div', {
          ...
          style: gotoMsg.status === 'err' || gotoBlocked ? errorStyle : (gotoMsg.status === 'ok' ? okStyle : dimStyle),
        },
          gotoMsg.status === 'busy' ? '正在把笔记恢复到那一楼…' : (gotoMsg.message || '已恢复'),
          gotoMsg.detail && Array.isArray(gotoMsg.detail.failed) && gotoMsg.detail.failed.length > 0
            ? e('div', { style: { marginTop: 3 } }, '⚠ 没写进去：'
                + gotoMsg.detail.failed.map((x) => String(x.name) + '（' + String(x.reason) + '）').join('、')
                + ' —— 其余照旧，不算全成功')
            : null,
          gotoMsg.detail && Array.isArray(gotoMsg.detail.restored) && gotoMsg.detail.restored.length > 0
            ? e('div', { style: { marginTop: 3 } }, '已写回：'
                + gotoMsg.detail.restored.map((x) => String(x.name) + (x.backup ? '（原样备份成 ' + String(x.backup) + '）' : '（这一份是新建的）')).join('、'))
            : null,
          gotoMsg.detail && Array.isArray(gotoMsg.detail.skipped) && gotoMsg.detail.skipped.length > 0
            ? e('div', { style: { marginTop: 3 } }, '跳过：' + floorSkipText(gotoMsg.detail.skipped))
            : null,
        )
        /** 那一句「最近一次回档」（没有就什么都不显示 —— ⛔ 不占位置、不写"暂无"）。
         *  ★ 2026-09-24 改口径：回档**不再自动跟随**（判到只提示、写盘要人点）⇒ 这句不再叫"自动回档跟随"，
         *   它现在装的是**最近一次回档事件**（可能是"检测到、没写盘"，也可能是"你在面板上手点恢复的那一脚"）。 */
        const floorAutoLine = floorAuto === null ? null : e('div', { 'data-dma': 'floor-auto', style: dimStyle },
          '⇄ 最近一次回档：'
          + (typeof floorAuto.message === 'string' && floorAuto.message !== '' ? floorAuto.message : '（详见日志）')
          + (typeof floorAuto.at === 'string' && floorAuto.at !== '' ? '（' + new Date(floorAuto.at).toLocaleString() + '）' : ''))
        /** 整块（列表 + 空态 + 拦路的那几条如实说明）—— 排在文件清单**下面**。 */
        const floorBlock = e('div', { 'data-dma': 'floor-block', style: { display: 'flex', flexDirection: 'column', gap: 6, marginTop: 16 } },
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' } },
            e('span', { style: { fontWeight: 700 } }, '按楼层绑定的笔记快照'),
            e('span', { style: dimStyle }, '每轮模型改过笔记就记一份，绑在「楼层」（时间线的节点）上；检测到回档只在档顶弹提示，⛔ 不自动跟随 —— 写盘要你自己点'),
          ),
          floorPane.status === 'loading' ? e('div', { style: dimStyle }, '正在读楼层快照…') : null,
          floorPane.status === 'error' ? e('div', { style: errorStyle }, '楼层快照读不出来：' + floorPane.error) : null,
          floorData && floorData.docError
            ? e('div', { 'data-dma': 'floor-doc-error', style: errorStyle, role: 'alert' },
          ...
          floorData && floorData.deadzonesReadable === false
            ? e('div', { 'data-dma': 'floor-deadzone-unknown', style: errorStyle, role: 'alert' },
                '⚠ 死区数据读不出来 ⇒ 认不出哪几段是死区 ⇒「把档案退回第 N 楼」那次不会动手（宁可不动，也不赌那几段不在死区里）。')
            : null,
          floorData && floorData.head && !floorRows.some((f) => f.current === true)
            ? e('div', { 'data-dma': 'floor-current-none', style: dimStyle },
                '当前在第 ' + String(Number.isFinite(floorData.head.seq) ? floorData.head.seq : '?') + ' 楼 —— 这一楼还没有快照，笔记保持原样。')
            : null,
          floorData && floorData.orderKnown === false
            ? e('div', { 'data-dma': 'floor-order-unknown', style: dimStyle },
          ...
          floorPane.status === 'ready' && floorRows.length === 0 && !(floorData && floorData.docError)
            ? e('div', { 'data-dma': 'floor-empty', style: dimStyle },
                '还没有楼层快照 —— 模型改过笔记之后（轮末）会自动记一份。')
            : null,
          floorRows.map(floorRow),
          gotoLine,
          e('div', { style: dimStyle },
            '只记这 5 份（模型维护的那几份）：'
        ...
        return e('div', { style: colFillStyle },
          ...
          gate === 'ready'
            ? e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
              ...
              alert.status === 'error' ? e('div', { style: errorStyle }, '死区现状读不出来：' + alert.error) : null,
              alertData && alertData.docError
                ? e('div', { 'data-dma': 'deadzone-doc-error', style: errorStyle, role: 'alert' },
                    '⚠ 死区数据（' + String(alertData.file || '.dma-deadzones.json') + '）读不出来（' + String(alertData.docError) + '）'
                    + ' —— 作者设的禁改区这次没能生效（约束没喂给模型、也没在比对）。请到那份文件里看一眼。')
                : null,
              alertBad.length > 0
                ? e('div', { 'data-dma': 'deadzone-alert', style: { display: 'flex', flexDirection: 'column', gap: 3 }, role: 'alert' },
                    e('div', { style: Object.assign({}, errorStyle, { fontWeight: 700 }) },
                      '⛔ 死区内容与快照不符（可能是模型改的，也可能是你在别处改过）—— 共 ' + String(alertBad.length) + ' 处：'),
                    alertBad.slice(0, 12).map((i, idx) => e('div', { key: 'dz-alert-' + idx, style: { fontSize: SMALL_FONT_SIZE, color: 'crimson', fontFamily: MONO, wordBreak: 'break-all' } }, '· ' + alertLine(i))),
              ...
              alertData && alertData.docError === null && alertData.zones > 0 && alertBad.length === 0
                ? e('div', { style: dimStyle, 'data-dma': 'deadzone-ok' },
              ...
              alertData && alertData.zones > 0 && alertData.sharedHint === true
                ...
                : null,
              // ★ 2026-09-23：那句「最近一次回档」（没有就什么都不显示；★ 2026-09-24 起不叫"自动回档跟随"了）
              floorAutoLine,
              rows,
              // ★ 2026-09-23：「按楼层绑定的笔记快照」整块（列表 + 「回到这一楼」+ 如实说明）
              floorBlock,
            ))
            : null,
        )
      ...
      function computeSources(mode, tavernOk, eventsSessionId) {
        ...
        return [['summaries', '摘要'], ['floors', '原文'], ['outline', '剧情大纲'], ['vector', '向量'], ['compact', '压缩']]
      }
