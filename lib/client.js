...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      const KNOB_DISABLED_TITLE = 'P2 才实现写入'
      ...
      // ---------- v5 P0：可写项块（本版只读：4 类 knob 当前值 + 「面板值 vs preset 实际值」一致性） ----------
      // 按钮（预览差异/应用/回滚）渲染但禁用，悬停理由 = KNOB_DISABLED_TITLE。⛔ 本版零写入：
      // 这里没有任何写盘代码路径，模板编辑（压缩指令/收纳占位）仍只在「记忆库」面板里。
      function KnobsPanel({ agent }) {
        const [open, setOpen] = React.useState(true)
        ...
        const consistentBadge = (consistent) => consistent === true
          ...
          : consistent === false
            ? e('span', { key: 'c', style: warnBadgeStyle, title: '★ 面板里的值与 preset 实际值不同 —— 面板改了也不会生效（写入面是 P2）' }, '不一致 ✗')
        ...
        return e('div', { style: Object.assign({}, colTurnStyle, { width: 270 }) },
          e('div', { style: { display: 'flex', gap: 4, alignItems: 'center', flex: 'none' } }, toggleBtn),
          e('div', { style: Object.assign({}, dimStyle, { flex: 'none' }) }, '4 类 knob（本版只读展示）'),
          knobRow('压缩指令',
            panelTextOf(tplComp),
          ...
          e('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', flex: 'none' } },
            e('button', { className: 'dma-btn', style: btnStyle, disabled: true, title: KNOB_DISABLED_TITLE }, '预览差异'),
            e('button', { className: 'dma-btn', style: btnStyle, disabled: true, title: KNOB_DISABLED_TITLE }, '应用'),
            e('button', { className: 'dma-btn', style: btnStyle, disabled: true, title: KNOB_DISABLED_TITLE }, '回滚'),
          ),
          e('div', { style: dimStyle }, '本版只读：只展示当前值与一致性，不写任何文件（写入面是 P2）。'),
        )
      }
        ...
        return e('div', { style: cardStyle },
          ...
          rows.map((b) => e('div', { key: String(b.file), style: itemStyle },
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', fontSize: 12 } },
              ...
              e('button', {
                className: 'dma-btn dma-mini', style: miniBtnStyle, disabled: true,
                title: '回滚操作等 P2 实现（本版只列出备份；手工回滚 = 把该文件内容盖回 agent.cordis.yml）',
              }, '回滚')))),
        )
      }
      ...
      function AgentEditorPanel({ onClose }) {
        ...
        React.useEffect(() => {
          ...
          const candidates = Array.isArray(detectReadyData.candidates) ? detectReadyData.candidates : []
          const writable = candidates.filter((c) => c && c.writable === true).map((c) => String(c.id))
          const target = apply.presetId != null && writable.includes(String(apply.presetId))
            ? apply.presetId
          ...
          return () => controller.abort()
        }, [detect.open, detectReadyData, apply.presetId, backupsTick])
