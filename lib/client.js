...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      function ViewerSessionList({ state, hostById, hostError, selectedId, onSelect, onRefresh, nameIdx, width }) {
        const [filter, setFilter] = React.useState('')
      ...
      function AgentEditorPanel({ onClose }) {
        ...
        React.useEffect(() => {
          ...
          const handle = schedule(() => {
            ...
            apiGet(PROMPT_API_BASE + '/api/sessions/resolve?ids=' + encodeURIComponent(batch.join(',')), controller.signal)
              .then((data) => {
                if (controller.signal.aborted) return
                for (const row of Array.isArray(data.sessions) ? data.sessions : []) {
                  // 非 number 一律不折 0（mergeRow 内部保持原值）—— 解析失败由宿主以 -2 哨兵表达
                  // ★ M8：readError 原样带进列表（成功行是显式 null = 已知能读；缺字段 = 不碰）
                  mergeRow(row.id, String(row.title == null ? '' : row.title), row.requests,
                    row.readError === undefined ? null : row.readError)
                }
              })
              .catch(() => {
