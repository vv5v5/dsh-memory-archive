/**
 * magictarven —— 客户端半侧（浏览器 bundle）v4.1 A 单「入口拆分 + 查看器对齐 DSH + 完整提示词 + 版块注释」
 ...
 *   数据面根是 /magictarven/prompt；原插件的外壳与席位注册一律不带进来，两处入口
 ...
 *
 * 宿主 API（根路径 /magictarven/api，与宿主半侧契约逐字对齐）：
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      const HOST_API_BASE = '/magictarven/api'
      const PROMPT_API_BASE = '/magictarven/prompt'   // 吸收自 dsh-prompt-viewer 的数据面（宿主半侧 ./prompt-viewer.js）
      ...
      function RootModePicker({ config, health, saveMode, modeBusy, modeError }) {
        ...
        const radio = (mode, label, desc, disabled, reason) => e('label', {
          ...
          e('input', {
            type: 'radio', name: 'magictarven-root-mode', className: 'dma-radio',
