      ...
      function TemplateCard({ templateKey, sessionId }) {
        ...
        const applyToPreset = (value, okText) => {
          ...
          ;(async () => {
            ...
            const url = AGENT_API + '/apply'
            await mutateJson(url, 'POST', { presetId: presetId, dryRun: true }, controller.signal)
            if (controller.signal.aborted) return
            const out = await mutateJson(url, 'POST', { presetId: presetId, dryRun: false }, controller.signal)
            if (controller.signal.aborted) return
          ...
          e('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' } },
            ...
            isCompaction
              ? e('button', {
                ...
                onClick: () => applyToPreset(draft, '已写进预设（压缩后端 customInstruction）'),
        ...
        return e('div', { style: { display: 'flex', flexDirection: 'column', gap: 10, padding: 12, overflow: 'auto', minHeight: 0 } },
          ...
          e('div', { style: cardStyle },
            ...
            e('div', { style: step },
              ...
              e('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
                e('button', { className: 'dma-btn dma-tab', disabled: !targetReady || text.trim() === '' || busy !== '', onClick: () => doPlan({ text }) }, busy === 'plan' && srcPath === '' ? '预览中…' : '预览粘贴的内容'),
