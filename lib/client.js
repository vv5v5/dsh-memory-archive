  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      function v3DigestLines(v3) {
        const out = []
        if (!v3 || typeof v3 !== 'object') return [{ label: '数据面', value: '取不到（响应形状不符）', kind: 'error' }]
        const tav = v3.tavern || {}
        if (v3.ok !== true) {
        ...
        out.push({ label: 'Tavern v3', value: '可达（HTTP ' + String(tav.status ?? '?') + '）· apiVersion ' + String(cap.apiVersion ?? '?') + ' · modes ' + JSON.stringify(cap.modes || []) })
        out.push({ label: '组合器', value: '已注册 ' + JSON.stringify(v3.composer?.owners || []) + (v3.composer?.enabled ? '（我们：' + String(v3.composer.owner) + '）' : '（我们未启用）') })
        if (v3.composer?.needsHostRestart) out.push({ label: '⚠ 待生效', value: '配置说启用但本进程没注册上 —— 改完这条要重启宿主', kind: 'warn' })
      ...
      function V3Panel({ sessionId }) {
        ...
        const body = e('div', { style: { display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 } },
          ...
          e('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' } },
            e('button', {
              className: 'dma-btn dma-mini', style: miniBtnStyle, disabled: !sessionId || mode === 'external',
              title: '把**这个会话**切到 external：之后 Tavern 默认 profile 正文不再产生，改由我们的组合器产出命名段',
            ...
            e('button', {
              className: 'dma-btn dma-mini', style: miniBtnStyle, disabled: !sessionId || mode === 'builtin',
              title: '把**这个会话**切回 builtin：装配交回 Tavern 内置策略',
            ...
            e('button', {
              className: 'dma-btn dma-mini', style: miniBtnStyle, disabled: act.busy,
              title: '打开/关闭**我们自己**的 v3 组合器（写 config.v3.composer.enabled）。⚠️ 改完要重启宿主才生效',
      ...
      const LAST_FLOORS_DETAIL = [
        '只在新周目的**第一句话**注入：会话里已经有真历史时它一个字都不注（不与真历史打架）。',
