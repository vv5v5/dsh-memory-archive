    ...
    return (() => {
      ...
      function RpMemoryFlow({ scrollBind }) {
        ...
        const reveal = () => {
          ...
          ;(async () => {
            try {
              ...
              if (controller.signal.aborted) return
              if (data.exists === false) { setGate('empty'); return }
              setGate('ready')
              setSt({
        ...
        return e('div', { style: colFillStyle },
          ...
          gate === 'empty' && e('div', { style: dimStyle }, '这个周目还没有记忆库（.roleplay-memory/）。'),
          gate === 'error' && e('div', { style: errorStyle }, '读取失败：' + st.error),
          gate === 'ready'
            ? e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
              e('div', { style: dimStyle }, '落点：' + (st.baseLabel || '未知') + (st.sharedHint ? ' · 记忆库按会话工作目录存放，可能被同一工作区里的多个周目共用。' : '')),
      ...
      // 会话模式只剩会话事件；工作区模式 摘要/原文/状态/剧情大纲（20260919 加第四档）；
      ...
      function computeSources(mode, tavernOk, eventsSessionId) {
        ...
        // ★ 第四档「剧情大纲」排在「状态」之后（用户口径：入口与摘要/原文/状态同层）。
        //   它的防剧透门在档内组件里 —— 排序只管入口位置，不管可见性。
        return [['summaries', '摘要'], ['floors', '原文'], ['state', '状态'], ['outline', '剧情大纲']]
      }
