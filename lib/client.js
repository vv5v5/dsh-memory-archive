...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      // 20260918 消息流单 T1：messages 栏已退出这套并列页签（那一栏只做消息流，见 PromptPartView）；
      // 本表与组件**只剩 FullPromptView 在用**（深链 box=full 才可达）——按任务书「别处还在用就只拆本栏」处理。
      ...
      function AgentEditorPanel({ onClose }) {
        ...
        const statusLine = [
          ...
          sessionId && part ? 'L3 详细 · ' + (part === 'full' ? '完整' : part) + (part === 'messages' && msgSeq != null ? ' · seq ' + msgSeq : '') : null,
          note,
        ].filter(Boolean).join(' | ')
                  ...
                  : part === 'full'
                    ...
                    : part === 'tools'
                      ...
                      : e('div', { 'data-l3-fallback': '1', style: colPartStyle },
                        e('div', { style: dimStyle },
                          '这一栏只做消息流（20260918 拆掉并列部件视图）：' + String(part) + ' 由装配地图覆盖 —— 请从地图对应行下钻。')),
            ),
