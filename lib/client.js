  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      function RpMemoryFlow({ scrollBind }) {
         ...
         * 当前站在哪一楼 / 最近一次自动回档跟随。⛔ 这里**一个字的判断都不做**（序号、当前那一楼、
         * 该不该高亮全是宿主给的）；只在**手动确认之后**与几个动作之后发，⛔ 不挂定时器、不轮询。
         */
        const loadFloors = () => {
        ...
        /**
         * 「回到这一楼」：**手动**触宿主那**同一个恢复函数**（与自动回档跟随共用一份）——
        ...
        /** 档顶那一句「最近一次**自动**回档跟随」（没有就什么都不显示 —— ⛔ 不占位置、不写"暂无"）。 */
        ...
        const floorBlock = e('div', { 'data-dma': 'floor-block', style: { display: 'flex', flexDirection: 'column', gap: 6, marginTop: 16 } },
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' } },
            ...
            e('span', { style: dimStyle }, '每轮模型改过笔记就记一份，绑在「楼层」（时间线的节点）上；回档时自动跟着回去'),
          ),
          ...
          gate === 'ready'
            ? e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
              ...
              alertData && alertData.zones > 0 && alertData.sharedHint === true
                ...
                : null,
              // ★ 2026-09-23：档顶那句「最近一次自动回档跟随」（没有就什么都不显示）
              floorAutoLine,
              rows,
