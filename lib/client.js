      ...
      function editorV2FixtureLookup(url) {
        ...
        if (p === PROMPT_API_BASE + '/api/part') {
          ...
          if (part === 'messages') {
            ...
            // 第 1 / 3 楼：第 2 楼的数据换掉前几条并改楼号 ⇒ 各楼内容确实不同（fixture 域内构造）
            ...
            variant.messages = variant.messages.map((m, i) => Object.assign({}, m, {
              ...
              preview: '（fixture 第 ' + turnN + ' 楼历史）' + String(m.preview || '').replace(/^（fixture[^）]*）/, ''),
            }))
            variant.text = '（fixture 构造文本 —— 这是第 ' + turnN + ' 楼实际发出的历史）\n\n'
              + variant.messages.map((m) => m.role + ': ' + m.preview).join('\n')
            return variant
          }
        ...
        React.useEffect(() => {
          ...
          Promise.all(jobs).then(() => {
            ...
            if (out.sections && out.sections.source) {
              setMapState({
                status: 'ready',
                data: buildMapFromSections(out.sections, { messagesText: out.messages ? out.messages.text : null }),
