...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      // ---------- 设置视图 C：向量检索 API（retrieval.url/model/rerankModel/key；密钥纪律与 api.key 一字不差） ----------
      // 20260918：这张卡从「总结侧路的渠道设置」改为「向量检索 API」，收整套四项。
      // 消费方是 dsh-anima-rag：它直接读本插件的配置文件（生效顺序只有两层：retrieval.* → anima 默认值）。
      // ⛔ 不存在"别处同名项以本卡为准"的覆盖规则 —— 这四项的唯一真相就是本配置文件。
      function ApiSettings({ config, reload }) {
        ...
        React.useEffect(() => { setUrl(savedUrl); setModel(savedModel); setRerankModel(savedRerankModel); setKey('') }, [config])
        ...
        function doClear() {
          runPut('clear', { retrieval: { key: '__CLEAR__' } }, '已清除密钥，并已回读配置同步界面')
        }
        function doTest() {
          if (ctl.current) ctl.current.abort()
          const controller = new AbortController()
          ...
          ;(async () => {
            ...
            if (data && typeof data.dims === 'number') bits.push('向量维度 ' + String(data.dims))
        ...
        return e('div', { style: cardStyle },
          ...
          e('label', { style: fieldLabelStyle }, '接口地址',
            e('input', { id: 'dma-retrieval-url', name: 'dma-retrieval-url', 'aria-label': '向量检索 baseURL', style: inputStyle, value: url, placeholder: 'API baseURL（重排端点 = 本地址 + /rerank）', onChange: (ev) => setUrl(ev.target.value) })),
          e('label', { style: fieldLabelStyle }, '向量模型',
            e('input', { id: 'dma-retrieval-model', name: 'dma-retrieval-model', 'aria-label': '向量模型名', style: inputStyle, value: model, placeholder: 'embedding 模型名', onChange: (ev) => setModel(ev.target.value) })),
          e('label', { style: fieldLabelStyle }, '重排模型',
            e('input', { id: 'dma-retrieval-rerank-model', name: 'dma-retrieval-rerank-model', 'aria-label': '重排模型名', style: inputStyle, value: rerankModel, placeholder: 'rerank 模型名', onChange: (ev) => setRerankModel(ev.target.value) })),
          e('label', { style: fieldLabelStyle }, '密钥',
            e('input', {
              id: 'dma-retrieval-key', name: 'dma-retrieval-key', 'aria-label': '密钥（只写不读）',
          ...
          e('div', { style: dimStyle }, '这四项的唯一真相就是本配置文件（预设不参与检索配置）；测试连接对 <接口地址>/embeddings 打一次最小请求。'),
