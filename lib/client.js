  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      const DMA_CSS = [
        ...
        // ---- ★★ 2026-09-24：顶部那条「回档立刻弹」的浮层（`dma-floor-popup`）----
        // 用户口径：「最好是从**上方弹出**的那种」⇒ 常态停在屏幕外（`translateY(-100%)`）、
        //   `data-shown="1"` 时滑到 `translateY(0)` —— **进/出各一条 transition**，方向天生就是"从上面下来"。
        // ⚠️ 位置/变换/过渡只能写在这儿（内联表达不了 `[data-shown]` 两态，也写不了 `transition` 的方向）；
        //   颜色照旧只用系统色关键字 + color-mix；z-index 压得住 DSH 界面与面板那几层（999/1000）。
        // ⚠️ 省电模式：不加过渡 ⇒ 计算出来的 `transitionDuration` 是 0s ⇒ 浮层**当场收**
        //   （客户端那段两段式收口的兜底判据，⛔ 不会卡在屏上收不掉）。
        '.dma-floor-popup { position: fixed; top: 0; left: 50%; z-index: 1100; box-sizing: border-box; width: max-content; max-width: min(560px, 92vw); display: flex; flex-direction: column; gap: 6px; padding: 10px 14px 12px; border-radius: 0 0 10px 10px; background: color-mix(in srgb, DarkGoldenrod 12%, Canvas); color: CanvasText; border: 2px solid color-mix(in srgb, DarkGoldenrod 55%, transparent); border-top: none; box-shadow: 0 6px 18px color-mix(in srgb, CanvasText 22%, transparent); transform: translateX(-50%) translateY(-100%); opacity: 0; transition: transform .22s ease-out, opacity .22s ease-out; }',
        '.dma-floor-popup[data-shown="1"] { transform: translateX(-50%) translateY(0); opacity: 1; }',
        '@media (prefers-reduced-motion: reduce) { .dma-floor-popup { transition: none; } }',
        // ---- ★★ 2026-09-25：「RP 记忆」档美化（用户拍板**方案 B：左右双栏 + 三段分组**，治「界面太散漫」）----
        // 为什么：档内原先 ②→⑨ 一长列堆叠、层级不清 ⇒ 档内改三段分组卡片；宽屏「当前状态 + 楼层快照」在左、
        //   「文件与阅读」在右，窄屏靠 .dma-rp-cols 的 flex-wrap（两栏 flex-basis 放不下时自动叠回单列）。
        // ★ 这里只加**类级**外观；全部锚（data-dma / data-floor* / id）原样留在元素上。
        // ★ 颜色照旧只用系统色关键字 + color-mix（亮/暗主题自动跟），⛔ 不写死十六进制色。
        '.dma-rp-cols { display: flex; flex-wrap: wrap; gap: 10px; align-items: flex-start; }',
        '.dma-rp-colL { flex: 1 1 300px; min-width: 0; display: flex; flex-direction: column; gap: 10px; }',
        '.dma-rp-colR { flex: 2 1 520px; min-width: 0; display: flex; flex-direction: column; gap: 10px; }',
        '.dma-rp-sec { border: 1px solid color-mix(in srgb, CanvasText 14%, transparent); border-radius: 8px; padding: 10px 12px; background: color-mix(in srgb, CanvasText 2%, Canvas); display: flex; flex-direction: column; gap: 8px; min-width: 0; }',
        '.dma-rp-sechead { display: flex; gap: 8px; align-items: baseline; flex-wrap: wrap; }',
        '.dma-rp-sectitle { font-size: 13px; font-weight: 700; color: GrayText; }',
        '.dma-rp-secsub { font-size: ' + SMALL_FONT_SIZE + 'px; color: GrayText; opacity: .85; }',
        '.dma-rp-filehd { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; min-width: 0; }',
        '.dma-rp-fll1 { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; min-width: 0; }',
        // 空态/错态/回执统一成一种「提示条」（左色条 + 极浅底）：颜色由 data-kind 分四色；
        //   定位/语义仍由元素上原有的 data-dma 锚承担（两种属性各管各的）。
        '.dma-rp-note { font-size: 12px; line-height: 1.6; border-left: 3px solid; border-radius: 6px; padding: 5px 9px; display: flex; flex-direction: column; gap: 3px; min-width: 0; }',
        '.dma-rp-note[data-kind="dim"] { color: GrayText; border-left-color: color-mix(in srgb, CanvasText 30%, transparent); background: color-mix(in srgb, CanvasText 3%, Canvas); }',
        '.dma-rp-note[data-kind="ok"] { color: green; border-left-color: color-mix(in srgb, green 55%, transparent); background: color-mix(in srgb, green 6%, Canvas); }',
        '.dma-rp-note[data-kind="warn"] { color: color-mix(in srgb, DarkGoldenrod 85%, CanvasText); border-left-color: color-mix(in srgb, DarkGoldenrod 60%, transparent); background: color-mix(in srgb, DarkGoldenrod 10%, Canvas); }',
        '.dma-rp-note[data-kind="err"] { color: crimson; border-left-color: color-mix(in srgb, crimson 55%, transparent); background: color-mix(in srgb, crimson 7%, Canvas); }',
      ].join('\n')
      ...
      // ---------- ★★ 2026-09-24：回档提示那**三颗动作** —— 唯一的实现 + 唯一的一份画法 ----------
      // 用户口径（逐字）：「还需要一个 dsh 的弹窗 提示，最好是从**上方弹出**的那种」；
      // 追问"什么时候弹"时：「**不能在回档操作之后立刻弹吗，检测 tarven**」。
      //
      // 这一块里的两样东西都是**给两处共用**的（⛔ 不许各写一份实现）：
      //   · `floorActionRun`      —— 三颗动作的**唯一实现**（忙态门 → 置忙 → 发那一脚 → 回执翻人话 → 重取清单）；
      //   · `FloorPendingActions` —— 三颗按钮的**唯一一份画法**（文案 / id / 禁用判据 / 有没有「退回」那颗）；
      //   两处 = ① 面板档顶那条**横幅**（`RpMemoryFlow` 里）；② ★ 常驻挂载的侧边栏组件里那条**顶部浮层**。
      // ⚠️ 三颗动作的差别只有"发哪一脚"（端点 + 请求体）与"回执落在哪个 state" —— 端点字符串因此只出现在
      //   各自的**调用点**上（⚠️ 面板那三处是**静态判据**钉过的：端点与请求体必须逐字写在那三个具名回调体里，
      //   见 `_selftest-client.mjs` 的「楼层快照」那一条）—— ⛔ 别把端点挪进共用实现里。

      /** ★ 2026-09-25（用户口径「**不要楼号了，基本没用，只标序号**」）：**序号**的人话
       *  （＝宿主现算的 `ordinal`：这一周目的楼层行按**记录时间**排第几版，`1..N`，`N` = 最新）。
       *  认不出（那一行不在清单里 / 那行没时间）就如实说「序号未知」，⛔ 不编一个 0 出来。 */
      function floorSeatText(n) {
        return Number.isFinite(n) ? '序号 ' + String(n) : '序号未知'
      }

      /** ★ 2026-09-25：从宿主那三个**序号**（`pendingOrdinals`，`{from,to,archive}`）里取一个 ——
       *  认不出（没有那一份 / 那个键不是数）就如实 `null`（显示成「序号未知」，⛔ 不编 0）。 */
      function floorOrdOf(set, key) {
        if (set === null || set === undefined || typeof set !== 'object') return null
        const v = set[key]
        return Number.isFinite(v) ? v : null
      }

      /**
       * ★★ 那三颗动作的**唯一实现**（档顶横幅 / 常驻状态行 / 顶部浮层 —— 三处都调它）。
       *
       * 共同的那一段只有一件事：**忙态门 → 置忙 → 发那一脚 → 回执翻人话 → 成功才跑收尾那一脚 → 重取一次清单**。
       * 各调用点提供几样：
       *   · `isBusy`  —— 忙态门（连点不许重复发）；
       *   · `busy`    —— 这一处的"忙"长什么样（三处各有各的 state）；
       *   · `request` —— **那一脚**（`() => Promise`；端点与请求体写在调用点上）；
       *   · `setMsg`  —— 这一处的回执落在哪个 state（⛔ 不许"回执落在看不见的地方"）；
       *   · `ok` / `err` / `reload` —— 成功、失败各自的形状 + 收尾重取清单那一脚；
       *   · `done`    —— ★ **只有成功之后**才跑的那一脚（★ 2026-09-25：浮层用它把自己收掉）。
       *     ⛔ 失败**绝不**跑它：请求没成 ⇒ 浮层该还在原地、错误如实画在它里面（fail-loud，见 `FloorPopup` 的 `dismiss`）。
       *     ⚠️ 它在 `try` **之外**跑：收尾那一脚自己炸了不该被画成"请求失败"。
       * ⚠️ **成败都如实落回执**（⛔ 不许静默失败、⛔ 不许"部分成功当全成功"）—— 三处的口径因此完全一致。
       * ⚠️ 这一脚是**异步**的（`void` 出去的）：面板那几处本来就这么写，⛔ 别改成 `await`（组件体不是 async）。
       */
      function floorActionRun(o) {
        if (typeof o.isBusy === 'function' && o.isBusy() === true) return
        o.setMsg(o.busy())
        ;(async () => {
          let succeeded = false
          try {
            ...
            // 宿主那一侧的错误负载里有机器码与明细（有哪几份没写进去 / 被死区挡住），⛔ 别只画一句话。
            const p = error && error.payload && typeof error.payload === 'object' ? error.payload : null
            const message = p && p.error && typeof p.error.message === 'string' && p.error.message !== ''
              ? p.error.message
              : errText(error)
            o.setMsg(o.err(message, p))
          } finally {
            if (typeof o.reload === 'function') o.reload()
          }
          // ★ 成功之后才收（`done`：浮层收掉自己，见 §1 那个「✕ 收不掉」的 bug）—— 失败 ⇒ **不收**。
          if (succeeded && typeof o.done === 'function') {
            try { o.done() } catch (error) { /* ⛔ 收尾那一脚收不掉 ≠ 请求失败：回执照旧是"成了" */ }
          }
        })()
      }

      /** 「这次跳过了什么 / 按块合并了什么」的人话（★ 2026-09-23 改口径：`deadzone` 不再是"整份跳过"，
       *  而是"**按块合并**" —— 死区那几段保留盘上现况，其余照快照回档）。
       *  ★★ 2026-09-26：这张表与下面 `floorRestoreDetail` **两处共用**（楼层清单每行的「回到这一楼」
       *  与弹窗那颗「对齐楼层」（先真回档）—— 回执里那几句**逐字同一份**，⛔ 不各写一套）。 */
      const FLOOR_SKIP_TEXT = {
        deadzone: '死区（作者预置）：按块合并 · 死区那几段保留盘上现况、其余照快照回档',
        'absent-then': '那一楼本来就没有这份',
        'no-content': '那一楼的正文没存下来（读不到）',
        unchanged: '与那一楼逐字节一样',
        'not-target': '不在那 5 份里',
      }
      const floorSkipText = (list) => (Array.isArray(list) ? list : [])
        .map((s) => String(s.name) + '（' + (FLOOR_SKIP_TEXT[s.reason] || String(s.reason)) + '）').join('、')

      /**
       * ★★ 2026-09-26（「弹窗那颗「对齐楼层」改成先真回档」那一单）：**写回那一脚的回执明细** ——
       * 「回到这一楼」（楼层清单每一行）与弹窗那颗「对齐楼层」**同一份画法**（⛔ 不各写一套：
       * 那几句要逐字一样 —— `（原样备份成 …bak-时间戳）`、`按块合并`、`那一楼本来就没有这份`）。
       *
       * ⚠️ 只读数：`detail` = 宿主那一脚回执里的那一份（`{restored, skipped, failed, blocked}`）；
       *   认不出（`null` / 不是对象 / 缺那三个数组）⇒ 如实回空数组（⛔ 不编一句"都成功了"）。
       * ⚠️ 返回的是**子元素数组**（调用方 `...floorRestoreDetail(x)` 摊开）；三块次序固定：
       *   **失败 → 写回 → 跳过**（"没写进去"那一句最要紧，排最前）。
       */
      const floorRestoreDetail = (detail) => {
        if (detail === null || detail === undefined || typeof detail !== 'object') return []
        const out = []
        const failed = Array.isArray(detail.failed) ? detail.failed : []
        const restored = Array.isArray(detail.restored) ? detail.restored : []
        const skipped = Array.isArray(detail.skipped) ? detail.skipped : []
        if (failed.length > 0) {
          out.push(e('div', { style: { marginTop: 3 } }, '⚠ 没写进去：'
            + failed.map((x) => String(x.name) + '（' + String(x.reason) + '）').join('、')
            + ' —— 其余照旧，不算全成功'))
        }
        if (restored.length > 0) {
          out.push(e('div', { style: { marginTop: 3 } }, '已写回：'
            + restored.map((x) => String(x.name) + (x.backup ? '（原样备份成 ' + String(x.backup) + '）' : '（这一份是新建的）')).join('、')))
        }
        if (skipped.length > 0) {
          out.push(e('div', { style: { marginTop: 3 } }, '跳过：' + floorSkipText(skipped)))
        }
        return out
      }

      /**
       * ★★ 那三颗按钮的**唯一一份画法**（档顶那条横幅与顶部那条浮层 —— 两处调它）。
       *
       * ★★ 2026-09-25 改口径（用户原话）：「另外，改一下选项，**1、对齐楼层 2、只对齐楼号 3、不处理**」
       *   ⇒ 这一排**就这三颗** ＋（给了 `onDismiss` 才画的）✕：
       *     · 「**对齐楼层**」（`onSettle`）＝就地登记：把"现在这份档案"记成剧情现在站的这一楼 ＋ 锚 `last` ＋ 清提示；
       *       ★★ 2026-09-26 改口径（用户原话「**不是不触发弹窗，是弹窗上点同步后，没有触发真回档**」＋
       *       用户选定的那一档「让「对齐楼层」**顺手真回档**」）：**弹窗（`FloorPopup`）那一处**的这颗
       *       改成**先真回档、再记账** —— 只换**那一处的动作**（那三个具名回调），⛔ 横幅与常驻状态行
       *       那两处照旧是"就地登记"（本单只动弹窗这一颗）；两处的悬停说明因此**各说各的**
       *       （弹窗那处用 `settleTitle` 覆盖默认那一句）。
       *     · 「**只对齐楼号**」（`onNumber`）＝★ 新：**只**把 `last` 锚过去（⛔ 不记快照、⛔ 不写 blob、⛔ 不新增楼层行）；
       *     · 「**不处理**」（`onAck`）＝清掉那条提示 ＋ 认下当前这一楼（⛔ 什么都不写）。
       *   三颗都 ⛔ **一个字节的正文都不写**（那是另一条路：`gotoFloor`）。
       *   ⛔ 「把档案退回第 N 楼」**不在这排里**了（用户口径：弹窗这一排只要这三件）—— 那个动作仍然在
       *   **楼层清单每一行的「回到这一楼」**里（`gotoFloor` ＋ `POST …/floors/restore`，本单一个字都没动）。
       *
       * 为什么必须共用：两颗「对齐楼层」、两颗「不处理」的**措辞与禁用判据**要是各写一份，
       * 迟早会漂（一处说"只对齐楼号"、一处说"对齐楼层"）。⚠️ id 用 `idPrefix` 拼 ——
       * 横幅与浮层**各有一套 id**（两处同时挂在屏上时，DOM 里不许出现重复 id）。
       *
       * @param {object} props
       *   · `pending`   —— 宿主那条**待处理**（⛔ 逐字照账：楼号、why、source 全是宿主给的，这里不自己算）；
       *   · `ordinals`  —— ★ 2026-09-25：旁边那份 `{from,to,archive}`（**序号** —— 按钮文案里那个「序号 N」
       *     读的就是它；⛔ 面板不自己排、不自己编，认不出就照实写「序号未知」）；
       *   · `idPrefix`  —— 这一处那三颗的 id 前缀；
       *   · `busy`      —— 忙态（三颗一起禁用，⛔ 免得连点两下）；
       *   · `onSettle` / `onNumber` / `onAck` —— 三颗无参动作；
       *   · `settleTitle` —— ★ 2026-09-26：**「对齐楼层」那颗的悬停说明**（给了就用它）。
       *     ⚠️ 为什么要有它：那一颗在**两处的语义不一样**（弹窗＝先真回档、再记账／横幅与常驻状态行
       *     ＝就地登记）⇒ 说明不能共用一句（各说各的才不会读混）；⛔ 不给 ⇒ 用默认那句"就地登记"。
       *     ⛔ **按钮上的字不变**（两处都是「对齐楼层」—— 用户没让改字）。
       *   · `onDismiss` —— 给了才画那颗 **✕**（走的是与「不处理」**同一个动作**）。
       */
      function FloorPendingActions(props) {
        const pending = props && props.pending ? props.pending : null
        if (pending === null) return null
        const busy = props.busy === true
        const idPrefix = typeof props.idPrefix === 'string' && props.idPrefix !== '' ? props.idPrefix : 'dma-floor-pending'
        const toOrdinal = floorOrdOf(props.ordinals, 'to')
        return e('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' } },
          e('button', {
            id: idPrefix + '-settle', className: 'dma-btn', style: btnStyle,
            disabled: busy,
            // ★ 2026-09-25：标题里的那个位置也换**序号**口径（`ordinals.to` = 宿主算好的"这是第几版"）。
            // ★★ 2026-09-26：语义按**调用点**分（弹窗＝先真回档、再记账／横幅与常驻状态行＝就地登记）
            //   ⇒ 说明由调用点用 `settleTitle` 覆盖；⛔ 不给就是下面这一句老口径（与从前**逐字一样**）。
            title: typeof props.settleTitle === 'string' && props.settleTitle !== ''
              ? props.settleTitle
              : '对齐楼层（就地登记）：认下现在这个起点 —— 把「现在这份档案」记成 ' + floorSeatText(toOrdinal)
                + ' 的样子，往后从这一楼接着记。⛔ 那 5 份的正文一个字节都不写（只动我们自己的清单和快照），⛔ 不影响会话。',
            onClick: () => { if (typeof props.onSettle === 'function') props.onSettle() },
          }, '对齐楼层'),
          // ★ 2026-09-25 新增那一颗：只把指针挪过来 —— ⛔ 不记快照（那一楼先不长新行，内容等下一轮自然记上）。
          e('button', {
            id: idPrefix + '-number', className: 'dma-btn', style: btnStyle,
            disabled: busy,
            title: '只对齐楼号：只把档案的指针锚到' + floorSeatText(toOrdinal)
              + '，⛔ 不记快照、⛔ 不新增楼层行 —— 这一楼先不长快照，下一轮轮到它时自然记上。'
              + '⛔ 那 5 份的正文一个字节都不写，⛔ 不影响会话。',
            onClick: () => { if (typeof props.onNumber === 'function') props.onNumber() },
          }, '只对齐楼号'),
          e('button', {
            id: idPrefix + '-ack', className: 'dma-btn', style: miniBtnStyle,
            disabled: busy,
            title: '不处理：把这条提示收掉，认下剧情现在站的这一楼（往后从这儿接着看）。⛔ 不登记、⛔ 那 5 份一个字节都不写。',
            onClick: () => { if (typeof props.onAck === 'function') props.onAck() },
          }, '不处理'),
          // ✕ 与「不处理」是**同一个动作**（用户口径：带 ✕、挂着直到处理）—— 只是这一颗更省事。
          typeof props.onDismiss === 'function' ? e('button', {
            id: idPrefix + '-x', className: 'dma-btn', style: miniBtnStyle,
            disabled: busy, 'aria-label': '收起这条提示（不处理）',
            title: '收起这条提示（与「不处理」同一件事：认下剧情现在站的这一楼，⛔ 不登记、⛔ 那 5 份一个字节都不写）。',
            onClick: () => { if (typeof props.onDismiss === 'function') props.onDismiss() },
          }, '✕') : null)
      }

      /**
       * ★★ 2026-09-24：**顶部浮层**（`data-dma="floor-popup"`）—— 你在 Tavern 一回档，它就**立刻**从上方滑下来。
       *
       * 用户口径（逐字）：「还需要一个 dsh 的弹窗 提示，最好是从**上方弹出**的那种」＋
       * 「**不能在回档操作之后立刻弹吗，检测 tarven**」；对"怎么收"没有明确选 ⇒ 本单按
       * **挂着直到处理**（带 ✕）做，⛔ 不要"几秒自动消失"。
       *
       * 挂在**常驻挂载**的那个侧边栏组件里（`MemoryArchiveButton` —— 那颗按钮一直在，与只在打开时才挂的
       * `ArchivePanel` 不同）⇒ 面板关着也能弹 —— 这正是这一条的意义（回档时你多半没开着面板）。
       *
       * 五件事（⛔ 一件都不多做）：
       *   · 内容 = `⚠ 剧情刚退回去了：第 X 楼 → 第 Y 楼（档案停在第 Z 楼）` ＋ 三颗按钮 ＋ ✕；
       *   · 三颗按钮与档顶那条横幅**同一份**（`FloorPendingActions` ＋ `floorActionRun`，⛔ 不各写一份实现）；
       *   · **挂着直到处理**：点三颗之一或 ✕ 才收（⛔ 没有定时器、⛔ 不自动消失）；
       *   · ⛔ 没有那条待处理 ⇒ **什么都不渲染**（不占位置、不渲染空壳）；
       *   · ★★ 2026-09-25：**那句说明整段不渲染**（用户点名不要：「「把档案退回」动那 5 份笔记的正文…
       *     ⛔ 都不影响会话」）—— 每颗按钮自己的 `title`（悬停说明）保留（那要鼠标悬停才出现，不占版面）。
       *
       * ★★ 2026-09-26 改口径（用户原话「**不是不触发弹窗，是弹窗上点同步后，没有触发真回档**」＋
       *   用户选定的那一档「让「对齐楼层」**顺手真回档**」）：**这一处**那颗「对齐楼层」从"就地登记"
       *   改成 **先真回档、再记账**（「只对齐楼号」/「不处理」/✕ 与横幅那两处**一个字都没动**）——
       *   具体走哪一脚见 `settle`（含"没有可退回的那一份"与"被死区挡住"两条边界）。
       *   ⚠️ 因此这一处的「对齐楼层」与横幅那一颗**语义不同**，两处的悬停说明也各说各的（`settleTitle`）。
       *
       * 收的口径：**成了才收**（`done` 那一脚）—— 没做成（宿主 4xx/5xx、断网）⇒ 浮层留在屏上、红字说清原因，
       * ⛔ 绝不"提示没了但事情没办"。
       * ★★ 2026-09-25 修那个 bug（用户实测「档案回退弹窗 ✕ 不掉」）：✕ / 「不处理」那一脚原来是
       *   **只清宿主那条 `pending`**（走 `…/pending/ack`）—— 宿主清了、**父组件里那个 `floorPopup` state
       *   从来没被碰过**（浮层是照**最后一帧 SSE** 渲染的）⇒ 浮层原地不动，要等下一帧 SSE（hello / fork）
       *   才被替掉。现在：**请求成功之后**才跑 `done`（`dismiss`）⇒ 当场收掉本地那条；**失败不收**
       *   （浮层该还在，错误如实画在它里面）。
       * ⚠️ 已知的一处（如实记着）：浮层那一脚**不会**去刷面板那一档的清单 ⇒ 面板正开着时它上面那条横幅
       *   可能短暂停在旧状态（下一次面板动作 / 重新打开面板 / 下一条 SSE 帧就自愈）。
       */
      function FloorPopup(props) {
        const pending = props && props.pending && typeof props.pending === 'object' ? props.pending : null
        // ★ 2026-09-25：标题里那三个位置显示的是**序号**（宿主给的 `pendingOrdinals`；⛔ 面板不自己排）。
        const ordinals = props && props.ordinals !== undefined ? props.ordinals : null
        // ⚠️ 三个 hook **必须**在下面那个"没有就不渲染"的早退**之前**（hook 次数不能随 props 变）。
        const [shown, setShown] = React.useState(false)                        // #0 进/出都翻它（CSS 里两条 transition）
        const [msg, setMsg] = React.useState({ status: 'idle', message: '' })   // #1 三颗动作的忙态/回执
        const elRef = React.useRef(null)                                       // #2 量过渡时长用
        React.useEffect(() => { setShown(true) }, [])                           // 挂上 ⇒ 从屏幕外滑下来
        if (pending === null) return null
        const onClose = typeof props.onClose === 'function' ? props.onClose : null
        const busy = msg.status === 'busy'
        const closeNow = () => { if (onClose !== null) onClose() }
        /** 收（两段式）：先把 `data-shown` 翻回 0（CSS 从上方滑回去），过渡走完再让父组件卸掉它。
         *  ⚠️ 过渡跑不起来（省电模式 / 过渡被判 0s / 浏览器把过渡合并掉了）⇒ **当场收** ——
         *     ⛔ 不拿定时器兜底：用户口径是"挂着直到处理"，挂住是**要**的、收不掉是**不许**的。
         *  ⚠️ **只在请求成功之后**被 `floorActionRun` 的 `done` 调到（失败 ⇒ 浮层留在原地，见上）。 */
        const dismiss = () => {
          let noTransition = true
          try {
            const el = elRef.current
            const cs = el && typeof getComputedStyle === 'function' ? getComputedStyle(el) : null
            const dur = cs && typeof cs.transitionDuration === 'string' ? cs.transitionDuration : ''
            noTransition = dur === '' || dur.split(',').every((x) => x.trim() === '' || x.trim() === '0s')
          } catch (error) { noTransition = true }
          setShown(false)
          if (noTransition) closeNow()
        }
        /** 过渡走完 ⇒ 才真卸掉（⚠️ 只认**这一层**自己的那一次：子元素冒泡上来的不算）。 */
        const onTransitionEnd = (ev) => {
          if (ev && ev.target !== elRef.current) return
          if (shown === false) closeNow()
        }
        const pendingKey = typeof pending.targetKey === 'string' ? pending.targetKey : ''
        /** ★★ 2026-09-26：「对齐楼层」那颗的**悬停说明**（只覆盖**这一处** —— 见 `FloorPendingActions`
         *  的 `settleTitle`）：弹窗这一处的语义变了（**先真回档、再记账**），横幅与常驻状态行那两处
         *  仍是"就地登记" ⇒ 两处各说各的（⛔ 不许拿这一句去说那两处）。
         *  ⚠️ 那一楼**没有可退回的那一份**时这一脚走的还是"就地登记"（见 `settle` 的边界①）⇒ 说明
         *  **跟着那一相走**（⛔ 不许拿"会写回正文"去说一颗不会写正文的按钮）。
         *  ⚠️ 按钮上的**字**没变（两处都是「对齐楼层」—— 用户没让改字）。 */
        const settleTitle = typeof pending.targetNodeId === 'string' && pending.targetNodeId !== ''
          ? '对齐楼层（先真回档、再记账）：先把那 5 份写回那一楼的样子，再把这个位置记成账'
            + ' —— 写前先把现在这份备份成 .bak-时间戳；有死区的那几份按块合并（死区那几段保留盘上现况）。'
            + '⛔ 不影响会话；有哪一份写不进去 / 被死区挡住 ⇒ 如实红字、这条提示不消。'
          : '对齐楼层（就地登记）：这一楼没有可退回的那一份 ⇒ 只把「现在这份档案」记成剧情现在站的这一楼的样子'
            + ' ＋ 把这个位置记成账（⛔ 那 5 份的正文一个字节都不写），⛔ 不影响会话。'
        /** 三颗动作与横幅**同一份实现**（`floorActionRun`）—— 这里只给"发哪一脚 + 回执落在哪 + 成了才收"。
         *  ★ 2026-09-25：`done: dismiss` 是**成功之后**才跑的那一脚（`floorActionRun` 里那句 `succeeded`）——
         *    这就是"✕ 收得掉"那道修法；⛔ 别改成"点下去就先收"（失败时用户会以为办成了）。 */
        const runAction = (o) => floorActionRun(Object.assign({
          isBusy: () => busy,
          busy: () => ({ status: 'busy', message: '' }),
          setMsg: setMsg,
          done: dismiss,
        }, o))
        /**
         * ★★ **弹窗那颗「对齐楼层」＝ 先真回档、再记账**（★★ 2026-09-26 改口径，一脚做完）。
         *
         * 用户口径（逐字）：「**不是不触发弹窗，是弹窗上点同步后，没有触发真回档**」；追问后用户**选定**
         * 的那一档（原话在选项里）：「**让「对齐楼层」顺手真回档**」—— 预览写着：`[对齐楼层]` 点它＝
         * **先把 5 份写回那一楼的样子，再把这个位置记成账**。
         * ⚠️ 记一笔历史（⛔ 别再按老口径实现）：2026-09-25 那次是用户**点名**把「把档案退回第 N 楼」从弹窗上
         *   拿掉的、并把「对齐楼层」定义成"**就地登记**（只记账、⛔ 不动正文）"；**本单就是来改这个语义的**。
         *
         * ① **目标** = 那条 pending 的 `target*`（`{nodeId, variantId}` —— 与当初「把档案退回第 N 楼」那颗
         *    **同一个目标**；`targetNodeId === ''` ⇒ 那一楼没有可退回的那一份，见下面的边界那一段）。
         * ② **走现成的那一个端点**：`POST …/floors/restore` —— **它自己就会**把 `last` 锚到那一份并把
         *    pending **清掉**（`runRestore` 第③步 + `clearedPending`）⇒ ⛔ **不要再调 settle**（两脚会打架）。
         *    ⚠️ 端点字符串与请求体**逐字写在下面那个 `request` 里**（仓里"静态判据钉端点"的惯例，照旧）。
         * ③ **回执两件事都在**：① 宿主那一脚的执行（写回/跳过/失败/被挡 —— 含 `.bak-时间戳`、`按块合并`、
         *    `absent` 那几句，走共用的 `floorRestoreDetail`，⛔ 不另写一套）；② 这一脚**同时把这个位置记成了账**
         *    （一句话带上 —— 锚 `last` ＋ 清 pending 都是 restore 那一脚自己做的）。
         * ④ **边界**（⛔ 一律不许静默、⛔ 不许写成成功）：
         *    · `targetNodeId === ''` ⇒ **不能假装回了档**：如实说"这一楼没有可退回的那一份 ⇒ 只把位置记成账、
         *      正文没动"，并按**就地登记**收尾（＝从前那颗的行为，`settle` 那一条）；
         *    · 宿主回 `no-snapshot` / `legacy-no-restore`（404/409）⇒ 走 `err`：如实红字、⛔ 不出"已对齐"这种话；
         *    · **被死区挡**（`blocked` —— 宿主回 **200** 但一个字节都没恢复、那条提示也**没清**）⇒ 见 `request`
         *      里那一抛：那一脚**不算成功** ⇒ 红字 + ⛔ **不收起浮层**（`done` 只在成功之后跑）。
         */
        const settle = () => {
          const targetNodeId = typeof pending.targetNodeId === 'string' ? pending.targetNodeId : ''
          const targetVariantId = typeof pending.targetVariantId === 'string' ? pending.targetVariantId : ''
          // ── 边界①：那一楼**没有可退回的那一份**（退回那条路走不了）⇒ 不许假装回了档 ——
          //   按**就地登记**收尾（＝从前那颗的行为），回执如实说"只记账、正文没动"。
          if (targetNodeId === '') {
            runAction({
              request: () => apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/pending/settle', undefined,
                Object.assign({ mode: 'full' }, pendingKey === '' ? {} : { targetKey: pendingKey })),
              ok: (data) => ({
                status: 'ok',
                message: (typeof data.message === 'string' && data.message !== '' ? data.message : '已把档案对齐到当前这一楼')
                  + '；这一楼没有可退回的那一份 ⇒ 只把位置记成账、正文没动',
                detail: data,
              }),
          ...
          // ── 主路：**先真回档**（把那 5 份写回那一楼那一支的样子），restore 那一脚顺手把位置记成账。
          runAction({
            request: async () => {
              const data = await apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/restore', undefined,
                { nodeId: targetNodeId, variantId: targetVariantId })
              // ★ 被死区挡住（`blocked`）⇒ 宿主那一脚**整次不做**：一个字节都没恢复、`last` 留在原地、
              //   那条 pending **也没清** ⇒ **这一脚不算成功** —— 抛出去走 `err` 那一条（红字如实说、
              //   ⛔ 不收起浮层；`done` 只认 `succeeded`）。⚠️ 判据用宿主回执里那个字段，⛔ 不自己猜。
              if (data && data.blocked !== null && data.blocked !== undefined && data.blocked !== '') {
                const blocked = new Error(typeof data.message === 'string' && data.message !== ''
                  ? data.message
                  : '这一脚被挡住了（死区数据读不出来）⇒ 笔记保持原样、那条提示也还在')
                blocked.payload = { restore: data }
                throw blocked
              }
              return data
            },
            ok: (data) => ({
              status: 'ok',
              // ① 宿主那一脚的执行（写回/跳过/失败/被挡那几句 —— ⛔ 不改写、⛔ 不另编一句）
              // ② 再加上"这一脚同时把位置记成了账"（★ 2026-09-26 新语义那一半；`pendingCleared` 是宿主
              //    回执里那个字段 —— 清掉了才说"那条回档提示也清了"，⛔ 不替它担保）。
              message: (typeof data.message === 'string' && data.message !== '' ? data.message : '已按那一楼写回笔记')
                + '；这一脚同时把这个位置记成了账（档案指针已锚到那一楼'
                + (data.pendingCleared === true ? '，那条回档提示也清了' : '') + '）',
              detail: data,
            }),
            err: (message, p) => ({
              status: 'err', message: message,
              // 有哪一份写不进去 / 被挡住 ⇒ 宿主把明细放在回执里（⛔ 如实画出来，⛔ 绝不"部分成功当全成功"）
              detail: p && p.restore && typeof p.restore === 'object' ? p.restore : null,
            }),
          })
        }
        const number = () => runAction({
          // ★ 2026-09-25 新增那颗「只对齐楼号」：**同一个端点**、体里带 `mode:'number'`
          //   （⛔ 不记快照、⛔ 不写 blob、⛔ 不新增楼层行 —— 判据在宿主那边）。
          request: () => apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/pending/settle', undefined,
            Object.assign({ mode: 'number' }, pendingKey === '' ? {} : { targetKey: pendingKey })),
          ok: (data) => ({ status: 'ok', message: typeof data.message === 'string' && data.message !== '' ? data.message : '已只对齐楼号（这一楼先不长快照，下一轮会自然记上）' }),
          err: (message) => ({ status: 'err', message: message }),
        })
        const ack = () => runAction({
          request: () => apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/pending/ack', undefined, {}),
          ok: (data) => ({ status: 'ok', message: typeof data.message === 'string' && data.message !== '' ? data.message : '已收掉那条提示（不处理）' }),
          err: (message) => ({ status: 'err', message: message }),
        })
        const variantNote = pending.why === 'variant' ? '（同一楼换了支：swipe 重 roll）' : ''
        return e('div', {
          ref: (el) => { elRef.current = el },
          'data-dma': 'floor-popup',
          'data-shown': shown ? '1' : '0',
          role: 'alert',
          onTransitionEnd: onTransitionEnd,
        },
          // ★ 标题照用户给的形状，★ 2026-09-25 起位置换成**序号**口径：
          //   `⚠ 剧情刚退回去了：序号 3 → 序号 1（档案停在 序号 3）`（三个都是宿主算好的 `ordinal`）。
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', fontSize: 14, fontWeight: 700 } },
            e('span', { 'data-dma': 'floor-popup-icon', style: { fontSize: 16 } }, '⚠'),
            '剧情刚退回去了：' + floorSeatText(floorOrdOf(ordinals, 'from')) + ' → ' + floorSeatText(floorOrdOf(ordinals, 'to'))
            + '（档案停在' + floorSeatText(floorOrdOf(ordinals, 'archive')) + '）' + variantNote),
          // ⛔ 这里原来那句说明（「把档案退回」动那 5 份笔记的正文…⛔ 都不影响会话）**整段不渲染** ——
          //   用户 2026-09-25 点名不要，也 ⛔ 不换成别的说明（每颗按钮自己的 `title` 还在）。
          e(FloorPendingActions, {
            pending: pending,
            ordinals: ordinals,
            idPrefix: 'dma-floor-popup',
            busy: busy,
            onSettle: settle,
            onNumber: number,
            onAck: ack,
            // ★★ 2026-09-26：**这一处**那颗「对齐楼层」的说明（先真回档、再记账 —— 见 `settleTitle`）；
            //   ⛔ 横幅/常驻状态行那两处不传它（那两处仍是"就地登记"，用默认那一句）。
            settleTitle: settleTitle,
            onDismiss: ack,     // ✕ 与「不处理」走**同一脚**（用户口径）
          }),
          // ★★ 2026-09-26：回执里**两件事都在** —— ① 宿主那一脚的执行（写回/跳过/失败那几句，与
          //   「回到这一楼」**同一份画法**：`.bak-时间戳`、`按块合并`、`那一楼本来就没有这份`）；
          //   ② 那句话里带的"这一脚同时把这个位置记成了账"。
          //   ⚠️ 被死区挡住时宿主回 200 但这一脚**什么都没做** ⇒ `settle` 里那一抛把它转成 `err`：
          //   红字（`floor-popup-err`）、⛔ 不收起浮层。
          msg.status === 'idle' ? null : e('div', {
            'data-dma': msg.status === 'err' ? 'floor-popup-err' : (msg.status === 'ok' ? 'floor-popup-ok' : 'floor-popup-busy'),
            role: msg.status === 'err' ? 'alert' : undefined,
            style: msg.status === 'err' ? errorStyle : (msg.status === 'ok' ? okStyle : dimStyle),
          }, msg.status === 'busy' ? '正在处理…' : msg.message, ...floorRestoreDetail(msg.detail)),
        )
      }

      // ---------- 阅读源·角色扮演记忆库（同一档的上一块，20260919）：★ 同样带防剧透门 ----------
      // 读什么：社区 RP 预设（oliblue-evan/dsh-roleplay-preset）**自己约定写出来**的
      //   <工作目录>/.roleplay-memory/{index,story,characters,world}.md
      ...
      //   这一档从"只能看"变成"能划死区 + 能直接改"，**新增的三件事都排在手动确认之后**：
      //     · **死区**（模型禁改区）：每个**块**（宿主切好的 markdown 块）一个「🔒 设为死区 / 🔓 解锁」，
      //       已死的块灰底 + 锁标。约束怎么喂给模型、被改了怎么告警，全在宿主侧（lib/deadzone.js）——
      //       面板只做两件：把用户的动作发出去、把宿主的比对结果**如实画出来**（红字）。
      //     · **直接编辑**：每个可写的文件（.md/.txt）给一个「✎ 直接改」——改的是**整份正文**
      //       （textarea，初值 = 宿主刚给的那份），保存带**乐观锁**（宿主返回的 sha256）；
      //       盘上要是被人改过 ⇒ 宿主回 409，面板如实说"盘上已经变了，先刷新再改"（⛔ 绝不静默覆盖）。
      //     · ⛔ 全档仍然**零 useEffect**：新动作全是**事件回调**；"死区现状"那一次查询挂在 reveal 里
      //       （与清单同一次"手动确认"之后），保存/划死区之后再各刷一次。
      function RpMemoryFlow({ scrollBind }) {
        ...
        const [pane, setPane] = React.useState({ status: 'idle', text: '', chars: 0, originalChars: 0, truncated: false, sha256: '', blocks: null, writable: false, deadzones: 0, error: '' })
        // ★ 2026-09-23：编辑态（'' = 在读；打开的文件名 = 正在改它）+ 草稿 + 保存回执
        const [editing, setEditing] = React.useState('')
        const [draft, setDraft] = React.useState('')
        const [saving, setSaving] = React.useState({ status: 'idle', message: '', receipt: null })
        // 划死区那一下的忙态（键 = 那块 sha）+ 回执；死区现状（宿主算好的红字料）
        const [zoneBusy, setZoneBusy] = React.useState('')
        const [zoneMsg, setZoneMsg] = React.useState({ status: 'idle', message: '' })
        const [alert, setAlert] = React.useState({ status: 'idle', data: null, error: '' })
        // ★★ 2026-09-23（用户口径「**按楼层绑定的笔记快照**：每轮模型修改都记录，能自动跟随回档」）：
        //   这一块只做两件事 —— 把宿主算好的**楼层清单**如实画出来、把「回到这一楼」发出去。
        //   ⛔ 面板**不许直连 Tavern、不许自己算"当前第几楼"**（一律走宿主端点）；
        //   ⛔ 全档仍然**零挂载取数**：楼层清单只在那几个具名回调里取（与死区现状同一套纪律）。
        // ★★ 2026-09-24 改口径（用户原话）：「**不要做跟随楼层的功能，先做手动挡**，开 fork 时（回档）
        //   **前台弹提示，让用户手动回退剧情档案**。做**更显著的更改表示**」⇒ 本档多两样东西：
        //   档顶一条**显著**横幅（`floorPendingBanner`：色块 + 加粗 + ⚠ 图标，⛔ 不是一行小灰字）＋
        // ★★ 2026-09-25 改口径（用户原话）：「另外，改一下选项，**1、对齐楼层 2、只对齐楼号 3、不处理**」
        //   ⇒ 档顶那条横幅那一排就这三颗 ＋ ⛔ **去掉「把档案退回第 N 楼」那颗**（用户口径：那排只要这三件；
        //   那个动作仍然在**楼层清单每一行的「回到这一楼」**里，走 `gotoFloor` ＋ `POST …/floors/restore`，
        //   ⛔ 那条路一个字都没动）；横幅里那句**说明**也**整段不渲染**（用户点名不要，⛔ 不换成别的说明）。
        //   ★ 收尾（同用户口径「**开 fork 时（回档）前台弹提示**」）：判据换成宿主的 `decideFork`（剧情 vs `seen`），
        //   取数那一脚从 `GET /floors` 改成 `POST /floors/scan`（开档补一次检测），并多一条**常驻状态行**。
        const [floorPane, setFloorPane] = React.useState({ status: 'idle', data: null, error: '' })
        const [gotoMsg, setGotoMsg] = React.useState({ status: 'idle', nodeId: '', message: '', detail: null })
        // ★ 2026-09-24（回档改手动挡）：档顶那条横幅上那两颗动作的忙态/回执。
        //   ⚠️ 新状态一律**追加在最后** —— 台子按 hook 序号注入（⛔ 不许把已有序号挪位）。
        const [pendingMsg, setPendingMsg] = React.useState({ status: 'idle', message: '' })
        // ★★ 2026-09-24 收尾：**档顶那条常驻状态行**上那颗「档案对齐到第 N 楼」的忙态/回执。
        //   与横幅那颗各画各的（⛔ 不互相顶掉，也⛔ 不让回执落在看不见的地方）—— 同样**追加在最后**。
        const [alignMsg, setAlignMsg] = React.useState({ status: 'idle', message: '' })
        // ★★ 2026-09-25（用户口径「**默认显示最近10楼，从序号大到小排序**」）：清单那一颗
        //   「显示其余 N 条」的开关（`false` = 只画最近 10 行 ∪ {当前剧情那一行, 档案指针那一行}）。
        //   ⚠️ 新状态一律**追加在最后** —— 台子按 hook 序号注入（⛔ 不许把已有序号挪位）。
        const [floorAll, setFloorAll] = React.useState(false)
        // ★★ 2026-09-25（用户口径「**开一个下钻，直接显示具体改动的字段，写改了哪些可以说完全没用**」）
        //   ⇒ 每一行那颗「看改动 ▸」的**按需**取数与展开态：`key` = 哪一行展开了（`''` = 都没展开）；
        //   `status` ∈ `'idle' | 'loading' | 'ready' | 'error'`。⛔ 只在那颗按钮的回调里发请求
        //   （⛔ 开档不预取、⛔ 不轮询、⛔ 不挂定时器）。
        const [floorDiff, setFloorDiff] = React.useState({ key: '', status: 'idle', data: null, error: '' })
        const idle = { status: 'idle', text: '', chars: 0, originalChars: 0, truncated: false, sha256: '', blocks: null, writable: false, deadzones: 0, error: '' }

        /**
         * 死区现状（宿主现算：每条死区在现文里重新定位，先 sha、再首行）。
         * ⛔ 这里**一个字的判断都不做** —— 哪条不一致、两个 sha 各是多少，全是宿主给的。
         * 只在**手动确认之后**（reveal 成功/保存成功/划完死区）发，⛔ 不挂定时器、不轮询。
         */
        const loadStatus = () => {
          setAlert({ status: 'loading', data: null, error: '' })
          const controller = new AbortController()
          ;(async () => {
            try {
              const data = await requestJson(HOST_API_BASE + '/playthrough/rp-memory/deadzones/status', controller.signal)
              if (controller.signal.aborted) return
              setAlert({ status: 'ready', data: data, error: '' })
            } catch (error) {
              if (controller.signal.aborted) return
              setAlert({ status: 'error', data: null, error: errText(error) })
            }
          })()
        }

        /**
         * 楼层清单（`POST /playthrough/rp-memory/floors/scan`）—— 宿主现算：每楼序号 / 时间 / 改了哪几份（+几字节）/
         * 当前站在哪一楼 / 最近一次回档 / ★ 那条**待处理**（`pending` —— 档顶横幅的判据）。
         * ⛔ 这里**一个字的判断都不做**（序号、当前那一楼、该不该高亮、该不该弹横幅全是宿主给的）；
         * 只在**手动确认之后**与几个动作之后发，⛔ 不挂定时器、⛔ 不轮询。
         * ★★ 2026-09-24 收尾：从 `GET /floors` 改成 **`POST /floors/scan`** —— 语义是"**开面板也检测一次**"
         *   （补掉"回档之后没发消息、直接开面板看"那个时序洞：检测原先只在轮边界跑）。宿主那一脚**只会**
         *   记/清那条提示 + 跟一下它自己的簿记文件，⛔ **那 5 份正文一个字节都不写**（写盘仍只在 `gotoFloor` 那一脚）。
         *   ⛔ 仍然**只在具名回调里**发：不挂定时器、不轮询（与死区现状同一套纪律）。
         */
        const loadFloors = () => {
          setFloorPane({ status: 'loading', data: null, error: '' })
          // ★ 2026-09-25：重取清单 ⇒ 展开着的**改动下钻**收起来（那一份 diff 是"上一刻那一份清单"的样子，
          //   动作（回到这一楼 / 对齐）之后它可能已经不对了 —— ⛔ 不留一块过期的旧内容在版面上）。
          setFloorDiff({ key: '', status: 'idle', data: null, error: '' })
          const controller = new AbortController()
          ;(async () => {
            try {
              const data = await apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/scan', controller.signal, {})
              if (controller.signal.aborted) return
              setFloorPane({ status: 'ready', data: data, error: '' })
            } catch (error) {
              if (controller.signal.aborted) return
              setFloorPane({ status: 'error', data: null, error: errText(error) })
            }
          })()
        }
        ...
        /**
         * ★★ 2026-09-25（用户口径「**开一个下钻，直接显示具体改动的字段，写改了哪些可以说完全没用**」）：
         * 「看改动 ▸」那一次下钻的**取数**（`GET …/floors/diff`）—— **只读、按需**：
         *   · ⛔ **只在点了那一颗之后**发这一个请求（⛔ 开档不预取、⛔ 不轮询、⛔ 不挂定时器）；
         *   · 传的只有 `(nodeId, variantId)`（清单里那一行的 id）；"跟谁比"缺省 = **前一条**
         *     （宿主按 `ordinal` 口径自己挑 —— ⛔ 面板不挑、不算新旧）；
         *   · 比对内容、有没有前一条、哪几份有死区 —— **全是宿主给的**，这里只把结果如实摊成人话：
         *     加载中 / 读不出来（红字＋原因）/ 空 diff（"与上一序号逐字节一样"）各自照实画，⛔ 不静默。
         * @param {object} f 清单里那一行（`nodeId` / `variantId`）
         * @param {string} key 那一行的键（`floorDiffKey(f)` —— 展开态就认它）
         */
        const loadFloorDiff = (f, key) => {
          setFloorDiff({ key: key, status: 'loading', data: null, error: '' })
          const controller = new AbortController()
          ;(async () => {
            try {
              const url = HOST_API_BASE + '/playthrough/rp-memory/floors/diff?nodeId='
                + encodeURIComponent(String(f.nodeId)) + '&variantId=' + encodeURIComponent(String(f.variantId || ''))
              const data = await requestJson(url, controller.signal)
              if (controller.signal.aborted) return
              setFloorDiff({ key: key, status: 'ready', data: data, error: '' })
            } catch (error) {
              if (controller.signal.aborted) return
              setFloorDiff({ key: key, status: 'error', data: null, error: errText(error) })
            }
          })()
        }

        /**
         * 「回到这一楼」（★ 楼层清单**每一行**那颗 —— ★ 2026-09-25 起回档提示那一排上**没有**它了，
         * 想"把档案退回去"就从这儿走）：**手动**触宿主那**同一个恢复函数**（★ 2026-09-24 起
         * 这是那 5 份**唯一的写入口** —— 回档那一侧只提示、⛔ 不写）——
         * 会把笔记恢复到**那一楼那一支**的样子，**不影响会话**；恢复前宿主会先把"当下"存一份（可撤销）。
         * ⛔ 传的只有 `(nodeId, variantId)`（列表里那一行的 id）—— 路径、判据、写盘全是宿主的事。
         * ★ 2026-09-23：快照按 `(楼层, 变体)` 记 ⇒ 变体一起传（同一楼 swipe 过就是两支两行）。
         * ★ 2026-09-24（浮层那一单）：动作的**实现**搬进共用的 `floorActionRun`（档顶横幅 / 常驻状态行 /
         *   顶部浮层三处同一份）—— 这里只剩"发哪一脚 + 回执落在哪个 state"（⚠️ 端点与请求体**逐字留在这里**：
         *   那两条是静态判据钉过的形状，⛔ 别跟着搬走）。
         */
        const gotoFloor = (nodeId, variantId) => {
          // 忙态门：连点不重复发（共用实现里 `isBusy` 还有一道 —— 这里是**具名回调体里**那一道，
          // 静态判据钉的就是它，⛔ 别挪走）。
          if (gotoMsg.status === 'busy') return
          floorActionRun({
            isBusy: () => gotoMsg.status === 'busy',
            busy: () => ({ status: 'busy', nodeId: nodeId, message: '', detail: null }),
            request: () => apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/restore', undefined, { nodeId: nodeId, variantId: variantId }),
            setMsg: setGotoMsg,
            ok: (data) => ({
              status: 'ok', nodeId: nodeId,
              message: typeof data.message === 'string' && data.message !== '' ? data.message : '已按那一楼恢复',
              detail: data,
            }),
            // 有哪一份写不进去 ⇒ 宿主回 500 + 明细（⛔ 如实画出来，⛔ 绝不"部分成功当全成功"）
            err: (message, p) => ({
              status: 'err', nodeId: nodeId, message: message,
              detail: p && p.restore && typeof p.restore === 'object' ? p.restore : null,
            }),
            reload: loadFloors,   // 恢复之后重取一次清单（当前那一楼、每楼的样子都可能变了）
          })
        }

        /**
         * ★★ 「**对齐楼层**」/「**只对齐楼号**」（都是 `POST /playthrough/rp-memory/floors/pending/settle`）——
         * 「对齐楼层」的语义是 **就地登记**（用户口径：「跟 Tavern 一致 —— 回档之后那段历史就是没有了，
         * 记忆回档之后也直接按新的来」）：宿主把**当下盘上那 5 份**的现文记成"剧情现在站的这一楼"那一份
         * 快照、并把账（`last`）锚到它 ⇒ 之后判据不再认为"还在回档"，记录无缝续上。
         * ★ 2026-09-25 新增「**只对齐楼号**」：**同一个端点**、体里带 `mode:'number'` —— 只把 `last` 锚过去，
         *   ⛔ 不记快照、⛔ 不写 blob、⛔ 不新增楼层行（"楼号先对上，内容等下一轮自然记上"）。
         * 两颗都 ⛔ 那 5 份的**正文一个字节都不写**、⛔ 不影响会话。
         * ★ 2026-09-24 收尾：**两个地方共用这一份** —— 档顶横幅上那两颗（有那条提示时）与**常驻状态行**
         *   上那颗（"剧情 / 档案不在同一楼"时，**可能压根没有待处理那条**）。带**面板看到的那条** `pending`
         *   的 `targetKey`（没有就传 `''`）：跟宿主当下那条不一致（这中间又发生了另一次 fork）⇒ 宿主回 409，
         *   面板如实红字 —— ⛔ 绝不"按老目标就地登记"。
         * @param {string} key 面板看到的那条 `pending.targetKey`（`''` = 面板没看到待处理那条）
         * @param {'banner'|'state'} where 回执画在哪一处（两处**各有自己的回执行**，⛔ 不互相顶掉）
         * @param {'full'|'number'} mode `'full'` = 对齐楼层（缺省）/ `'number'` = 只对齐楼号
         */
        const settlePending = (key, where, mode) => {
          const fromState = where === 'state'
          // ⛔ 只这两颗按钮 → 只这两个值；别的值不猜（宿主那边认不出会 400，这里也绝不自己编一个）
          const wantMode = mode === 'number' ? 'number' : 'full'
          const onlyNumber = wantMode === 'number'
          // 三处的实现同一份（`floorActionRun`）—— 只有"回执落在哪个 state"与"带不带 targetKey"不同。
          floorActionRun({
            isBusy: () => (fromState ? alignMsg.status === 'busy' : pendingMsg.status === 'busy'),
            busy: () => ({ status: 'busy', message: '' }),
            request: () => apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/pending/settle', undefined,
              Object.assign({ mode: wantMode }, key === '' ? {} : { targetKey: key })),
            setMsg: fromState ? setAlignMsg : setPendingMsg,
            ok: (data) => ({
              status: 'ok',
              message: typeof data.message === 'string' && data.message !== ''
                ? data.message
                : (onlyNumber ? '已只对齐楼号（这一楼先不长快照，下一轮会自然记上）' : '已把档案对齐到当前这一楼'),
            }),
            err: (message) => ({ status: 'err', message: message }),
            reload: loadFloors,   // 横幅/状态行的判据都在宿主那边（那条 pending 与 seen）⇒ 清完重取一次清单
          })
        }

        /**
         * ★★ 2026-09-24 收尾：横幅上那颗「**不处理**」（`POST …/floors/pending/ack`；
         * ★ 2026-09-25 从「先不管」改的名，语义一个字没动）——
         * 宿主只做两件事：清掉那条提示 ＋ 把"上次认过的剧情位置"跟到当前这一楼（＝**算认了**，下一轮不再重复弹）。
         * ⛔ **不登记**（⛔ 不动账、⛔ 一个字节都不写那 5 份）—— 这是它与两颗「对齐」的区别。
         * ★ 2026-09-24（浮层那一单）：**顶部那条浮层上的 ✕ 走的就是这一脚**（用户口径：✕ 与「不处理」同一件事）。
         */
        const ackPending = () => floorActionRun({
          isBusy: () => pendingMsg.status === 'busy',
          busy: () => ({ status: 'busy', message: '' }),
          request: () => apiPost(HOST_API_BASE + '/playthrough/rp-memory/floors/pending/ack', undefined, {}),
          setMsg: setPendingMsg,
          ok: (data) => ({ status: 'ok', message: typeof data.message === 'string' && data.message !== '' ? data.message : '已收掉那条提示（不处理）' }),
          err: (message) => ({ status: 'err', message: message }),
          reload: loadFloors,   // 那条提示在宿主那边被清掉了 ⇒ 重取一次清单（横幅该消失）
        })
        ...
        const reveal = () => {
          ...
          ;(async () => {
            try {
              ...
              loadStatus() // ★ 确认之后才问"死区现状"（与清单同一次确认；⛔ 未确认路径零请求）
              loadFloors() // ★ 同一次确认里也问一遍"楼层快照"（⛔ 仍然零轮询）
            } catch (error) {
              if (controller.signal.aborted) return
        ...
        /**
         * 取一份文件的正文（★ 只在**点开那一行**时取）。
         * `force` = 保存之后刷新同一份（那时 openName 已经是它，不能按"再点一次收起"处理）。
         */
        const toggleFile = (name, force) => {
          if (openName === name && force !== true) { setOpenName(null); setEditing(''); return } // 再点一次收起（单开：同看两份没意义）
          setOpenName(name)
          setEditing('')            // 换一份 ⇒ 回到阅读态（⛔ 不把上一份的草稿留在这一份上）
          setSaving({ status: 'idle', message: '', receipt: null })
          setZoneMsg({ status: 'idle', message: '' })
          setPane({ status: 'loading', text: '', chars: 0, originalChars: 0, truncated: false, sha256: '', blocks: null, writable: false, deadzones: 0, error: '' })
          const controller = new AbortController()
          ;(async () => {
            try {
              ...
              setPane({
                ...
                // ★ 2026-09-23：乐观锁的"用户看到的那份"就是这两个字段（sha + 逐块）；
                //   ⛔ 面板不自己算 sha、不自己切块（宿主的切法是唯一一份，两处各切迟早漂）。
                sha256: typeof f.sha256 === 'string' ? f.sha256 : '',
                blocks: Array.isArray(f.blocks) ? f.blocks : null,
                writable: f.writable === true,
                deadzones: Number.isFinite(f.deadzones) ? f.deadzones : 0,
                error: '',
              })
            } catch (error) {
              if (controller.signal.aborted) return
        ...
        /** 进入编辑态：草稿初值 = **宿主刚给的那份正文**（⛔ 不许拿别处的副本当底稿）。 */
        ...
        /**
         * 保存（`POST /playthrough/rp-memory/write`）：整份正文 + **乐观锁**（那次取正文时宿主的 sha256）。
         * 成败都如实播报；成功后把这一份**重新取一遍**（拿到新的 sha / 新的块），并把死区现状重问一次
         * （用户自己改的 ⇒ 宿主那边快照跟着走，这里必须看到"一致"，⛔ 不许自己以为还红着）。
         * ⛔ 失败**一个字都不写盘**（宿主拒了就拒了）；409 那条要说清"先刷新再改"。
         */
        const saveFile = (name) => {
          if (saving.status === 'busy') return
          setSaving({ status: 'busy', message: '', receipt: null })
          ;(async () => {
            try {
              const data = await apiPost(HOST_API_BASE + '/playthrough/rp-memory/write', undefined, {
                file: name, text: draft, sha256: pane.sha256,
              })
              setEditing('')
              toggleFile(name, true)   // ★ 就地刷新：新的字节/新的 sha/新的块（⛔ 不拿旧的那份继续改）
              // ⚠️ 回执要**排在刷新之后**（toggleFile 会把 saving 清空 —— 反了就什么都看不到）
              setSaving({ status: 'ok', message: '', receipt: data })
              loadStatus()
              loadFloors()   // ★ 决定④：这一份是用户自己改的 ⇒ 那一楼的快照在宿主那边已经跟着刷新了，重取一遍
            } catch (error) {
              // 撞乐观锁（409）：⛔ 一个字都没写下去 —— 草稿**留着**（用户可能想把自己那段挪过去），
              //   同时给一颗"重新读取"（会**丢掉**草稿、重取盘上那一份），由用户自己决定。
              const stale = error && error.code === 'RP_MEMORY_WRITE_STALE'
              setSaving({ status: 'err', code: stale ? 'RP_MEMORY_WRITE_STALE' : '', message: errText(error), receipt: null })
            }
          })()
        }

        /**
         * 划死区 / 解锁（`POST /playthrough/rp-memory/deadzones`）：凭**那一段的快照 sha** 定位，
         * 原文快照一并交上去（宿主存下来当"身份证"）。⛔ 不动剧情文本一个字节 —— 宿主那边也只动
         * `.dma-deadzones.json`。完成后刷新这一份（锁标要跟着变）。
         */
        const toggleZone = (name, block, want) => {
          if (zoneBusy !== '') return
          setZoneBusy(block.sha256)
          setZoneMsg({ status: 'idle', message: '' })
          ;(async () => {
            try {
              await apiPost(HOST_API_BASE + '/playthrough/rp-memory/deadzones', undefined, {
                action: want ? 'add' : 'remove', file: name, sha256: block.sha256, text: block.text,
              })
              setZoneMsg({ status: 'ok', message: want ? '这一段已设为死区（模型不许改写/覆盖/删除）' : '这一段已解锁' })
              toggleFile(name, true)
              loadStatus()
            } catch (error) {
              setZoneMsg({ status: 'err', message: errText(error) })
            } finally {
              setZoneBusy('')
            }
          })()
        ...
        // ★ 2026-09-23：死区现状摊成人话（**判据全在宿主**：哪条不一致、两个 sha 各是多少，⛔ 这里不自己比）。
        const alertData = alert.status === 'ready' && alert.data ? alert.data : null
        const alertItems = alertData && Array.isArray(alertData.items) ? alertData.items : []
        const alertBad = alertItems.filter((i) => i && i.state !== 'ok')
        const badFiles = new Set(alertBad.map((i) => i.file))
        /** 那条不一致怎么说（state 三种各自一句话 + 两个 sha）。⛔ 文案**不定性**：不写"模型干的"。 */
        const alertLine = (i) => {
          const head = i.file + ' 的「' + i.firstLine + '」：'
          if (i.state === 'missing-file') return head + '那份文件现在读不到（被删了？还是挪走了？）'
          if (i.state === 'missing') return head + '那一段在现文里找不到了（被整段改写/删除/挪走了）'
          return head + '快照 ' + String(i.snapSha).slice(0, 12) + '… → 现况 ' + String(i.nowSha).slice(0, 12) + '…'
        }
        /** 锁标（一块一段）：已死的块灰底 + 🔒；没死的给「设为死区」。 */
        const blockRow = (name, b) => {
          const busy = zoneBusy !== '' && zoneBusy === b.sha256
          return e('div', {
            key: 'blk-' + name + '-' + b.index,
            'data-block': String(b.index), 'data-dead': b.dead ? '1' : '0',
            style: Object.assign({}, itemStyle, b.dead
              ? { background: 'color-mix(in srgb, CanvasText 8%, Canvas)', borderColor: 'color-mix(in srgb, CanvasText 22%, transparent)' }
              : {}),
          },
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', marginBottom: 4 } },
              e('span', { style: Object.assign({}, dimStyle, { flex: 1, minWidth: 0, fontFamily: MONO }) },
                '第 ' + String(b.index + 1) + ' 块 · ' + (b.firstLine || '（空行开头的一段）') + ' · ' + fmtChars(b.chars) + ' 字'),
              b.dead
                ? e('span', { style: warnBadgeStyle, 'data-zone-locked': '1' }, '🔒 死区 · 模型不许改')
                : null,
              e('button', {
                id: 'dma-rpmem-zone-' + name + '-' + b.index,
                className: 'dma-btn', style: b.dead ? miniBtnStyle : Object.assign({}, miniBtnStyle, { fontWeight: 700 }),
                disabled: zoneBusy !== '',
                title: b.dead
                  ? '解锁这一段（它不再算模型禁改区）'
                  : '把这一段划为死区：宿主会把这条约束喂给模型（每轮都看得见），并在每轮比对这一段有没有被改；改了只在面板/日志里告警，⛔ 不会替你改回去。',
                // ⚠️ 必须**吃掉冒泡**：这一层外面就是"点一下整行收起"的老行为
                //   （不收的话，点"设为死区"会把这一份收起来、回执也一起没了）
                onClick: (ev) => { if (ev && ev.stopPropagation) ev.stopPropagation(); toggleZone(name, b, !b.dead) },
              }, busy ? '…' : (b.dead ? '🔓 解锁' : '🔒 设为死区')),
            ),
            e(MarkdownBody, { text: b.text }),
          )
        }
        const rows = st.files.map((f) => e('div', {
          ...
          // ★ 2026-09-25 美化：行头包一层 flex（名字+徽标在左，字数/时间贴右），不再一串 inline 顺排；锚与文案未动
          e('div', { className: 'dma-rp-filehd' },
            e('span', { style: { fontFamily: MONO, fontWeight: 600 } }, f.name),
            // ★ 2026-09-26（收纳升级）：notes.md 过保养线 ⇒ **亮 ⚠**（与「死区不符」同款琥珀徽标）。
            //   判据按**字节**近似（清单只给 bytes；CJK 在 UTF-8 ≈ 3 字节/字 ⇒ 6000 字 ≈ 18000 字节；
            //   精确的字数判定在宿主注入那一路 `notesMaintenanceFor`，这里只是让用户一眼看见）。
            f.name === 'notes.md' && f.exists && Number.isFinite(f.bytes) && f.bytes >= 6000 * 3
              ? e('span', {
                  style: warnBadgeStyle, 'data-notes-over': '1',
                  title: 'notes.md 已超过保养线（约 6000 字）—— 该收纳了：最早场记压成一行、伏笔移进 world.md',
                }, '⚠ 超保养线')
              : null,
            f.deadzones > 0 ? e('span', { style: dimBadgeStyle, 'data-zones': String(f.deadzones) }, '🔒 ' + String(f.deadzones) + ' 段死区') : null,
            badFiles.has(f.name) ? e('span', { style: warnBadgeStyle, 'data-deadzone-bad': '1' }, '⚠ 死区不符') : null,
            e('span', { style: Object.assign({}, dimBadgeStyle, { marginLeft: 'auto' }) }, f.exists ? (formatBytes(f.bytes) + ' · ' + (f.mtime ? new Date(f.mtime).toLocaleString() : '—')) : '还没有'),
          ),
          ...
          openName === f.name && pane.status === 'ready'
            ? e('div', null,
                // ── 工具条：阅读 / 直接改（⛔ 只有过了手动确认、且是 .md/.txt 才给编辑入口）──
                e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', marginBottom: 6 } },
                  e('span', { style: dimStyle },
                    fmtChars(pane.chars) + ' 字 · sha ' + String(pane.sha256).slice(0, 12) + '…'
                    + (pane.deadzones > 0 ? ' · 其中 ' + String(pane.deadzones) + ' 段是死区' : '')),
                  e('button', {
                    id: 'dma-rpmem-edit-' + f.name,
                    className: 'dma-btn', style: miniBtnStyle,
                    disabled: pane.writable !== true || pane.truncated === true || pane.blocks === null,
                    title: pane.writable !== true
                      ? '面板里只直接改 .md / .txt（这一份不是）—— 请用系统编辑器'
                      : (pane.truncated === true
                        ? '这一份太大，面板只显示了前面一段 ⇒ ⛔ 不给编辑入口（免得把没显示出来的部分写没）'
                        : '整份正文直接改：保存前先自动备份成 .bak-时间戳，带乐观锁（盘上变了就拒，绝不覆盖）'),
                    onClick: (ev) => { if (ev && ev.stopPropagation) ev.stopPropagation(); if (editing === f.name) { setEditing(''); return } startEdit(f.name) },
                  }, editing === f.name ? '✎ 收起编辑器' : '✎ 直接改'),
                  // ★ 编辑器**就地**摊开（⛔ 不弹窗、不跳走）：读的是**这一份**的正文
                  editing === f.name && e('span', { style: dimStyle }, '正在编辑『' + f.name + '』——保存后原件先备份成 .bak-时间戳'),
                ),
                editing === f.name
                  ? e('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
                      e('textarea', {
                        ...
                        style: {
                          ...
                          padding: '8px 10px', fontFamily: MONO, fontSize: MONO_FONT_SIZE, lineHeight: MONO_LINE_HEIGHT,
                          whiteSpace: 'pre-wrap', background: 'Canvas', color: 'CanvasText',
                          border: '1px solid color-mix(in srgb, CanvasText 30%, transparent)', borderRadius: 6,
                        },
                      }),
                      e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' } },
                        e('button', {
                          ...
                          onClick: (ev) => { if (ev && ev.stopPropagation) ev.stopPropagation(); saveFile(f.name) },
                        }, saving.status === 'busy' ? '正在保存…' : '保存这一份'),
                        e('button', {
                          id: 'dma-rpmem-cancel',
                          className: 'dma-btn', style: miniBtnStyle,
                          disabled: saving.status === 'busy',
                          onClick: (ev) => { if (ev && ev.stopPropagation) ev.stopPropagation(); setEditing(''); setSaving({ status: 'idle', message: '', receipt: null }) },
                        }, '放弃改动'),
                        e('span', { style: dimStyle },
                          '保存 = 用你看到的这份整份覆盖（先备份）；盘上要是已经变过 ⇒ 拒改并让你先刷新（乐观锁）。'),
                      ),
                      saving.status === 'err' ? e('div', { 'data-dma': 'rp-memory-save-err', style: errorStyle },
                        '没保存（盘上一个字节都没动）：' + saving.message,
                        saving.code === 'RP_MEMORY_WRITE_STALE'
                          ? e('button', {
                              id: 'dma-rpmem-reload',
                              className: 'dma-btn', style: Object.assign({}, miniBtnStyle, { marginLeft: 8 }),
                              disabled: saving.status === 'busy',
                              title: '丢开你手里这份草稿、重新读盘上那一份（盘上那份才是有可能刚被改过的那份）',
                              onClick: (ev) => {
                                if (ev && ev.stopPropagation) ev.stopPropagation()
                                setEditing('')
                                toggleFile(f.name, true)
                                loadStatus()
                              },
                            }, '重新读取（丢掉我这份草稿）')
                          : null)
                        : null,
                      saving.status === 'ok' && saving.receipt
                        ? e('div', { 'data-dma': 'rp-memory-save-ok', style: okStyle },
                            '已保存：' + fmtChars(saving.receipt.chars) + ' 字 / ' + String(saving.receipt.bytes) + ' 字节 · sha '
                            + String(saving.receipt.sha256 || '').slice(0, 12) + '… · 备份 ' + String(saving.receipt.backup || '无')
                            + (saving.receipt.deadzones && saving.receipt.deadzones.refreshed > 0
                              ? ' · 死区快照已跟着刷新 ' + String(saving.receipt.deadzones.refreshed) + ' 条'
                                + (saving.receipt.deadzones.updated > 0 ? '（其中 ' + String(saving.receipt.deadzones.updated) + ' 条是你刚改过的那段）' : '')
                              : '')
                            + (saving.receipt.deadzones && Array.isArray(saving.receipt.deadzones.dropped) && saving.receipt.deadzones.dropped.length > 0
                              ? ' · 有 ' + String(saving.receipt.deadzones.dropped.length) + ' 条死区的那一段被你整段改掉了 ⇒ 已经解除'
                              : '')
                            + (saving.receipt.deadzones && saving.receipt.deadzones.error
                              ? ' · ⚠ 死区快照没跟着刷新（' + String(saving.receipt.deadzones.error) + '）：文件写好了，但死区那份数据有问题，去看一眼'
                              : ''))
                        : null,
                    )
                  : e('div', null,
                      // ── 阅读态：**逐块**渲染（每块一个锁标：这就是"划死区"的落点）──
                      pane.blocks !== null && pane.blocks.length === 0 && e('div', { style: dimStyle }, '（这一份是空的：整份只有空行。）'),
                      pane.blocks === null && e(MarkdownBody, { text: pane.text }),
                      pane.blocks !== null && pane.blocks.map((b) => blockRow(f.name, b)),
                      zoneMsg.status === 'ok' ? e('div', { 'data-dma': 'rp-memory-zone-ok', style: okStyle }, zoneMsg.message) : null,
                      zoneMsg.status === 'err' ? e('div', { 'data-dma': 'rp-memory-zone-err', style: errorStyle }, '没改成：' + zoneMsg.message) : null,
                    ),
                pane.truncated ? e('div', { style: dimStyle }, '（已截断：显示 ' + fmtChars(pane.chars) + ' / 共 ' + fmtChars(pane.originalChars) + ' 字符）') : null)
            : null,
        ))

        // ── ★★ 2026-09-23「**按楼层绑定**的笔记快照」（用户口径：「每轮模型修改都记录，能自动跟随回档」）──
        //   判据**全在宿主**（哪一楼是第几楼、改了哪几份、当前站在哪一楼、该不该恢复），这里只把结果摊成人话。
        //   ⛔ 面板一个重要判断都不做 —— 与死区那块同款纪律。
        const floorData = floorPane.status === 'ready' && floorPane.data ? floorPane.data : null
        const floorRows = floorData && Array.isArray(floorData.floors) ? floorData.floors : []
        // ── ★★ 2026-09-24「**回档改成手动挡**」：档顶那条**显著**横幅 ──────────────────────
        //   用户原话：「**不要做跟随楼层的功能，先做手动挡**，开 fork 时（回档）**前台弹提示，让用户手动
        //   回退剧情档案**。做**更显著的更改表示**」。判据**全在宿主**（那条 `pending` 是宿主记的、
        //   楼号与目标也是宿主算的）：面板只把它摊成人话、把三颗按钮发出去。
        //   ★★ 收尾改口径（用户原话「回档我没看到有横幅。」＋「**开 fork 时（回档）前台弹提示**」）：
        //     弹不弹**不再**看"档案在哪一楼"（老判据在真机恒为 forward ⇒ 横幅这辈子不弹），而是看
        //     **剧情有没有开 fork**（宿主的 `decideFork`）。★ 2026-09-25 起这一排只有**三颗**（用户逐字给的口径）：
        //       ①「**对齐楼层**」= `settlePending(…, 'full')` ＝ **就地登记**（⛔ 正文一个字节都不写）；
        //       ②「**只对齐楼号**」= `settlePending(…, 'number')` ＝ ★ 新：只把 `last` 锚过去（⛔ 不记快照、
        //          ⛔ 不写 blob、⛔ 不新增楼层行）；
        //       ③「**不处理**」= `ackPending`（清掉那条提示，⛔ 什么都不写）。
        //     ⛔ 「把档案退回第 N 楼」**不在这排里**了（那颗随本单撤掉）—— 那个动作仍然在**楼层清单每一行**
        //     的「回到这一楼」里（`gotoFloor`，⛔ 那条路本单一个字都没动）。✕ 只在浮层那边（横幅不需要）。
        //   ⛔ 没有 pending ⇒ **什么都不渲染**（不是渲染一行"暂无"）。
        const floorPending = floorData && floorData.pending && typeof floorData.pending === 'object' ? floorData.pending : null
        // ★ 2026-09-25：那三个位置显示的是**序号**（宿主给的另一份 `pendingOrdinals`；⛔ 面板不自己排、不自己编）。
        const floorPendingOrd = floorData && floorData.pendingOrdinals ? floorData.pendingOrdinals : null
        const pendingKey = floorPending && typeof floorPending.targetKey === 'string' ? floorPending.targetKey : ''
        /** ★ 2026-09-25（用户口径「**不要楼号了，基本没用，只标序号**」）：**序号**的人话 ——
         *  认不出（那一行不在清单里 / 那行没时间）就如实说「序号未知」，⛔ 不编一个 0 出来。
         *  ⚠️ 用它显示的那几个数**全是宿主给的**（`fromOrdinal` / `toOrdinal` / `archiveOrdinal` /
         *  `head.ordinal` / `last.ordinal` / 每一行的 `ordinal`）—— ⛔ 面板不自己排、不自己编。 */
        const floorSeat = (n) => (Number.isFinite(n) ? '序号 ' + String(n) : '序号未知')
        /** 横幅/状态行共用的那套"显著"配色（用户口径「做更显著的更改表示」；照现成那条横幅的抄）。 */
        const floorAlertBox = {
          background: 'color-mix(in srgb, DarkGoldenrod 12%, Canvas)',
          border: '2px solid color-mix(in srgb, DarkGoldenrod 55%, transparent)',
        }
        const floorAlertText = { color: 'color-mix(in srgb, DarkGoldenrod 85%, CanvasText)', fontWeight: 700 }
        /** "剧情怎么动的"那一句：同一楼换支（swipe 重 roll）时说"换了支"，退楼时说"从序号 X 退到序号 Y"
         *  —— ⛔ 不拿"从序号 4 退到序号 4"那种话糊弄（两个号一样就说明"同一楼的另一种分支"）。
         *  ⚠️ 序号那两处**两侧留空格**（`序号 4` 结尾是数字 ⇒ 不留空格会跟后面的汉字黏成一坨）。 */
        const floorMoveText = floorPending === null ? '' : (floorPending.why === 'variant'
          ? '剧情在 ' + floorSeat(floorOrdOf(floorPendingOrd, 'to')) + ' 换了支（swipe 重 roll）'
          : '剧情刚从 ' + floorSeat(floorOrdOf(floorPendingOrd, 'from')) + ' 退到 ' + floorSeat(floorOrdOf(floorPendingOrd, 'to')))
        /** 那条**待处理**横幅（★ 2026-09-25：三颗按钮，⛔ 不再有「把档案退回」那颗，也没有那一句说明）。 */
        const floorPendingBanner = floorPending === null ? null : e('div', {
          'data-dma': 'floor-pending',
          role: 'alert',
          style: Object.assign({ display: 'flex', flexDirection: 'column', gap: 6, padding: '10px 12px', borderRadius: 8 }, floorAlertBox),
        },
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', fontSize: 14, fontWeight: 700 } },
            e('span', { 'data-dma': 'floor-pending-icon', style: { fontSize: 16 } }, '⚠'),
            // ★ 标题（形状照用户口径给的）：`⚠ 检测到回档：序号 40 → 序号 38`（剧情**之前站的** → **现在站的**）
            '检测到回档：' + floorSeat(floorOrdOf(floorPendingOrd, 'from')) + ' → ' + floorSeat(floorOrdOf(floorPendingOrd, 'to'))),
          e('div', { style: { fontSize: SMALL_FONT_SIZE, lineHeight: 1.6, opacity: 0.85 } },
            floorMoveText + '，但剧情档案（模型维护的那 5 份笔记）'
            + '现在停在' + floorSeat(floorOrdOf(floorPendingOrd, 'archive')) + ' —— 跟 Tavern 一样，回档之后那段历史就按新的算；'
            + '档案要不要跟着对上来，由你点下面那三颗。'),
          // ★★ 2026-09-24（浮层那一单）：三颗按钮**不再各画一份** —— 与顶部那条浮层共用
          //   `FloorPendingActions`（文案 / id / 禁用判据，只有那一处说了算）。
          //   ⚠️ 这一处的 id 前缀是 `dma-rpmem-floor-pending`（浮层那处是另一套 —— 两处同时在屏上时
          //   DOM 里不许出现重复 id）；这一处的动作 = 面板这三个具名回调（横幅那三颗就画在这儿）。
          //   ★ 2026-09-25：前两颗同一个端点、靠 `mode` 分（`'full'` = 对齐楼层 / `'number'` = 只对齐楼号）。
          e(FloorPendingActions, {
            pending: floorPending,
            ordinals: floorPendingOrd,
            idPrefix: 'dma-rpmem-floor-pending',
            busy: pendingMsg.status === 'busy' || gotoMsg.status === 'busy',
            onSettle: () => settlePending(pendingKey, 'banner', 'full'),
            onNumber: () => settlePending(pendingKey, 'banner', 'number'),
            onAck: ackPending,
          }),
          pendingMsg.status === 'idle' ? null : e('div', {
            'data-dma': pendingMsg.status === 'err' ? 'floor-pending-err' : (pendingMsg.status === 'ok' ? 'floor-pending-ok' : 'floor-pending-busy'),
            role: pendingMsg.status === 'err' ? 'alert' : undefined,
            style: pendingMsg.status === 'err' ? errorStyle : (pendingMsg.status === 'ok' ? okStyle : dimStyle),
          }, pendingMsg.status === 'busy' ? '正在处理…' : pendingMsg.message),
        )
        // ── ★★ 2026-09-24 收尾：档顶那条**常驻状态行** ────────────────────────────────────
        //   用户原话「做**更显著的更改表示**」⇒ 让"剧情在哪一条 / 档案在哪一条"**一直看得见**：
        //   `剧情 序号 38 · 档案 序号 34`；**两处一样** ⇒ 灰（安静）；**不一样** ⇒ 琥珀 + 一颗
        //   「**档案对齐到序号 N**」（= `settlePending` ＝就地登记，⛔ 正文一个字节都不写）。
        //   ★ 2026-09-25（用户口径「**不要楼号了……只标序号**」）：这里的两个位置换**序号**口径 ——
        //     显示的是 `head.ordinal` / `last.ordinal`（宿主按记录时间现算的"第几版"）。
        //   ⚠️ 序号全是**宿主给的**（⛔ 面板不自己排、⛔ 认不出不编 0 ⇒ 照实写「序号未知」）；
        //     判"不一样"只是把两个数比一下 —— 弹不弹、能不能对齐仍是宿主的事。
        const floorStateLine = floorData === null || floorData.ok !== true ? null : (() => {
          const headOrd = floorData.head !== null && floorData.head !== undefined && Number.isFinite(floorData.head.ordinal) ? floorData.head.ordinal : null
          const lastOrd = floorData.last !== null && floorData.last !== undefined && Number.isFinite(floorData.last.ordinal) ? floorData.last.ordinal : null
          // ⚠️ 认不出（任一边没有序号）⇒ **不算"一样"**（也⛔ 不许编一个 0）：按"不在同一条上"标出来，
          //   让用户知道这一刻对不齐/看不出来；有没有可对齐的目标仍由宿主判（它有 target 才画那颗按钮）。
          const same = headOrd !== null && lastOrd !== null && headOrd === lastOrd
          const canAlign = floorData.head !== null && floorData.head !== undefined && typeof floorData.head.nodeId === 'string' && floorData.head.nodeId !== ''
          // ★ 2026-09-25 美化：一致态不再自带盒底（进了「当前状态」分组卡，安静一行即可）；
          //   不一致仍是琥珀盒（floorAlertBox，与横幅/浮层同一套"显著"配色）。锚与文案未动。
          return e('div', {
            'data-dma': 'floor-state-line',
            'data-floor-same': same ? '1' : '0',
            style: Object.assign({ display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' },
              same ? {} : Object.assign({ padding: '6px 10px', borderRadius: 8 }, floorAlertBox)),
          },
            e('span', { style: same ? dimStyle : Object.assign({}, floorAlertText, { fontSize: SMALL_FONT_SIZE }) },
              '剧情 ' + floorSeat(headOrd) + ' · 档案 ' + floorSeat(lastOrd) + (same ? '' : '（不在同一楼）')),
            !same && canAlign ? e('button', {
              id: 'dma-rpmem-floor-align', className: 'dma-btn', style: miniBtnStyle,
              disabled: alignMsg.status === 'busy' || pendingMsg.status === 'busy' || gotoMsg.status === 'busy',
              title: '就地登记：把「现在这份档案」记成剧情现在站的这一楼（' + floorSeat(headOrd) + '）的样子，往后从这一楼接着记。'
                + '⛔ 那 5 份的正文一个字节都不写（只动我们自己的清单和快照），⛔ 不影响会话。',
              onClick: () => settlePending(pendingKey, 'state', 'full'),
            }, alignMsg.status === 'busy' ? '正在对齐…' : '档案对齐到' + floorSeat(headOrd)) : null,
            alignMsg.status === 'idle' ? null : e('span', {
              'data-dma': alignMsg.status === 'err' ? 'floor-align-err' : (alignMsg.status === 'ok' ? 'floor-align-ok' : 'floor-align-busy'),
              role: alignMsg.status === 'err' ? 'alert' : undefined,
              style: alignMsg.status === 'err' ? errorStyle : (alignMsg.status === 'ok' ? okStyle : dimStyle),
            }, alignMsg.message),
            e('span', { style: dimStyle }, same
              ? '（档案与剧情在同一楼）'
              : '（档案不在剧情这一楼 —— 这几份笔记是"剧情到过的那一刻"的样子；要就地对齐就点左边那颗）'),
          )
        })()
        /** 一份文件"改了多少"（宿主给的是"比上一楼 +N 字节"；没有基数就报总量 —— ⛔ 不编 0）。 */
        const floorFileText = (c) => {
          if (!c || typeof c.name !== 'string') return ''
          if (Number.isFinite(c.delta)) return c.name + '（' + (c.delta >= 0 ? '+' : '') + String(c.delta) + ' 字节）'
          return c.name + (Number.isFinite(c.bytes) ? '（共 ' + String(c.bytes) + ' 字节）' : '')
        }
        /** "为什么这一份没动"（跳过原因）的人话 —— 认不出的原样透出，⛔ 不硬翻。
         *  ★ 2026-09-23 改口径：`deadzone` 不再是"整份跳过"，而是"**按块合并**"（那几段保留盘上现况）。
         *  ★★ 2026-09-26：那张表与 `floorSkipText` **挪到上面去了**（`FloorPopup` 那颗「对齐楼层」
         *    也是走 restore 那一脚、回执要同一份画法 ⇒ 两处共用一个 —— ⛔ 不在这一档里再养一份）。
         *    下面这一档只留"这一行/这一脚的回执怎么画"，见 `floorRestoreDetail`（也在上面）。 */
        // ── ★★ 2026-09-25（用户口径「**默认显示最近10楼，从序号大到小排序**」）：清单的**默认窗口** ──
        //   ① 顺序：宿主**已经排好**了（按 `ordinal` 从大到小 ⇒ 最新在最上）—— ⛔ 面板不排序、不比新旧；
        //   ② 默认只画**最近 10 行**，下面给一颗「显示其余 N 条」（点开 = 全部，再点收起）；
        //   ③ ⚠️ **「当前剧情那一行」与「档案指针那一行」必须永远可见** —— 用户回档到很早以前时，
        //      它们可能落在最近 10 条**之外**（那就等于"回档完找不到高亮那一条"，这功能就废了）
        //      ⇒ 窗口 = **最近 10 行 ∪ {那两行}**；中间被省掉的那几段用**一行** `…（省略 N 条）` 表示。
        //      ⚠️ "哪一行是当前 / 哪一行是档案指针"全是**宿主**给的（`current` / `pointer` 两个布尔），
        //      ⛔ 面板不拿 nodeId 去比（与"判据全在宿主"同一条纪律）。
        const FLOOR_RECENT_N = 10
        /** 默认窗口里**留下**的那些下标（最近 10 行 ∪ {当前剧情那一行, 档案指针那一行}）——
         *  展开态与那颗按钮的计数都读它（⛔ 两处各算一遍迟早会漂）。 */
        const floorKeepSet = (() => {
          const keep = new Set()
          floorRows.forEach((f, i) => {
            if (i < FLOOR_RECENT_N || f.current === true || f.pointer === true) keep.add(i)
          })
          return keep
        })()
        const floorVisible = (() => {
          if (floorRows.length === 0) return []
          if (floorAll === true) return floorRows.map((f) => ({ kind: 'row', f: f }))
          const out = []
          let gap = 0
          floorRows.forEach((f, i) => {
            if (floorKeepSet.has(i)) {
              // 中间被省掉的那几段：**一行**省略标记（⛔ 不逐条列出来 —— 那正是"默认只 10 条"要治的噪声）。
              if (gap > 0) out.push({ kind: 'gap', n: gap })
              gap = 0
              out.push({ kind: 'row', f: f })
            } else gap += 1
          })
          // 末尾那一段不画标记（下面那颗「显示其余 N 条」已经说了这件事）—— 留白比重复一句更清楚。
          return out
        })()
        /** 默认窗口外面还有几条（＝那颗按钮的计数；**展开态也照它**，不然展开之后就没得收回了）。 */
        const floorHidden = floorRows.length - floorKeepSet.size
        /** 一行的键（展开态与请求都认它 —— 与宿主清单里的 `floorKeyOf` 同一个拼法：`nodeId` + \\0 + `variantId`）。 */
        const floorDiffKey = (f) => String(f.nodeId) + '\u0000' + String(f.variantId || '')
        /** 那一行是不是**正展开着**改动下钻（展开态只认一行）。 */
        const floorDiffOpen = (f) => floorDiff.status !== 'idle' && floorDiff.key === floorDiffKey(f)
        /** 点那一颗：没展开 ⇒ **发这一次**请求（按需取）；已展开 ⇒ 收起来（⛔ 不发任何请求）。 */
        const toggleFloorDiff = (f) => {
          const key = floorDiffKey(f)
          if (floorDiffOpen(f)) { setFloorDiff({ key: '', status: 'idle', data: null, error: '' }); return }
          loadFloorDiff(f, key)
        }
        /** 一份文件的 kind → 人话（⛔ 认不出的原样透出，不硬翻）。`changed`/`same` 两相由 diff 内容自己说。 */
        const FLOOR_DIFF_KIND_TEXT = {
          absent: '这一楼当时没有这一份',
          'not-target': '不在那 5 份里',
          deadzone: '这一份有死区 ⇒ 恢复时按块合并（死区那几段保留盘上现况）',
          unreadable: '这一份的正文读不出来 ⇒ 这里看不了内容（⛔ 不代表没改动）',
        }
        /** 增删行的配色（⛔ 只用系统色关键字 —— 亮/暗主题自动跟着走）。 */
        const floorDiffLineStyle = (t) => (t === '+'
          ? { color: 'color-mix(in srgb, green 78%, CanvasText)' }
          : t === '-'
            ? { color: 'color-mix(in srgb, crimson 82%, CanvasText)' }
            : t === '@'
              ? { color: 'GrayText', fontStyle: 'italic' }
              : { color: 'GrayText' })
        /** 一份文件那一条：名字 · 标签 · ±行数 · （超长时）截断说明 · 逐行内容。 */
        const floorDiffFile = (file) => {
          const kindText = file.kind === 'changed' || file.kind === 'same' ? '' : (file.reason || FLOOR_DIFF_KIND_TEXT[file.kind] || String(file.kind))
          const counts = file.added > 0 || file.removed > 0 ? '+' + String(file.added) + ' / −' + String(file.removed) : ''
          const lines = Array.isArray(file.lines) ? file.lines : []
          return e('div', { key: 'fdf-' + String(file.name), style: { marginTop: 6 } },
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', fontSize: SMALL_FONT_SIZE } },
              e('span', { style: { fontFamily: MONO, fontWeight: 600 } }, String(file.name)),
              kindText === '' ? null : e('span', { style: file.kind === 'unreadable' ? errorStyle : dimStyle }, kindText),
              counts === '' ? null : e('span', { style: dimStyle }, counts + ' 行'),
              file.truncated === true
                ? e('span', { style: dimStyle }, '（太长，只显示前 ' + String(lines.length) + ' / ' + String(file.total) + ' 行）')
                : null,
            ),
            lines.length === 0
              ? (kindText === ''
                ? e('div', { style: dimStyle }, '与上一条逐字节一样')
                : null)
              : e('div', {
                'data-dma': 'floor-diff-lines',
                style: {
                  fontFamily: MONO, fontSize: SMALL_FONT_SIZE, whiteSpace: 'pre-wrap', wordBreak: 'break-word',
                  marginTop: 2, padding: '4px 6px', borderRadius: 6,
                  background: 'color-mix(in srgb, CanvasText 5%, Canvas)',
                },
              }, lines.map((l, i) => e('div', { key: 'fdl-' + String(i), 'data-diff-t': String(l.t), style: floorDiffLineStyle(l.t) },
                l.t === '@' ? '… ' + String(l.text) : String(l.t) + ' ' + String(l.text)))),
          )
        }
        /** 就地展开的那一块（点了那一颗之后才有内容；加载中 / 失败 / 空 diff 都**如实画**）。 */
        const floorDiffBlock = (f) => {
          if (!floorDiffOpen(f)) return null
          if (floorDiff.status === 'loading') {
            return e('div', { 'data-dma': 'floor-diff-loading', style: Object.assign({}, dimStyle, { marginTop: 6 }) }, '正在读这一条的改动…')
          }
          if (floorDiff.status === 'error') {
            return e('div', { 'data-dma': 'floor-diff-err', role: 'alert', className: 'dma-rp-note', 'data-kind': 'err', style: { marginTop: 6 } },
              '改动读不出来：' + String(floorDiff.error || '（宿主没说为什么）'))
          }
          const d = floorDiff.data
          if (!d || d.ok !== true) {
            return e('div', { 'data-dma': 'floor-diff-err', role: 'alert', className: 'dma-rp-note', 'data-kind': 'err', style: { marginTop: 6 } },
              '改动读不出来：' + String((d && d.error && d.error.message) || '宿主没给可读的原因'))
          }
          const files = Array.isArray(d.files) ? d.files : []
          const nothing = d.against === null
          return e('div', {
            'data-dma': 'floor-diff',
            'data-floor-diff-against': nothing ? '' : String(d.against.nodeId) + '|' + String(d.against.variantId || ''),
            style: Object.assign({}, itemStyle, { marginTop: 6, background: 'color-mix(in srgb, CanvasText 3%, Canvas)' }),
          },
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap', fontSize: SMALL_FONT_SIZE } },
              e('span', { style: { fontWeight: 700 } }, '看改动'),
              e('span', { style: dimStyle }, String(d.message || '')),
              d.truncated === true ? e('span', { style: dimStyle }, '（有文件太长，内容已截断）') : null),
            // 死区数据读不出来 ⇒ 认不出哪几份里有死区 ⇒ **如实说**（⛔ 不许当"都没有死区"）
            d.deadzonesReadable === false
              ? e('div', { style: Object.assign({}, errorStyle, { marginTop: 4 }) }, '⚠ 死区数据读不出来 ⇒ 认不出哪一份里有死区（下面那几份的"死区"标记可能不全）')
              : null,
            nothing
              ? null
              : (files.length === 0
                ? e('div', { style: dimStyle }, '（这一条没有可比的份）')
                : files.map(floorDiffFile)),
            d.against === null
              ? null
              : e('div', { style: Object.assign({}, dimStyle, { marginTop: 4 }) },
                '只列模型维护的那 5 份（预置那几份不在快照里）；「有死区」的那几份恢复时按块合并，死区那几段不会被回档改动。'),
          )
        }
        /** 一楼**一行 = 一支**（`第 N 楼 · 变体 · 时间 · 改了哪几份（+几字节）· [回到这一楼]`）；
         *  当前站的那一支高亮。★ 2026-09-23 改口径：快照按 `(楼层, 变体)` 记 ⇒ 同一楼 swipe 过会有多行。 */
        // ★★ 2026-09-25 美化（用户拍板）：一楼改**两行定式** —— 第 1 行 = 楼号·徽标·时间·按钮（时间与按钮
        //   `margin-left:auto` 贴右，不再跟「改了哪几份」挤同一行窄屏换行三四次），第 2 行 = 改了哪几份；
        //   死区/缺失附注仍只在有内容时出现。锚（data-floor*）与全部文案逐字未动。
        const floorRow = (f) => e('div', {
          key: 'floor-' + f.nodeId + '-' + (f.variantId || ''),
          'data-floor': String(Number.isFinite(f.seq) ? f.seq : '?'),
          // ★ 2026-09-25 新增（**新锚**，老那几个一个都没动）：这一行显示的**序号**（宿主算的 `ordinal`）。
          //   ⛔ `data-floor` 里那个 `seq` 照旧一个字节没变（台子与别的定位还按它找这一行）。
          'data-floor-ordinal': String(Number.isFinite(f.ordinal) ? f.ordinal : '?'),
          'data-floor-variant': String(f.variantId || ''),
          'data-floor-current': f.current === true ? '1' : '0',
          'data-floor-legacy': f.legacy === true ? '1' : '0',
          style: Object.assign({}, itemStyle, f.current === true
            ? { borderColor: 'color-mix(in srgb, CanvasText 45%, transparent)', background: 'color-mix(in srgb, CanvasText 7%, Canvas)' }
            : {}),
        },
          e('div', { className: 'dma-rp-fll1' },
            // ★ 2026-09-25（用户口径「**不要楼号了，基本没用，只标序号**」）：行头显示 **序号**（宿主算的），
            //   ⛔ 不再显示 Tavern 那个楼号；认不出 ⇒ 如实「序号未知」（⛔ 不编 0）。
            e('span', { style: { fontFamily: MONO, fontWeight: f.current === true ? 700 : 400 } }, floorSeat(f.ordinal)),
            f.current === true ? e('span', { style: Object.assign({}, dimBadgeStyle, { fontWeight: 700 }) }, '◀ 当前这一楼') : null,
            // 同一楼 swipe 出来的**另一种分支**：如实标一句（⛔ 不把它当"当前这一楼"）。
            // ★ 2026-09-25：这一句原来说的是 Tavern 节点（"同一楼的另一支"）⇒ 改成与**序号**口径一致的说法
            //   （"这一条的另一个分支"），**意思一个字都没变**（用户口径第 1 条那个追问）。
            f.current !== true && f.sameNode === true && f.legacy !== true
              ? e('span', { style: dimBadgeStyle }, '这一条的另一个分支（swipe 重 roll）') : null,
            e('span', { style: Object.assign({}, dimStyle, { marginLeft: 'auto' }) }, (f.updatedAt || f.at) ? new Date(f.updatedAt || f.at).toLocaleString() : '—'),
            // ★★ 2026-09-25（用户口径「**开一个下钻，直接显示具体改动的字段**」）：每一行一颗「看改动 ▸」——
            //   点开就地展开这一条 vs **时间上紧随其前那一条**的真内容（增行/删行）。⚠️ **点了才发那一个请求**。
            e('button', {
              id: 'dma-rpmem-floor-diff-' + f.nodeId + '-' + (f.variantId || 'legacy'),
              className: 'dma-btn', style: miniBtnStyle,
              'data-dma': 'floor-diff-toggle',
              'data-floor-diff-open': floorDiffOpen(f) ? '1' : '0',
              title: '看改动：把这一条跟「时间上紧接着它的前一条」逐份比一遍 —— 显示**真正改了哪些行**'
                + '（增行 / 删行，带几行上下文），不是"改了哪几份"那句话。点了才去读，⛔ 不预取。',
              onClick: (ev) => {
                if (ev && ev.stopPropagation) ev.stopPropagation()
                toggleFloorDiff(f)
              },
            }, floorDiffOpen(f) ? '收起 ▴' : '看改动 ▸'),
            // ★ 老记录**只读、不许拿来恢复**（宿主那边也拒）⇒ 面板上这一行的按钮是禁用的，并说明为什么。
            e('button', {
              id: 'dma-rpmem-floor-goto-' + f.nodeId + '-' + (f.variantId || 'legacy'),
              className: 'dma-btn', style: miniBtnStyle,
              disabled: f.legacy === true || gotoMsg.status === 'busy',
              title: f.legacy === true
                ? '这一条是早先记的"只有楼层、没有变体"的老记录 ⇒ 不知道是哪一支，不能拿来恢复（只读）。'
                : '把笔记恢复到这一楼这一支的样子（恢复前先把你现在的样子另存一份，可撤销）。⛔ 不影响会话；⛔ 不碰预置那几份，⛔ 死区那几段保留盘上现况。',
              onClick: (ev) => {
                if (ev && ev.stopPropagation) ev.stopPropagation()
                if (f.legacy === true) return
                gotoFloor(f.nodeId, f.variantId)
              },
            }, gotoMsg.status === 'busy' && gotoMsg.nodeId === f.nodeId ? '正在恢复…' : (f.legacy === true ? '老记录·不能恢复' : '回到这一楼')),
          ),
          e('div', { style: { fontSize: SMALL_FONT_SIZE, color: 'GrayText', minWidth: 0 } },
            f.legacy === true
              ? '（老记录：只有楼层、不知道是哪一支）'
              : (Array.isArray(f.changed) && f.changed.length > 0
                ? '改了 ' + f.changed.map(floorFileText).join('、')
                : '（与上一楼逐字节相同，没记新东西）')),
          Array.isArray(f.deadzone) && f.deadzone.length > 0
            ? e('div', { style: dimStyle }, '🔒 其中 ' + f.deadzone.join('、') + ' 有死区 ⇒ 恢复时按块合并：那几段保留盘上现况，其余照这一楼回档（⛔ 死区内容永不自动还原）')
            : null,
          Array.isArray(f.absent) && f.absent.length > 0
            ? e('div', { style: dimStyle }, '这一楼当时没有：' + f.absent.join('、') + '（恢复时不动它 —— 回档恢复不删任何东西）')
            : null,
          // ★★ 就地展开的**改动下钻**（只有展开了的那一行画它）—— 加载中 / 失败 / 读不出来都如实画。
          floorDiffBlock(f),
        )
        /** 「回到这一楼」的回执（成功/失败/跳过都如实；⛔ 不许部分成功当全成功）。 */
        //   ⚠️ 被**拦住而没恢复**（死区名单未知）⇒ 红字，⛔ 不画成"成功"（那是"什么都没做"）。
        const gotoBlocked = gotoMsg.status === 'ok' && !!gotoMsg.detail
          && gotoMsg.detail.blocked !== null && gotoMsg.detail.blocked !== undefined
        const gotoLine = gotoMsg.status === 'idle' ? null : e('div', {
          'data-dma': gotoMsg.status === 'err' ? 'floor-goto-err' : (gotoBlocked ? 'floor-goto-blocked' : (gotoMsg.status === 'ok' ? 'floor-goto-ok' : 'floor-goto-busy')),
          role: gotoMsg.status === 'err' || gotoBlocked ? 'alert' : undefined,
          // ★ 2026-09-25 美化：回执走统一「提示条」（dma-rp-note），颜色由 data-kind 给（锚与文案未动）
          className: 'dma-rp-note',
          'data-kind': gotoMsg.status === 'err' || gotoBlocked ? 'err' : (gotoMsg.status === 'ok' ? 'ok' : 'dim'),
        },
          gotoMsg.status === 'busy' ? '正在把笔记恢复到那一楼…' : (gotoMsg.message || '已恢复'),
          // ★★ 2026-09-26：那三块（没写进去 / 已写回 / 跳过）改走**共用那一份** —— 弹窗那颗「对齐楼层」
          //   （也是走 restore 那一脚）画的是**同一份**（⛔ 不两处各写一遍；文案一个字没变）。
          ...floorRestoreDetail(gotoMsg.detail),
        )
        // ★★ 2026-09-25（用户口径第 4 条，逐字）：「**⇄ 最近一次回档：… 这一整段删掉**」
        //   ⇒ 那句「⇄ 最近一次回档」（`data-dma="floor-auto"`）**整块撤掉**：元素、渲染分支、
        //   以及为它留的取数点（原来读 `floorData.lastAuto`）**一并去掉**，本档**不再画它**。
        //   ⚠️ **宿主那份状态文件与它落的账一个字都没动**（`writeFloorStatus` 照旧写着）—— 本单只是不再画；
        //   那条账还有别的用途（判据、日志、回执里那句），将来要再画出来时它就是现成的取数口。
        /** 整块（列表 + 空态 + 拦路的那几条如实说明）。★ 2026-09-25 美化：升级成「楼层快照」分组卡，
         *  挂到左栏「当前状态」卡的下面（用户拍板方案 B —— 原先"排在文件清单下面"的一长列不再成立）；
         *  data-dma="floor-block" 锚留在这张卡上；
         *  空/错态统一走提示条（dma-rp-note + data-kind）；★ 2026-09-25 起清单走**序号**口径
         *  （`ordinal`：按记录时间编的"第几版"，最新在最上）＋ 默认只画最近 10 行 ＋ 每行一颗「看改动 ▸」。 */
        const floorBlock = e('div', { 'data-dma': 'floor-block', className: 'dma-rp-sec' },
          e('div', { className: 'dma-rp-sechead' },
            e('span', { className: 'dma-rp-sectitle' }, '楼层快照'),
            e('span', { className: 'dma-rp-secsub' }, '每轮模型改过笔记就记一份，绑在「楼层」（时间线的节点）上；检测到回档只在档顶弹提示，⛔ 不自动跟随 —— 写盘要你自己点'),
          ),
          floorPane.status === 'loading' ? e('div', { style: dimStyle }, '正在读楼层快照…') : null,
          floorPane.status === 'error' ? e('div', { className: 'dma-rp-note', 'data-kind': 'err' }, '楼层快照读不出来：' + floorPane.error) : null,
          floorData && floorData.docError
            ? e('div', { 'data-dma': 'floor-doc-error', className: 'dma-rp-note', 'data-kind': 'err', role: 'alert' },
                '⚠ 楼层快照清单（' + String(floorData.dir || '.dma-floor-snapshots') + '/index.json）读不出来（' + String(floorData.docError) + '）'
                + ' —— 这一档现在看不到楼层，回档提示（那条待处理）也读不出来。请到那份文件里看一眼。')
            : null,
          floorData && floorData.deadzonesReadable === false
            ? e('div', { 'data-dma': 'floor-deadzone-unknown', className: 'dma-rp-note', 'data-kind': 'err', role: 'alert' },
                '⚠ 死区数据读不出来 ⇒ 认不出哪几段是死区 ⇒「回到这一楼」那次不会动手（宁可不动，也不赌那几段不在死区里）。')
            : null,
          floorData && floorData.head && !floorRows.some((f) => f.current === true)
            ? e('div', { 'data-dma': 'floor-current-none', className: 'dma-rp-note', 'data-kind': 'dim' },
                // ★ 2026-09-25：这里的"当前在哪一条"也换成**序号**口径（认不出 ⇒ 「序号未知」，⛔ 不编 0）。
                '剧情现在站的这一楼（' + floorSeat(floorData.head.ordinal) + '）还没有快照，笔记保持原样。')
            : null,
          floorData && floorData.orderKnown === false
            ? e('div', { 'data-dma': 'floor-order-unknown', className: 'dma-rp-note', 'data-kind': 'dim' },
                '认不出当前这一楼在时间线里的位置 ⇒ 不判回档、不动笔记（不拿时间戳猜）。下面只列我们记过的楼层。')
            : null,
          floorData && floorData.sharedHint === true
            ? e('div', { style: dimStyle }, '（这一处在共用落点上：同一工作区里的别的周目也会看到同一份楼层快照。）')
            : null,
          floorPane.status === 'ready' && floorRows.length === 0 && !(floorData && floorData.docError)
            ? e('div', { 'data-dma': 'floor-empty', className: 'dma-rp-note', 'data-kind': 'dim' },
                '还没有楼层快照 —— 模型改过笔记之后（轮末）会自动记一份。')
            : null,
          // ★★ 2026-09-25：清单渲染的是**窗口**（默认 = 最近 10 行 ∪ {当前剧情那一行, 档案指针那一行}），
          //   中间被省掉的那几段各画**一行** `…（省略 N 条）`；点开「显示其余 N 条」⇒ 全量（顺序仍是宿主给的）。
          floorVisible.map((x, i) => (x.kind === 'gap'
            ? e('div', { key: 'floor-gap-' + String(i), 'data-dma': 'floor-omitted', style: dimStyle },
                '…（省略 ' + String(x.n) + ' 条）')
            : floorRow(x.f))),
          floorHidden > 0
            ? e('button', {
              id: 'dma-rpmem-floor-more', className: 'dma-btn', style: miniBtnStyle,
              'data-dma': 'floor-more',
              'data-floor-more-open': floorAll === true ? '1' : '0',
              title: floorAll === true
                ? '收起：回到默认只画最近的 10 条'
                : '把清单里其余 ' + String(floorHidden) + ' 条也画出来（最近 10 条一直都在上面）',
              onClick: () => setFloorAll(floorAll !== true),
            }, floorAll === true
              ? '收起其余 ' + String(floorHidden) + ' 条'
              : '显示其余 ' + String(floorHidden) + ' 条')
            : null,
          gotoLine,
          e('div', { style: dimStyle },
            '只记这 5 份（模型维护的那几份）：'
            + (floorData && Array.isArray(floorData.targets) && floorData.targets.length > 0
              ? floorData.targets.join('、')
              : 'notes.md、index.md、state.md、characters.md、world.md')
            + '；作者预置那几份回档永不自动改，死区那几段按块合并（保留你现在的样子）。「回到这一楼」只动笔记，不影响会话。'),
        )
        ...
        return e('div', { style: colFillStyle },
          ...
          gate === 'ready'
            ? e('div', Object.assign({}, scrollBind), e('div', { style: Object.assign({}, readColStyle, { width: 'min(1080px, 100%)' }) },
              // ★★ 2026-09-25 美化（用户拍板**方案 B：左右双栏**，治「界面太散漫」）：
              //   档顶横幅仍跨双栏悬在最顶（没有待处理时是 null，⛔ 不占位置）→ 下面分两栏：
              //   左栏 =「当前状态」+「楼层快照」两张分组卡；右栏 =「文件与阅读」占主位；
              //   窄屏由 .dma-rp-cols 的 flex-wrap 自动叠回单列。这一档的正文列放宽到 min(1080px, 100%)
              //   （双栏需要更宽的底；⛔ 没动共享常量 READ_COL_WIDTH，别的档不受影响）。
              //   ★ 只动容器与外观：所有 data-dma / data-floor* / id 锚、取数时机、hook 序（0-14）一个没动。
              // ★★ 2026-09-24：**档顶那条显著横幅**（回档改手动挡）—— 用户口径「前台弹提示」⇒ 排在最前面。
              floorPendingBanner,
              e('div', { className: 'dma-rp-cols' },
                // ── 左栏：当前状态 + 楼层快照 ────────────────────────────────
                e('div', { className: 'dma-rp-colL' },
                  e('div', { className: 'dma-rp-sec' },
                    e('div', { className: 'dma-rp-sechead' },
                      e('span', { className: 'dma-rp-sectitle' }, '当前状态'),
                      e('span', { className: 'dma-rp-secsub' }, '楼位 · 落点 · 死区')),
                    // ★★ 2026-09-24 收尾：那条**常驻状态行**（`剧情 第 38 楼 · 档案 第 34 楼`）—— 用户口径
                    //   「做**更显著的更改表示**」⇒ 一直看得见；两处不一样时它旁边那颗按钮＝就地登记（⛔ 不动正文）。
                    floorStateLine,
                    ...
                    // ── ★★ 2026-09-23 死区红字：**只告警、不还原**（用户明确否掉了"自动改回去"那个选项）──
                    //   文案**不定性**：用户自己也会在面板外改这些文件 ⇒ 一律写「可能是模型改的，也可能是你在别处改过」。
                    alert.status === 'loading' ? e('div', { style: dimStyle }, '正在看死区有没有被改…') : null,
                    alert.status === 'error' ? e('div', { className: 'dma-rp-note', 'data-kind': 'err' }, '死区现状读不出来：' + alert.error) : null,
                    alertData && alertData.docError
                      ? e('div', { 'data-dma': 'deadzone-doc-error', className: 'dma-rp-note', 'data-kind': 'err', role: 'alert' },
                          '⚠ 死区数据（' + String(alertData.file || '.dma-deadzones.json') + '）读不出来（' + String(alertData.docError) + '）'
                          + ' —— 作者设的禁改区这次没能生效（约束没喂给模型、也没在比对）。请到那份文件里看一眼。')
                      : null,
                    alertBad.length > 0
                      ? e('div', { 'data-dma': 'deadzone-alert', className: 'dma-rp-note', 'data-kind': 'err', role: 'alert' },
                          e('div', { style: { fontWeight: 700 } },
                            '⛔ 死区内容与快照不符（可能是模型改的，也可能是你在别处改过）—— 共 ' + String(alertBad.length) + ' 处：'),
                          alertBad.slice(0, 12).map((i, idx) => e('div', { key: 'dz-alert-' + idx, style: { fontSize: SMALL_FONT_SIZE, fontFamily: MONO, wordBreak: 'break-all' } }, '· ' + alertLine(i))),
                          alertBad.length > 12 ? e('div', { style: dimStyle }, '（还有 ' + String(alertBad.length - 12) + ' 处，见日志）') : null,
                          e('div', { style: dimStyle },
                            '本插件只告警、不会替你改回去 —— 文件始终是你的。要它不再红：'
                            + '自己确认后改回原样、解锁这一段，或在下面用「✎ 直接改」保存一次（保存会让快照跟着走）。'),
                        )
                      : null,
                    // 一致时只留一句**安静的**小字（⛔ 不红、不闪、不刷屏；没有死区时连这句都不出现）
                    alertData && alertData.docError === null && alertData.zones > 0 && alertBad.length === 0
                      ? e('div', { className: 'dma-rp-note', 'data-kind': 'dim', 'data-dma': 'deadzone-ok' },
                          '🔒 死区 ' + String(alertData.zones) + ' 段，与快照一致。'
                          + (alertData.lastWatch && alertData.lastWatch.at
                            ? '（模型每轮会比对一次，最近一次：' + new Date(alertData.lastWatch.at).toLocaleString() + '）'
                            : ''))
                      : null,
                    // 死区是**跟文件走**的：这一处在共用落点上 ⇒ 同一工作区里别的周目也会看到同样的死区（如实说）
                    alertData && alertData.zones > 0 && alertData.sharedHint === true
                      ? e('div', { style: dimStyle },
                          '（这些死区跟着共用那处目录走：同一工作区里的别的周目也会看到它 —— 想只给本局用，就把文件放进周目目录那份。）')
                      : null,
                  ),
                  // ★ 2026-09-23：「按楼层绑定的笔记快照」整块（★ 2026-09-25 起是左栏第二张卡「楼层快照」，
                  //   「⇄ 最近一次回档」也挂在这里）
                  floorBlock,
                ),
                // ── 右栏：文件与阅读（占主位）────────────────────────────────
                e('div', { className: 'dma-rp-colR' },
                  e('div', { className: 'dma-rp-sec' },
                    e('div', { className: 'dma-rp-sechead' },
                      e('span', { className: 'dma-rp-sectitle' }, '文件与阅读'),
                      e('span', { className: 'dma-rp-secsub' }, '点一行就地展开阅读')),
                    rows,
                  ),
                ),
              ),
      ...
      // 动什么：POST /vector/action —— `enable` 写宿主自己的 config；入库/重建写一张**请求单**；
      //   ★★ 2026-09-22（用户口径「是这个流程反直接，直接冻面板 60s，然后告诉我要下一轮？**改成直接删**」）：
      //   **两个删除动作当场就做完**（宿主平面自己改名留档 + 忘账本，不需要 anima 的引擎）⇒ 回执在
      //   本次响应里（deleted:true），面板立刻显示结果、按钮立刻可用、⛔ 不起轮询。
      //   「立即入库 / 重建」仍只能排队（引擎在 anima 的 apply() 闭包里，宿主拿不到）—— 但**不再冻面板**：
      //   提交后立刻显示"已排队"并放开按钮，回执改成**后台低频跟踪**（留意「最近一次动作」那一行）。
      // ★ 时机口径（用户 2026-09-20）：「向量生成的时机需要明确为自动压缩入库或工具检索调用时」
      //   —— 本档常驻不生成任何向量，只在注入时/检索时由 anima 触发；「立即入库」是手工补一脚。
      // ⛔ 空闲时零定时器：只有提交了"排队类"动作才有那个后台跟踪，回执对上/放手就停。
      const VECTOR_TRACK_MS = 10000            // 排队类动作：后台每 10s 看一眼回执（**低频**，⛔ 不卡界面）
      const VECTOR_TRACK_LIMIT_MS = 30 * 60000 // 跟踪 30 分钟就放手（回执可能几小时后才来 ⇒ 不许无限转）
      ...
      const VECTOR_ARMED = {
        ...
        // ★ 删除是**点了就删**（宿主当次做完）：这两个动作不排队、不用等下一轮 —— 说明也照这个写。
        //   （本档的用户可见文案不用 `**` 那种强调写法：面板是按纯文本渲染的，星号会原样显示。）
        'delete-vector': { action: 'delete-vector', label: '删除向量库', hint: '删掉整个向量库：当场生效（先改名留档，不彻底销毁；下次入库会重新长出来）；删完检索只剩 BM25。' },
        'delete-bm25': { action: 'delete-bm25', label: '删除 BM25 库', hint: '删掉整个 BM25 库：当场生效（先改名留档，不彻底销毁；下次入库会重新长出来）；删完检索只剩向量。' },
      }
      ...
      /** 读侧最近一次结果的人话（T1）：成功带耗时/上次成功时间；失败带故障码与"那一轮注入的是故障说明"。 */
      function retrievalDetailText(ret) {
        if (!ret || typeof ret !== 'object') return '读不到（还没有快照：进过一轮对话才有）'
        const at = Number.isFinite(ret.lastOkAt) ? ' · 上次成功 ' + new Date(ret.lastOkAt).toLocaleString() : ''
        const dim = Array.isArray(ret.dimMismatch) && ret.dimMismatch.length > 0
          ? ' · ⚠维度对不上：' + ret.dimMismatch.slice(0, 2).join('；')
          : ''
        if (ret.ok === true) {
          const ms = Number.isFinite(ret.ms) ? '（' + Math.round(ret.ms) + 'ms）' : ''
          return '最近一次成功' + ms + at + dim
        }
        const f = ret.failure && typeof ret.failure === 'object' ? ret.failure : null
        const label = f && typeof f.code === 'string' ? f.code : '未知故障'
        const detail = f && typeof f.detail === 'string' && f.detail !== '' ? '：' + f.detail.slice(0, 120) : ''
        const when = f && Number.isFinite(f.at) ? ' · ' + new Date(f.at).toLocaleString() : ''
        return '❌ 最近一次**没跑成**（' + label + '）' + detail + when + at + dim
          + ' —— 那一轮注入的是故障说明，不是记忆'
      }
      ...
      function VectorFlow({ archivePath, playthroughId, scrollBind }) {
        const [snap, setSnap] = React.useState({ status: 'loading', info: null, result: null, live: null, entries: null, staleMs: null, deletedAfterInfo: false, error: '' })
        ...
        const [track, setTrack] = React.useState('')     // 后台跟踪中的排队动作 id（'' = 没在跟踪；⛔ 不占用 busy）
        ...
        const applySnap = (d) => {
          setSnap({
            ...
            // ★ 快照是不是"删除之前"的（宿主判的：最近一张回执是删除、且比快照新）—— 面板据此如实标过期
            deletedAfterInfo: !!(d && d.deletedAfterInfo === true),
            error: '',
          })
        }
        const load = () => requestJson(HOST_API_BASE + '/vector/state')
          ...
          .catch((error) => {
            if (alive.current) setSnap({ status: 'error', info: null, result: null, live: null, entries: null, staleMs: null, deletedAfterInfo: false, error: errText(error) })
          })

        /**
         * 排队类动作的**后台低频跟踪**（⛔ 与"冻面板 60 秒"是两回事）：
         * 提交后按钮立刻可用、面板不转圈；这里只是每 10s 悄悄看一眼回执（panel-result.json），
         * 对上 id 就把结果写到横幅与「最近一次动作」；30 分钟还没轮到就放手并如实说一句（不再转）。
         */
            ...
            try {
              ...
              if (r && String(r.id || '') === id) {
                ...
                setOut({
                  ...
                  message: (r.ok === true ? '执行成功' : '执行失败')
                    + (typeof r.message === 'string' && r.message !== '' ? '：' + r.message : ''),
                })
                return
              }
            } catch (error) {
              // 单次取数失败不停跟踪（宿主可能正好在重启）；⛔ 也不弹错——它只是后台那一脚
            }
            if (Date.now() - started >= VECTOR_TRACK_LIMIT_MS) {
              setTrack('')
              setOut({ status: 'queued', message: '已排队，但跟踪到这里就停了（还没轮到执行）。结果会记在下面「最近一次动作」里，回来看即可。' })
              return
            }
            pollTimer.current = setTimeout(tick, VECTOR_TRACK_MS)
          }
          pollTimer.current = setTimeout(tick, VECTOR_TRACK_MS)
        }
        ...
        const submit = (action) => {
          if (busy !== '') return
          // ★ 两个删除动作 = 宿主当场做完（不需要 anima 的引擎）⇒ 回执在本次响应里，⛔ 不起跟踪。
          const isDelete = action === 'delete-vector' || action === 'delete-bm25'
          setBusy(action)
          setOut({ status: 'waiting', message: '正在提交…' })
          ;(async () => {
            try {
              // 删除要把"是哪个周目"带上（宿主据此自己去读那份归档的摘要清单 ⇒ 忘账本只忘本周目的）。
              // ⛔ 面板递过去的只有周目 id，⛔ 绝不递文件名清单（那是宿主的知识）。
              const payload = isDelete ? { action: action, playthroughId: playthroughId || '' } : { action: action }
              const d = await apiPost(HOST_API_BASE + '/vector/action', undefined, payload)
              if (!alive.current) return
              if (isDelete) {
                // 当次就有回执（deleted:true）⇒ 直接显示结果，按钮立刻恢复；再拉一次状态把"库不在"这件事实摆出来
                const msg = String((d && d.message) || '')
                setBusy('')
                setOut({
                  status: d && d.ok === true ? 'ok' : 'fail',
                  message: (d && d.ok === true ? '已删除' : '删除失败') + (msg !== '' ? '：' + msg : '')
                    + (d && typeof d.receiptNote === 'string' && d.receiptNote !== '' ? '（' + d.receiptNote + '）' : ''),
                })
                await load()
                return
              }
              const id = String((d && d.id) || '')
              // 排队那条路：**立刻**放开面板（按钮可用、不转圈），回执靠后台跟踪
              setBusy('')
              setOut({
                status: 'queued',
                message: String((d && d.hint) || '已排队：会在该周目会话的下一轮开始前执行')
                  + '——不用在这儿等，按钮可以继续用；结果会记在下面「最近一次动作」。',
              })
              await load()
              if (!alive.current) return
              if (id === '') {
                // 宿主 200 但没带回执 id（不该发生，比如模块降级）⇒ 空 id 永远对不上，跟踪也是白转，
                // 如实停下说清（⛔ 不许一直转）。
              ...
              if (!alive.current) return
              setBusy('')
              setOut({ status: 'fail', message: errText(error) })
              // 删除失败（归档失败那种）宿主也会留一张回执 ⇒ 顺手刷新一次，让「最近一次动作」说实话
              if (isDelete) await load()
            }
          })()
        }
        ...
        // ★ 2026-09-26（T1）：anima 写的"读侧最近一次到底跑成没有"（`live.retrieval`）；
        //   故障时注入那一格有一句如实说明，面板这一行也要能看见同一件事。
        const ret = live.retrieval && typeof live.retrieval === 'object' ? live.retrieval : null
        ...
        // ★★ 2026-09-22（本单硬约束「删完不许有两个真相」）：删除是宿主**当场**做的，而上面那份快照是
        //   anima 写的、要等它下一脚才刷新 ⇒ 刚删完这一小段里，快照里的条数与归属统计都是**删之前**的。
        //   宿主判好了（deletedAfterInfo：最近一张回执是删除且比快照新）⇒ 这里如实标过期，⛔ 不拿旧条数骗人。
        const staleAfterDelete = snap.deletedAfterInfo === true
        ...
        healthRows.push(kvRow('库存',
          vec === null
            ? '读不到'
            : ((vec.exists === false ? '库不在（文件已被归档；下次入库会重新长出来）' : (vec.count === null ? '条数读不到' : '共 ' + fmtChars(vec.count) + ' 条'))
              + (staleAfterDelete
                // 删之后的快照还没来 ⇒ 归属统计是删之前的，⛔ 不许当成现状摆出来
                ? ' · 归属统计是删除之前的（快照还没刷新）'
        ...
        // ★ 刚删过（宿主当次做完）⇒ 上面那份快照是"删之前"的：如实说清，⛔ 不让人拿旧条数当现状
        if (staleAfterDelete) {
          healthRows.push(e('div', { key: 'staledel', id: 'dma-vector-stale-after-delete', style: { fontSize: SMALL_FONT_SIZE, color: 'darkorange' } },
            '刚刚删过一次（' + (snap.result && snap.result.message ? String(snap.result.message) : '见下面「最近一次动作」')
            + '）：这份状态快照是删除之前的，条数与归属统计都过期了 —— 库的现状以上面「库存」行的现算结果与下面「库」那一行为准。',
          ))
        }
        ...
        const outStyle = out.status === 'ok' ? okStyle : (out.status === 'fail' ? errorStyle : dimStyle)
        ...
        return e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
          ...
          e('div', { id: 'dma-vector-collection', style: cardStyle },
            ...
            // ★ 2026-09-26（T1）：**读侧最近一次跑成没有** —— 改前检索失败只写一行控制台日志，
            //   用户是从"翻 system 全文发现 anima 那格是空的"才发现的。现在面板直接说。
            e('div', { id: 'dma-vector-retrieval', style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', padding: '4px 0', borderTop: '1px solid color-mix(in srgb, CanvasText 8%, transparent)' } },
              e('span', { style: { fontSize: SMALL_FONT_SIZE, opacity: 0.75, width: 52, flex: 'none' } }, '最近检索'),
              e('span', { style: dimStyle }, retrievalDetailText(ret)),
            ),
          ),
          entriesCard,
          armedDoc
          ...
          // 排队类动作的后台跟踪（⛔ 不占 busy：按钮这一栏永远是能点的）
          track !== '' ? e('div', { id: 'dma-vector-tracking', style: dimStyle },
            '后台跟踪中：这一脚要等该周目会话的下一轮开始前才会跑；不用在这儿等，回执来了这一栏会自动更新。') : null,
          e('div', { style: dimStyle },
            '状态和条目都来自向量插件（dsh-anima-rag）落盘的文件。「删除」是当场生效的（改名留档，不真删；'
            + '账本里那些摘要会被忘掉，下次入库会重新长出来），「立即入库 / 重建」要等该周目的下一轮对话开始前由它执行。'),
        ))
      }
      ...
      // =====================================================================
      // 「压缩」档（2026-09-21）：自动压缩触发阈值的**控制面板**
      //
      // 为什么放在这个面板里（⛔ 不是设置视图）：它要读**本会话**的实时占用 —— 设置视图没有会话上下文
      // （那里是 LastFloorsCard 那种纯配置卡的地盘）；本面板已经知道"当前是哪个会话"（复用同一个
      // sessions 快照，⛔ 不新开第二条会话查询）。
      //
      // 两条硬纪律：
      //   ① ⛔ 客户端**不自己算百分比**、⛔ 不直连任何官方内部接口 —— 一律读宿主的 GET /compaction/state
      //      （那里面两个数都由宿主按官方公式算好：percent = DSH 那个 / triggerPercent = 真正判定的那个）；
      //   ② 读不到就显示「读不到（原因）」**红字**，⛔ 绝不用 0% 冒充。
      // =====================================================================

      /** 轮询间隔（3–5 秒区间：够"看着它涨"，又不会把宿主问烦）。 */
      const COMPACT_POLL_MS = 4000
      /** 滑块/数字框的范围与步长 —— 与宿主 lib/compaction-threshold.js 的常量同值（那边是唯一判定处）。 */
      const COMPACT_MIN_PERCENT = 5
      const COMPACT_MAX_PERCENT = 90
      const COMPACT_STEP_PERCENT = 5

      /** 千位分隔的人话数（`128000` → `128k`）；null ⇒ '—'（⛔ 不显示 0）。 */
      function compactTokenText(n) {
        if (!Number.isFinite(n)) return '—'
        if (n >= 1000) return Math.round(n / 1000) + 'k'
        return String(n)
      }

      /** 「当前 %」那一行（读不到 ⇒ null，由调用方显示红字原因）。 */
      function compactPercentText(percent) {
        return Number.isFinite(percent) ? String(percent) + '%' : null
      }

      function CompactFlow({ sessionId, scrollBind }) {
        const [snap, setSnap] = React.useState({ status: 'loading', data: null, error: '' })
        const [cfg, setCfg] = React.useState(null)          // GET /config 的 autoCompact（开关初值）
        const [usePanel, setUsePanel] = React.useState(true)
        ...
        const load = () => {
          const url = HOST_API_BASE + '/compaction/state' + (sessionId ? '?sessionId=' + encodeURIComponent(sessionId) : '')
          return requestJson(url)
            .then((d) => { if (alive.current) setSnap({ status: 'ready', data: d, error: '' }) })
            .catch((error) => {
              if (alive.current) setSnap({ status: 'error', data: null, error: errText(error) })
            })
        }

        // 面板关掉就停轮询（任务书口径）；换会话（sessionId 变）重新起一轮。
        React.useEffect(() => {
          alive.current = true
          void load()
          const tick = () => {
            timer.current = setTimeout(() => {
              if (!alive.current) return
              void load().then(() => { if (alive.current) tick() })
            }, COMPACT_POLL_MS)
          }
          tick()
          return () => {
        ...
        // 开关与滑块的初值走既有的 /config 投影（⛔ 不为它开新端点）。
        React.useEffect(() => {
          const c = new AbortController()
          requestJson(HOST_API_BASE + '/config', c.signal)
            .then((d) => {
              if (c.signal.aborted) return
              const ac = d && d.autoCompact ? d.autoCompact : null
              if (ac === null) return
              setCfg(ac)
              setUsePanel(ac.usePanelThreshold !== false)
              setPercent(String(ac.thresholdPercent === undefined || ac.thresholdPercent === null ? 15 : ac.thresholdPercent))
            })
            .catch(() => { /* 读不到配置就按默认显示（开关默认开、15%），保存时宿主会给真值 */ })
          return () => c.abort()
        }, [])

        React.useEffect(() => () => { if (ctl.current) ctl.current.abort() }, [])

        /** 控件互相同步：滑块 → 数字框 / 数字框 → 滑块（同一份 state，两个控件都读它） */
        const onPercent = (raw) => {
          const n = Number(raw)
          if (!Number.isFinite(n)) { setPercent(raw); return }
          const clamped = Math.min(COMPACT_MAX_PERCENT, Math.max(COMPACT_MIN_PERCENT, Math.round(n / COMPACT_STEP_PERCENT) * COMPACT_STEP_PERCENT))
        ...
        const onSave = () => {
          ...
          setSave({ busy: true, ok: null, text: '正在保存…', detail: null })
          const body = { usePanelThreshold: usePanel }
          const n = Number(percent)
          if (Number.isFinite(n)) body.thresholdPercent = n
          mutateJson(HOST_API_BASE + '/compaction/config', 'POST', body, controller.signal)
            .then((d) => {
              if (controller.signal.aborted) return
              setSave({ busy: false, ok: true, text: '已保存', detail: d })
              void load()
            })
            .catch((error) => {
              if (controller.signal.aborted) return
              // 宿主"两件事各自如实"：整体 ok:false 时把两半的原因都摊开显示（err.payload 里就有）
              const payload = error && error.payload ? error.payload : null
              setSave({ busy: false, ok: false, text: errText(error), detail: payload })
              void load()
            })
        }

        const data = snap.data
        const readError = data && typeof data.readError === 'string' && data.readError !== '' ? data.readError : ''
        const percentText = data ? compactPercentText(data.percent) : null
        const triggerText = data ? compactPercentText(data.triggerPercent) : null
        const notes = data && Array.isArray(data.notes) ? data.notes : []
        const presetRatio = data ? data.presetThresholdRatio : null
        const yamlPart = save.detail && save.detail.yaml ? save.detail.yaml : null
        const configPart = save.detail && save.detail.config ? save.detail.config : null

        return e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
          e('div', { style: { fontSize: 12, fontWeight: 700 } }, '自动压缩触发阈值'),
          e('div', { style: dimStyle },
            '到多少 % 就让官方压缩把前面的历史收成 checkpoint。以 **DSH 自带的那个上下文百分比**为单位。'),

          // ---------- 实时读数（每 4 秒现读；面板关掉就停） ----------
          e('div', { style: cardStyle },
            e('div', { style: { display: 'flex', gap: 10, alignItems: 'baseline', flexWrap: 'wrap' } },
              e('span', { style: { fontSize: 12, opacity: 0.75 } }, '当前'),
              percentText !== null
                ? e('span', { style: { fontSize: 20, fontWeight: 700 } }, percentText)
                : e('span', { style: errorStyle }, '读不到' + (readError !== '' ? '（' + readError + '）' : '')),
            ),
            e('div', { style: { display: 'flex', gap: 10, alignItems: 'baseline', flexWrap: 'wrap' } },
              e('span', { style: { fontSize: 12, opacity: 0.75 } }, '触发判定'),
              triggerText !== null
                ? e('span', { style: { fontSize: 13, fontWeight: 700 } }, triggerText + '（含输出 token）')
                : e('span', { style: errorStyle }, '读不到' + (readError !== '' ? '（' + readError + '）' : '')),
            ),
            e('div', { style: dimStyle },
              '窗口 ' + compactTokenText(data ? data.contextWindow : null)
              + ' · 预设阈值 ' + (Number.isFinite(presetRatio) ? Math.round(presetRatio * 100) + '%' : '读不到')
              + (data && Number.isFinite(data.thresholdTokens) ? ' · 阈值 ' + compactTokenText(data.thresholdTokens) + ' token' : ''),
            ),
            snap.status === 'loading' && e('div', { style: dimStyle }, '正在读宿主…'),
            snap.status === 'error' && e('div', { style: errorStyle }, '读不到实时数：' + snap.error),
            !sessionId && e('div', { style: errorStyle }, '还不知道本会话是哪一个 ⇒ 读不到实时占用（面板里选/打开一个会话后再看）。'),
            data && !data.enabled && e('div', { style: dimStyle }, '当前**没有**用面板阈值 ⇒ 走预设 YAML 里的原值。'),
          ),
          ...
          e('div', { style: cardStyle },
            ...
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', fontSize: 12 } },
              e('input', {
                id: 'dma-compact-range', type: 'range',
                min: COMPACT_MIN_PERCENT, max: COMPACT_MAX_PERCENT, step: COMPACT_STEP_PERCENT,
                'aria-label': '自动压缩触发阈值（百分比）',
                value: Number.isFinite(Number(percent)) ? String(Number(percent)) : '15',
                disabled: save.busy, style: { flex: '1 1 180px', minWidth: 140 },
                onChange: (ev) => onPercent(ev.target.value),
              }),
              e('input', {
                id: 'dma-compact-percent', type: 'number',
                min: COMPACT_MIN_PERCENT, max: COMPACT_MAX_PERCENT, step: COMPACT_STEP_PERCENT,
                'aria-label': '自动压缩触发阈值（数字）',
                value: percent, disabled: save.busy, style: { width: 70, fontSize: 12 },
                onChange: (ev) => onPercent(ev.target.value),
              }),
              e('span', { style: dimStyle }, '%'),
            ...
            // 两件事各自如实：一半失败时把两半摊开（⛔ 不许一个失败另一个显示成成功）
            (configPart !== null || yamlPart !== null) && e('div', { style: { fontSize: 12 } },
              e('div', null, 'config.json：' + (configPart && configPart.ok === true
                ? ('已写入（阈值 ' + String(configPart.thresholdPercent) + '%）' + (configPart.reason ? ' · ' + configPart.reason : ''))
                : ('没写成' + (configPart && configPart.reason ? '：' + configPart.reason : '')))),
              e('div', null, '预设 YAML：' + (yamlPart && yamlPart.ok === true
                ? (yamlPart.changed === false
                  ? '本来就是这个数（未改动）'
                  : ('已写入 ' + String(yamlPart.path || '') + '（' + String(yamlPart.before) + ' → ' + String(yamlPart.after) + '），备份 ' + String(yamlPart.backupPath || '无')))
                : ('没改成' + (yamlPart && yamlPart.reason ? '：' + yamlPart.reason : '')))),
            ),
          ),

          // ---------- 如实小字（宿主给的就是逐字这些） ----------
          e('div', { style: cardStyle },
            e('div', { style: sectionTitleStyle }, '如实说明'),
            notes.length === 0 && e('div', { style: dimStyle }, '（宿主没给说明 —— 正常不会发生）'),
            notes.map((line, i) => e('div', { key: 'dma-compact-note-' + i, style: { fontSize: 12, lineHeight: 1.6 } }, line)),
          ),
        ))
      }
      ...
      function computeSources(mode, tavernOk, eventsSessionId) {
        if (mode === 'session') {
          // ★ 2026-09-21：「压缩」档在**会话模式也有** —— 它读的是"本会话"的实时占用（与
          //   workspace 那几档不同，不依赖 Tavern 归档），会话模式照样有实时数可看、照样能改阈值。
          return eventsSessionId ? [['events', '会话事件'], ['compact', '压缩']] : []
        }
        if (!tavernOk) {
        ...
        // ★ 2026-09-21：「压缩」档（自动压缩触发阈值控制面板）—— 同样与它们同层，排最后。
        // ★ 2026-09-25 美化单（用户拍板）：页签名「剧情大纲」→「RP 记忆」—— 这一档实际是角色扮演预设
        //   维护的记忆库（5 份笔记 + 楼层快照），旧名与内容不符（任务书 §6.1）；台子断言已同步改。
        return [['summaries', '摘要'], ['floors', '原文'], ['outline', 'RP 记忆'], ['vector', '向量'], ['compact', '压缩']]
      }
      ...
      function ReadArea({ mode, tavernOk, healthStatus, archivePath, playthroughId, eventsSessionId, currentSessionId, nameIdx, catalogError, onClose, fullscreen, setFullscreen }) {
        ...
        if (cur === 'summaries') {
          ...
          //   真拥有向量库的是 dsh-anima-rag（按会话挂载）⇒ 「立即入库 / 重建」这一档只「读状态 / 写请求单」，
          //   动作由 anima 在该周目会话的下一轮开始前执行（实现全在宿主侧 lib/vector-panel.js）。
          //   ★★ 2026-09-22：「删除」不再走这条路 —— 宿主平面**当场做完**（改名留档 + 忘账本）。
          //   archivePath 传进去只为把条目和「摘要」页签按文件名对上（同一份读法，⛔ 不另造）；
          //   playthroughId 只为删除时告诉宿主"该忘哪个周目的账本"（⛔ 不递文件名清单）。
          content = e(VectorFlow, { archivePath: archivePath, playthroughId: playthroughId || '', scrollBind: scrollBind })
        } else if (cur === 'compact') {
          // ★ 压缩档（2026-09-21）：本会话的实时占用 + 自动压缩阈值控制面板。
          //   sessionId 用 `currentSessionId ?? eventsSessionId`：前者是**当前活跃会话**（面板已经在问
          //   /playthrough/for-session 时拿到的那个 id，⛔ 不新开第二次会话查询），后者是根会话
          //   （会话模式的兜底）。两个都没有 ⇒ 组件如实显示"读不到"，⛔ 不显示 0%。
          content = e(CompactFlow, { sessionId: currentSessionId || eventsSessionId || '', scrollBind: scrollBind })
        } else if (cur === 'events') {
      ...
      //      rp:policy 45 · state:card 50 · （54 空位：dma:echo 2026-09-26 退役） · anima:memory 55 · rp:storyAnchor 56
      ...
      const PM_SECTION_NOTES = {
        ...
        // ★ 2026-09-26（T3）：`dma:echo` 已退役 —— 段本身不再注册。这里留一条说明，是因为
        //   段表要能解释**历史会话**里那一段（老 prompt 抓包里还有它），⛔ 不是"这个段还在"。
        'dma:echo': '【已退役 · 2026-09-26 摘除】原为本地的 FTS5 关键词回响（归档原文的 3-gram 命中）；'
          + '现在那一格（<memoryEcho>）由 dsh-anima-rag 的**语义命中**装（与 anima:memory 同一批）。',
      ...
      const PM_SECTION_OWNERS_EXACT = {
        ...
        'dma:echo': '【已退役 · 2026-09-26】本插件（dsh-memory-archive · 记忆回响 D2）注册的；接力者：dsh-anima-rag',
      ...
      // ---------- 设置视图：记忆回响（★ 2026-09-26 起：已由 anima 接管，本卡只作**退役说明**） ----------
      //
      // 为什么把开关删掉而不是留着：那一格的引擎（FTS5 trigram 关键词命中，`lib/echo-index.js`
      // / `lib/echo-inject.js`）**整块退役**了 —— 现在装的是 `dsh-anima-rag` 每轮的**语义命中**，
      // 预算在它那边配（`inject.echo`：topK / maxChars / perHitChars）。
      // ⛔ 不许把这个卡删掉当"清理干净"：用户口径是「摘除要如实体检」——
      //    面板必须能说出"谁接管了、去哪调"，静默消失正是要避免的那件事。
      function EchoCard({ config }) {
        const [detailOpen, setDetailOpen] = React.useState(false)
        const saved = (config && config.echo && typeof config.echo === 'object') ? config.echo : {}
        const retired = saved.retired === true
        const detailLink = { border: 'none', background: 'none', color: 'inherit', textDecoration: 'underline', cursor: 'pointer', padding: 0 }
        return e('div', { style: cardStyle, 'data-echo-card': '1', 'data-echo-retired': retired ? '1' : '0' },
          e('div', { style: { display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'space-between' } },
            e('div', { style: sectionTitleStyle }, '记忆回响（已由 anima 接管）'),
            e('span', { style: { fontSize: 12, flex: 'none', opacity: 0.8 } }, retired ? '已退役 · 2026-09-26' : '已退役'),
          ),
          e('div', { style: dimStyle },
            '原先这一格（<memoryEcho>）装的是本地 FTS5 关键词命中；2026-09-26 起改装 dsh-anima-rag 每轮的语义命中，'
            + '仍然是每轮装配时自动算（无需开关）。'),
          e('div', { style: { textAlign: 'right' } },
            e('button', { style: detailLink, onClick: () => setDetailOpen((v) => !v) }, detailOpen ? '【详细】▴' : '【详细】▾')),
          detailOpen ? e('div', { id: 'dma-echo-retired-detail', style: { display: 'flex', flexDirection: 'column', gap: 4, paddingLeft: 10, borderLeft: '2px solid rgba(128,128,128,0.35)' } },
            e('div', { style: { fontSize: 12 } }, '摘掉了什么：FTS5 索引库（echo-index.db）、建表/入库/抽候选/df 筛/查询、本卡原来的开关与预算（echo.enabled / topK / maxChars / minDf / maxDf）。'),
            e('div', { style: { fontSize: 12 } }, '现在由谁算：dsh-anima-rag 的向量检索（同一份 merged 命中，取前 topK 条）。'),
            e('div', { style: { fontSize: 12 } }, '去哪调预算：dsh-anima-rag 的 inject.echo（enabled / topK / maxChars / perHitChars）。'),
            e('div', { style: { fontSize: 12 } }, '失败怎么办：读侧嵌入失败/超时时，anima 会在注入那一格如实写一句故障说明（详见「向量」页签的「最近检索」行）。'),
            e('div', { style: dimStyle }, '旧配置里的 echo 段会被如实忽略；旧索引文件 echo-index.db 没有删，确认不需要了自行删除即可。'),
          ) : null,
        )
      }
      ...
      function ArchivePanel({ onClose, sessions }) {
        ...
        //   ★ 2026-09-21：「压缩」档要看**本会话**的实时占用 ⇒ 顺手把这次的 sessionId 一起记进来
        //     （同一次查询的副产物，⛔ 不为它再问一遍宿主）。
        const [sessPt, setSessPt] = React.useState({ status: 'loading', playthroughId: '', source: '', sessionId: '' })
        React.useEffect(() => {
          ...
          const sync = async () => {
            ...
            try {
              ...
              // ★ sessionId 一并记下（「压缩」档要用；未归入的会话也记 —— "认不出周目"与"知道是哪个会话"是两件事）
              setSessPt({ status: 'ready', playthroughId: mapped ? play0 : '', source: mapped ? 'session' : 'none', sessionId: sid })
              if (!mapped) return
              const char = char0
              const play = play0
              const key = char + '/' + play
            ...
            e('div', { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' } },
              view === 'read'
                ? e(ReadArea, {
                  ...
                  // ★ 2026-09-22：向量档的「删除」要把"是哪个周目"递给宿主（宿主据此自己去读那份归档的
                  //   摘要清单 ⇒ 忘账本只忘本周目的）。⛔ 递的是 id，不是文件名清单、也不是路径。
                  playthroughId: wsPlay || '',
                  // 「压缩」档要的**本会话** id：来自上面那次 /playthrough/for-session（⛔ 不新开查询）。
                  // 会话模式下没有那次查询 ⇒ 退到根会话（eventsSessionId），两个都没有时组件如实说读不到。
                  currentSessionId: sessPt.sessionId || '',
      ...
      function apply(ctx) {
        ...
        // ★★ 2026-09-24（「回档 → 立刻弹」那一单）：这一个组件是**常驻挂载**的那一个 —— 侧边栏那颗按钮
        //   一直在（与只在打开时才挂的 `ArchivePanel` 不同）⇒ 宿主那条 SSE 长连与**顶部浮层**都挂在这里，
        //   面板关着也弹得出来（回档时你多半没开着面板 —— 那正是这一条的意义）。
        ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
          { name: 'sidebar.footer.action', id: 'memory-archive' },
          function MemoryArchiveButton(props) {
            const [open, setOpen] = React.useState(false)
            // ★★ 2026-09-24：**顶部浮层**那条待处理（宿主推来的那一帧；没有 ⇒ `null` ⇒ 什么都不渲染）。
            //   ⚠️ 新 hook 一律**追加在最后**（自检台按 hook 序号注入预设值，⛔ 不许把已有序号挪位）。
            const [floorPopup, setFloorPopup] = React.useState(null)
            // ★★ 2026-09-25（用户口径「**不要楼号了，基本没用，只标序号**」）：同一帧里那份
            //   `pendingOrdinals`（`{from,to,archive}`）—— 浮层标题显示的「序号 N」就是它（宿主算的；⛔ 不编 0）。
            //   ⚠️ 单独一格是因为**那条 `pending` 本身一个字都不许动**（它与盘上那条逐字一致是硬判据）。
            const [floorPopupOrd, setFloorPopupOrd] = React.useState(null)
            // ★★ 订阅宿主那条 SSE（`GET …/floors/events`）—— 「回档 → 立刻弹」那条**唯一**的通道。
            //   · 这是本插件**唯一**允许的长连：⛔ 仍然不挂定时器、⛔ 不轮询 —— 断开重连由 `EventSource`
            //     自己管，重连之后第一帧是 `hello`（带**现状**）⇒ 自然对齐（有就弹、没有就清掉）；
            //   · 帧只认 `type`：`fork` ⇒ 记下那条 pending、弹出浮层；不认的 `type` ⇒ **安静忽略**
            //     （前向兼容：宿主以后加帧类型，旧客户端不会出洋相）；
            //   · 清理路径：这个组件是常驻的（侧边栏那颗按钮），**真机上没有卸载路径**；
            //     但清理函数照样挂上 `close()` —— 万一哪天真有机会卸载（换壳 / 热重载），⛔ 不留野长连。
            React.useEffect(() => {
              ensureDmaStyles()
              // 老浏览器 / 非浏览器环境没有 EventSource ⇒ **安静降级**（面板那半边照旧能用，只是没有"立刻弹"）
              if (typeof EventSource !== 'function') return undefined
              let alive = true
              let es = null
              try { es = new EventSource(HOST_API_BASE + '/playthrough/rp-memory/floors/events') } catch (error) { return undefined }
              const onFrame = (ev) => {
                let frame = null
                try { frame = JSON.parse(ev && ev.data ? ev.data : '') } catch (error) { return }  // 认不出的帧 ⇒ 安静忽略
                if (!frame || typeof frame !== 'object') return
                const type = typeof frame.type === 'string' ? frame.type : ''
                if (type !== 'fork' && type !== 'hello') return
                if (!alive) return
                // `hello` = 连上 / 重连之后的第一帧（带**现状**）；`fork` = 判到**开 fork** 那一刻推的。
                // 两者都只做一件事：把"该不该挂着那条浮层"对齐成宿主给的那条 pending（没有就清掉）。
                setFloorPopup(frame.pending && typeof frame.pending === 'object' ? frame.pending : null)
                // ★ 2026-09-25：同一帧里还带着那三个**序号**（浮层标题显示的「序号 N」读它；⛔ 不编 0）。
                setFloorPopupOrd(frame.pendingOrdinals && typeof frame.pendingOrdinals === 'object' ? frame.pendingOrdinals : null)
              }
              es.onmessage = onFrame
              return () => {
                alive = false
                try { if (es && typeof es.close === 'function') es.close() } catch (error) { /* 关不掉也只能算了 */ }
              }
            }, [])
            return e('div', { style: { position: 'relative' } },
              e('button', {
              ...
              // ★★ 2026-09-24：**顶部那条浮层**（回档「立刻弹」）—— ⛔ 没有那条待处理时**什么都不渲染**
              //   （不占位置、不渲染空壳）。判据（`pending`）全是宿主给的，⛔ 客户端一个字的判断都不做。
              floorPopup === null ? null : e(FloorPopup, { pending: floorPopup, ordinals: floorPopupOrd, onClose: () => setFloorPopup(null) }),
      ...
      exports.__promptMap = {
        ...
        PM_SECTION_OWNERS_EXACT: PM_SECTION_OWNERS_EXACT,   // 精确名覆盖（rp:storyAnchor/rp:firstRound/dma:echo（退役留名）/state:card）
