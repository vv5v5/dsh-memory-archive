    ...
    return (() => {
      ...
      async function apiPost(url, signal, payload) {
        ...
        if (!res.ok || !data || data.ok !== true) {
          const err = new Error((data && (data.error || (data.error && data.error.message))) || ('HTTP ' + res.status))
      ...
      // ④ 人话注释表（任务书 B/§3：段名用运行时真名；表里没有的段注释位写「未收录」，⛔ 不许编一句）。
      //    每一条都在本仓库里有出处（rp-agent.js / 既有 NOTE_* 常量），不是当场发明的。
      const PM_SECTION_NOTES = {
        'harness:identity': 'DSH 核心身份段：告诉模型它在 DSH 里、检出目录在哪与行为纪律（恒定注入，永远最前）',
        'deployment:persona': '部署级人设段：RP 身份（它以为自己是干什么的）落最前（rp-agent 生成时强校验的槽位）',
        'roleplay:policy': 'RP 模式策略段：默认只说明高风险操作被锁，不是扮演身份；身份与文风来自 preset / 角色卡',
        'ui:deliverable-file-references': '可交付文件引用说明（system 尾部固定段）',
        'context:file-reference': '可交付文件引用的上下文注入（每轮求值，本楼为空即不带内容）',
      }
      function pmSectionNote(name) {
        const hit = PM_SECTION_NOTES[String(name || '')]
      ...
      const PM_SECTION_OWNERS = [
        ...
        ['rp:', 'pmp-dsh-tavern（RP 策略段）'],
        ['state:', 'dsh-state-bridge'],
        ['anima:', 'dsh-anima-rag'],
      ...
      function pmDetectedBlock(def, text, judged) {
        const b = {
          ...
          label: def.label, order: def.order, orderNum: def.orderNum, color: def.color, dashed: false,
      ...
      function buildMapFromSections(c4, opts) {
        ...
        for (const s of sections) {
          ...
          const b = {
            ...
            color: mut === 'per-turn' ? 'yellow' : mut === 'static' ? 'red' : 'gray',
      ...
      exports.__promptMap = {
        ...
        pmSectionOwner: pmSectionOwner,       // ★ 归属按段名前缀认（2026-09-14）
