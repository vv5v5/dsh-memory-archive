  ...
  factory: (require) => {
    ...
    return (() => {
      ...
      function RpMemoryFlow({ scrollBind }) {
        ...
        const [openFolder, setOpenFolder] = React.useState({ status: 'idle', message: '' })
        ...
        /** 「打开文件夹」：POST 出去、宿主在系统文件管理器里打开记忆库目录；失败如实说。 */
        const openMemFolder = () => {
          if (openFolder.status === 'busy') return
          setOpenFolder({ status: 'busy', message: '' })
          ;(async () => {
            try {
              const data = await apiPost(HOST_API_BASE + '/playthrough/reveal', undefined, {})
              setOpenFolder({ status: 'ok', message: '已交给系统文件管理器打开：' + ((data && data.baseLabel) || '记忆库目录') })
            } catch (error) {
              setOpenFolder({ status: 'err', message: errText(error) })
            }
          })()
        }
        ...
        return e('div', { style: colFillStyle },
          ...
          gate === 'ready'
            ? e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
              // 落点那一行右侧挂「打开文件夹」（2026-09-20 用户口径）—— 只在这一档、且目录确实存在时才给。
              e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' } },
                e('div', { style: Object.assign({}, dimStyle, { flex: 1, minWidth: 0 }) },
                  '落点：' + (st.baseLabel || '未知') + (st.sharedHint ? ' · 记忆库按会话工作目录存放，可能被同一工作区里的多个周目共用。' : '')),
                e('button', {
                  id: 'dma-rpmem-openfolder', className: 'dma-btn', style: miniBtnStyle,
                  disabled: openFolder.status === 'busy',
                  title: '在系统文件管理器里打开这个周目的记忆库目录（.roleplay-memory/）',
                  onClick: openMemFolder,
                }, openFolder.status === 'busy' ? '正在打开…' : '打开文件夹'),
              ),
              openFolder.status === 'ok' ? e('div', { 'data-dma': 'openfolder-ok', style: dimStyle }, openFolder.message) : null,
              openFolder.status === 'err' ? e('div', { 'data-dma': 'openfolder-err', style: errorStyle }, '打开文件夹失败：' + openFolder.message) : null,
      ...
      // ---------- 阅读源·向量（2026-09-20 第四档）：状态体检 + dsh-memory 库行 + 参与检索开关 ----------
      ...
      const VECTOR_ARMED = {
        'ingest-now': { action: 'ingest-now', label: '立即入库', hint: '把归档里还没入库的切片补进向量库与 BM25（要调嵌入接口，可能花几十秒、消耗 token）。' },
        'rebuild:vector': { action: 'rebuild', label: '重建（向量 + BM25）', hint: '会重新嵌入，可能花几十秒、消耗 token。' },
        'rebuild:bm25': { action: 'rebuild', label: '重建（向量 + BM25）', hint: '会重新嵌入，可能花几十秒、消耗 token。' },
        'delete-vector': { action: 'delete-vector', label: '删除向量库', hint: '删掉整个向量库（改名留档，不是销毁；删完检索只剩 BM25 那一路）。' },
        'delete-bm25': { action: 'delete-bm25', label: '删除 BM25 库', hint: '删掉整个 BM25 库（改名留档，不是销毁；删完检索只剩向量那一路）。' },
      }
      ...
      function vectorIngestText(g) {
        ...
        if (typeof g.skipped === 'string' && g.skipped !== '') bits.push('跳过原因：' + g.skipped)
      ...
      function VectorFlow({ scrollBind }) {
        const [snap, setSnap] = React.useState({ status: 'loading', info: null, result: null, live: null, staleMs: null, error: '' })
        ...
        const load = () => requestJson(HOST_API_BASE + '/vector/state')
          ...
          .catch((error) => {
            if (alive.current) setSnap({ status: 'error', info: null, result: null, live: null, staleMs: null, error: errText(error) })
          })
        ...
        const startPolling = (id) => {
          ...
          const tick = async () => {
            ...
            if (Date.now() - started >= VECTOR_POLL_LIMIT_MS) {
              setOut({ status: 'timeout', message: '还没被执行（anima 在该周目会话的下一轮开始前才取走这张单）' })
              setBusy('')
              return
            }
          ...
          ;(async () => {
            try {
              ...
              if (id === '') {
                ...
                setBusy('')
                setOut({ status: 'fail', message: '提交了但宿主没回执 id，无法跟踪（看「最近一次动作回执」）' })
                return
              }
              startPolling(id)
            } catch (error) {
        ...
        const toggleChat = () => {
          ...
          ;(async () => {
            try {
              ...
              setOut({ status: 'ok', message: '已' + (next ? '打开' : '关掉') + '「参与检索」（同时管向量与 BM25；下一轮装配生效）' })
            } catch (error) {
        ...
        const bound = iso && typeof iso.bound === 'string' && iso.bound !== '' ? iso.bound : ''
        ...
        const boundSrcLabel = boundSrc === 'session' ? '活跃会话' : (boundSrc === 'config' ? '面板绑定' : '')
        // 上次入库的人话：★ 用 ingest-state 里 anima 写好的 `reason`（describeIngest 的那句，如
        // 「没有入库：账本说这些摘要都入过了（内容没变）」）。live 这份是新读的，快照里那份
        // （info.ingest.state）是兜底；两处都没有才退回逐字段拼（vectorIngestText），⛔ 不自己编句子。
        ...
        // ★ 用户今天真正踩到的坑：库里有切片、但**一条都不属于当前绑定的周目** ⇒ 隔离把候选池清空，
        //   检索永远 0 命中，而且一声不响。这一句必须一眼可见（红字），所以摆在体检行里。
        const isolationDead = iso !== null && boundCount === 0 && total !== null && total > 0
        const armedDoc = armed !== '' ? VECTOR_ARMED[armed] : null
        const missingBadge = (exists) => (exists === false ? e('span', { style: warnBadgeStyle }, '⚠缺失') : null)

        // 一组的按钮（重建 / 删除）——两个库各一组；「重建」两组指向同一个 action（见 VECTOR_ARMED 说明）。
        // `missing`=这个库定位不到（快照说了它没有 / 根缺失）⇒ 行内摆 ⚠缺失 徽标（任务口径：缺失要 ⚠）。
        const group = (key, label, detail, missing) => {
          const armedHere = armed === key
          return e('div', { style: { display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' } },
            e('span', { style: Object.assign({}, rowLabelStyle, { minWidth: 0 }) }, label),
            e('span', { style: dimBadgeStyle }, detail),
            missing === true ? e('span', { style: warnBadgeStyle }, '⚠缺失') : null,
            e('button', {
              id: 'dma-vector-rebuild-' + (key === 'rebuild:bm25' ? 'bm25' : 'vector'),
              className: 'dma-btn', style: miniBtnStyle, disabled: busy !== '',
              title: '重建这个库（anima 的 rebuild 是向量 + BM25 一起重建；会重新嵌入，可能花几十秒、消耗 token）',
              onClick: () => act(key),
            }, armedHere ? '再点一次确认' : '重建'),
            e('button', {
              id: 'dma-vector-delete-' + (key === 'rebuild:bm25' ? 'bm25' : 'vector'),
              className: 'dma-btn', style: miniBtnStyle, disabled: busy !== '',
              title: '删掉这个库（改名留档，不是销毁）',
              onClick: () => act(key === 'rebuild:bm25' ? 'delete-bm25' : 'delete-vector'),
            }, armed === (key === 'rebuild:bm25' ? 'delete-bm25' : 'delete-vector') ? '再点一次确认' : '删除'),
          )
        }

        const healthRows = []
        healthRows.push(e('div', { key: 'h1', style: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'baseline' } },
          e('span', { style: sectionTitleStyle }, '向量库体检'),
          e('span', { style: dimBadgeStyle }, '集合：' + String((info && info.collectionId) || 'dsh-memory')),
        ))
        healthRows.push(e('div', { key: 'h2', style: dimStyle },
          '绑定周目：' + (bound !== '' ? bound : '（未绑定）') + (boundSrcLabel !== '' ? '（来源：' + boundSrcLabel + '）' : '')
          + ' · 库里 ' + (vec && vec.count !== null ? String(vec.count) + ' 条' : '条数读不到')
          + '（属于本周目 ' + (boundCount === null ? '—' : String(boundCount)) + ' 条'
          + ' · 被隔离排除 ' + (deniedCount === null ? '—' : String(deniedCount)) + ' 条）',
        ))
        healthRows.push(e('div', { key: 'h3', style: dimStyle },
          'BM25：' + (bm === null ? '读不到（还没有快照）' : (bm.exists === true ? (formatBytes(bm.bytes) + ' · ' + (bm.mtime ? new Date(bm.mtime).toLocaleString() : '—')) : '⚠缺失'))
          + ' · 上次入库：'
        ...
        healthRows.push(e('div', { key: 'h4', style: dimStyle },
          '快照：' + (info && Number.isFinite(info.at) ? new Date(info.at).toLocaleString() + '（落后 ' + vectorStaleText(snap.staleMs) + '）' : '还没有快照（anima 还没跑过那一脚）')
          + ' · 入库时机：自动压缩入库 + 工具检索调用时（本档常驻不生成向量）',
        ))
        if (isolationDead) {
          healthRows.push(e('div', { key: 'dead', id: 'dma-vector-isolation-dead', style: errorStyle },
            '库里 ' + String(total) + ' 条全部属于别的周目（' + (bound !== '' ? bound : '未绑定') + ' ≠ 当前绑定）⇒ 隔离把候选池清空了，所以检索永远是 0 命中',
          ))
        }

        const resultRow = snap.result
          ? e('div', { id: 'dma-vector-lastresult', style: dimStyle },
              '最近一次动作回执：' + String(snap.result.action || '') + ' · '
        ...
        return e('div', Object.assign({}, scrollBind), e('div', { style: readColStyle },
          ...
          snap.status === 'ready' && info === null
            ...
            : null,
          resultRow,
          // 库行：只列 dsh-memory 这一个集合（用户 2026-09-20 拍板「只列这一个」）
          e('div', { id: 'dma-vector-collection', style: cardStyle },
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' } },
              e('span', { style: { fontFamily: MONO, fontSize: 13 } }, String((info && info.collectionId) || 'dsh-memory')),
              missingBadge(vec ? vec.exists : undefined),
              e('button', {
                id: 'dma-vector-ingest', className: 'dma-btn', style: miniBtnStyle, disabled: busy !== '',
                title: '把归档里还没入库的切片补进向量库与 BM25（要调嵌入接口，可能花几十秒、消耗 token）',
                onClick: () => act('ingest-now'),
              }, armed === 'ingest-now' ? '再点一次确认' : '立即入库'),
            ),
            group('rebuild:vector', '向量库', vec === null ? '读不到' : (vec.exists === false ? '—' : (vec.count !== null ? String(vec.count) + ' 条' : '条数读不到')), vec !== null && vec.exists === false),
            group('rebuild:bm25', 'BM25', bm === null ? '读不到' : (bm.exists === false ? '—' : formatBytes(bm.bytes)), bm !== null && bm.exists === false),
          ),
          armedDoc
          ...
          e('div', { style: cardStyle },
            e('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
              ...
              e('span', { style: dimStyle }, '这一个开关同时管向量与 BM25 两条支线：关掉后这一档的两个库都不再参与检索。'),
              e('button', {
          ...
          e('div', { style: dimStyle }, '状态由 dsh-anima-rag 落的三张文件驱动（快照 / 请求单 / 回执）；本档只读状态、写请求单，真正的动作在 anima 下一轮装配前执行。'),
        ))
      }
        ...
        React.useEffect(() => {
          ...
          const sync = async () => {
            try {
              ...
              if (!alive || !data || data.ok !== true || data.source !== 'session') return
              const char = String(data.characterId || '')
              const play = String(data.playthroughId || '')
              if (char === '' || play === '') return
              const key = char + '/' + play
      ...
      function IngestToast() {
        ...
        if (!running && !finishedRecently) return null
        const text = running
          ? '记忆库正在后台收纳当前会话…'
