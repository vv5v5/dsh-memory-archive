 ...
 *     不并进 __internals 等既有出口）。会话正文里划选 AI 回复 ⇒ 点「质疑」⇒ 把
 *     「> 逐行引用（超 120 字符截断并如实标注）+ 【OOC】我的质疑：」**追加**进输入框草稿，
 *     发送由玩家自己按。三条底线：① 先读后写 —— 草稿从输入卡片 DOM 读（InputActions 只写），
    ...
    return (() => {
      ...
      function AgentEditorPanel({ onClose }) {
        ...
        const drillToPart = (boxId, block) => {
          ...
          setL3Section((boxId === 'system' || boxId === 'contexts') && (key.indexOf('sec:') === 0 || key.indexOf('ctx:') === 0) ? block : null)
      ...
      //   「> 逐行引用 + 【OOC】我的质疑：」追加进输入框草稿，玩家自己按发送。
      ...
      //   ③ 拼装 = 原草稿逐字保留在最前 + 空行 + 引用块；引用超长按 prompt-viewer 的
      //     PREVIEW_MAX_CHARS=120 同口径截断并标注「（已截断，原 N 字符）」。
      // ==================================================================
      ...
      /** 拼装（纯函数，自检直测）：★ 原草稿逐字保留在最前，引用块只追加在其后、绝不重排 */
      ...
      function OocQuoteButton(props) {
        ...
        return e('button', {
          ...
          onClick: onClick,
          style: { fontSize: 12, padding: '4px 8px', border: 'none', borderRadius: 6 },
        }, '质疑')
      }
