 ...
 *   ① 整段逐字出现在指令里；或
 *   ② 正文里**过半的字**（按字数，不按行数）能在指令里逐字找到。
 *   `instruction` 为空 = 拿不到当前生效的指令 ⇒ **只按结构判**（本函数恒 false，不瞎猜）。
 *
 * ★★ 2026-09-22：② 从「命中**行数**过半」改成「命中**字数**过半」。行数判据在**结构化输出**
 *   上必然误伤：模型按指令规定的 JSON 形状回时，`"tags": {` / `"special": [],` / `"important": false`
 *   这些 **schema 行本来就逐字在指令里**（指令就是这么规定输出的）⇒ 真机实测一份**完全正常**的
 *   归档摘要在旧判据下 27/45 行命中（0.60 ≥ 0.5）被判成回声**拒收**，整段模型摘要白白丢掉；
 *   同一份按字数只有 229/2082 = 0.11。回声的本质是"回吐的文字**按字数**来自指令"，抄几句
 *   schema 不算；而真回声（模型把指令原文当摘要回）比例 ~1.0，照样咬得住。
 */
export function looksLikePromptEcho(text, instruction, opts = {}) {
  ...
  const hit = lines.filter((l) => insS.includes(scrub(l)))
  const mass = (arr) => arr.reduce((n, l) => n + scrub(l).length, 0)
  const total = mass(lines)
  if (total === 0) return false
  return mass(hit) / total >= (opts.minEchoMassRatio ?? 0.5)
}
