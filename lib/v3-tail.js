 ...
 */
export function registerV3Tail(ctx, config = {}, log) {
  ...
  const holder = { reason: 'init', chars: 0, at: 0, sid: null }
  ...
  const computeTail = (assembleContext) => {
    ...
    try {
      ...
      if (typeof sid !== 'string' || sid === '') { stamp('no-session', null, 0); return '' }
      const service = typeof ctx.get === 'function' ? ctx.get('pmpDshTavernPrompt') : null
      if (!service || typeof service.getSources !== 'function') { stamp('no-tavern-service', sid, 0); return '' }
      let sources = null
      try { sources = service.getSources(sid) } catch (e) { stamp('sources-threw:' + String((e && e.message) || e), sid, 0); return '' }
      const r = tailTextFromSources(sources, { tag: TAG, dataKey: DATA_KEY, label: LABEL })
      if (r.text === '') { stamp(r.reason, sid, 0); return '' }
      const changed = holder.chars !== r.chars || holder.sid !== sid
      stamp(r.reason, sid, r.chars)
      if (changed) info(`[mt] 尾部段已产出：会话 …${sid.slice(-6)} · ${r.chars} 字符 · order ${order}`)
      return r.text
    } catch (e) {
      stamp('threw:' + String((e && e.message) || e), null, 0)
      return ''
    }
  }
  ...
  return {
    ...
    name,
    order,
    status: () => ({ enabled: true, name, order, chars: holder.chars, reason: holder.reason, at: holder.at, sessionTail: holder.sid ? holder.sid.slice(-6) : null }),
  }
}
