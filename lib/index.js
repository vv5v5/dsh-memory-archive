...
/**
 * ★ 2026-09-24 收尾：**「上次认过的剧情位置」（`seen`）的簿记文件** —— 判"开 fork"的基准就存在这儿
 * （`{schemaVersion:1, byPlaythrough:{[周目id]:{nodeId,variantId,seq,at}}}`）。
 * ⚠️ 这是本单**唯一**新增的写盘，写的是**我们自己的**文件（与 `floor-status.json` 同一处）；
 *    ⛔ 不塞周目目录的 `index.json`（用户的账）、⛔ 不塞记忆库（笔记的地盘）。
 */
const FLOOR_HEAD_FILE = 'floor-head.json'

/** 装配期"取最近一轮文本"的有界等待（超时 ⇒ 本轮不注入；绝不阻塞主对话）。 */
const PREP_BUDGET_MS = 1200
...
// ★★ 2026-09-26（T3）：`dma:echo` **已退役** ⇒ 从名册里摘掉。
//   本插件不再注册这个段（`<memoryEcho>` 那一格改由 `dsh-anima-rag` 的语义命中装，
//   段挂在 anima 自己那边）；留着它只会让 replace 看守永远报"缺失"。
//   ⛔ 别把 `dma:echo` 加回来 —— 两个插件各算一份"回响"就是两份真相。
const OUR_SECTION_NAMES = Object.freeze([
  'anima:memory', 'rp:firstRound', 'mt:lastFloors', 'mt:memoryHome',
])
...
// 归档楼层读取（./archive-floors.js）：`mt:lastFloors`（最近几楼）与楼层跟随要用的**纯读盘**件。
// ★★ 2026-09-26（T3「摘掉 fts5」）：「记忆回响」那一格的 **FTS5(trigram) 引擎整块退役** ——
//   `lib/echo-index.js`（建表/入库/抽候选/df 筛/MATCH 查询 + `<storageDir>/echo-index.db`）与
//   `lib/echo-inject.js`（拼 `<memoryEcho>` + 工厂）**两个文件都已删除**，`registerEcho` 那段接线
//   也整段删掉。为什么：回响那一格改由 `dsh-anima-rag` 的**语义检索**装（用户口径
//   「把anima也加入回响吧。也就是自动触发」＋「fts5目前看用处不大，摘除吧」）——
//   两个插件各算一份"回响"就是两份真相，而且关键词命中在中文上本来就不如语义命中。
//   退役是**如实体检**的：面板上明说「记忆回响已由 anima 接管」「BM25 已退役」，⛔ 不是静默消失。
// 保留的那一件（本模块）：只读归档楼层 —— lastFloors / 楼层跟随仍在用，与 fts5 无关。
let archiveFloors = null
let archiveFloorsLoadError = null
try {
  const mod = await import('./archive-floors.js')
  archiveFloors = typeof mod.readArchiveFloors === 'function' ? mod : null
  if (archiveFloors === null) archiveFloorsLoadError = new Error('archive-floors.js 未导出 readArchiveFloors')
} catch (e) {
  archiveFloorsLoadError = e
}
...
// ★★ 2026-09-24（用户口径「**不能在回档操作之后立刻弹吗，检测 tarven**」）：**主机侧盯时间线**
// （./floor-watch.js —— 纯逻辑内核，`watch` 由这里注入）。
// 同样顶层守卫加载 —— 缺席时**只有"即时检测"这一件不接线**（退化成"轮末 / 开面板才发现"），
// 插件其余功能（含那 5 条楼层端点、SSE 端点）与宿主启动完全不受影响。
let floorWatch = null
let floorWatchLoadError = null
try {
  const mod = await import('./floor-watch.js')
  floorWatch = typeof mod.watchTimeline === 'function' ? mod : null
  if (floorWatch === null) floorWatchLoadError = new Error('floor-watch.js 未导出 watchTimeline')
} catch (e) {
  floorWatchLoadError = e
}

/**
 * ★ 注入给 `floor-watch.js` 的那个 watcher（生产 = `node:fs` 的 `watch`）。
 * ⚠️ 做成可换的一格**只为自检台**：假 watcher 才能**直接喂事件**（真 fs.watch 也能跑，
 *   但要等文件系统事件、看平台脸色，台子会飘）。⛔ 运行时不换它、⛔ 也不改任何判据。
 */
/**
 * 注入给 `lib/floor-watch.js` 的文件监视器（生产 = `node:fs` 的 `watch`）。
 *
 * ★★ 2026-09-24 **真机踩到，⛔ 别把 `persistent:false` 去掉**：`fs.watch` 默认是
 *   **persistent**（`{persistent:true}`）—— 它会**把进程的事件循环吊住**。而我们的自检台结尾
 *   **故意不调 `process.exit`**（那会在 Windows 上撞 libuv 断言 `!(handle->flags & UV_HANDLE_CLOSING)`，
 *   见台子结尾那段注释），靠的是"事件循环自己空掉"⇒ 一条活着的 watcher 让它**永远空不掉**：
 *   台子永不退出 ⇒ 全量门（`spawnSync` + `stdio:'inherit'`，**没设超时**）**永久卡住**。
 *   生产宿主本来就常驻（吊不吊都一样），所以这条只为测试而设也不亏：
 *   "别拿文件监视器吊住别人的事件循环"本来就是该有的礼貌。
 */
function watchNonPersistent(dir, listener) {
  return fsWatch(dir, { persistent: false }, listener)
}
let floorWatchImpl = watchNonPersistent

/** 仅自检台用：换掉文件监视器（不传 / 传非函数 ⇒ 换回 `node:fs` 的 `watch`）。 */
export function __setFloorWatch(fn) {
  floorWatchImpl = typeof fn === 'function' ? fn : fsWatch
}

/**
 * 仅自检台用：**马上拆掉**当前那条 watcher（＝宿主卸载时会做的那一脚）。
 *
 * ⚠️ ★ 2026-09-24 真机踩到：**台子收尾必须调它**。`ensureFloorWatch` 在 `apply()` 里就建好了
 *   watcher，而那些台子结尾**故意不调 `process.exit`**（会撞 Windows 的 libuv 断言）⇒ 靠"事件循环
 *   自己空掉"退出；实测**这条 watcher 会把循环吊住**（哪怕给 `fs.watch` 传了 `persistent:false`：
 *   单跑一个 `fs.watch(dir,{persistent:false})` 的 2 行脚本立刻退，但走插件这条就吊住）
 *   ⇒ 台子**跑完不退** ⇒ 全量门白等到超时。做实验证实：把这函数改成"不建 watcher"⇒ 台子立刻退。
 *   ⇒ 收尾拆掉它（生产宿主本来就常驻，这一脚只对台子有意义）。
 */
export function __disposeFloorWatch() {
  try { disposeFloorWatch() } catch { /* 拆不掉也只能算了 */ }
}
...
function defaultConfig() {
  return {
    ...
    // ★ chatEnabled（2026-09-20）：「向量」页签那颗「参与检索」开关 —— 只管**读**（向量库是否参与检索）。
    //   ★ 2026-09-26：BM25 退役后只剩向量这一条支线。
    ...
    // ★★ 2026-09-26（T3）：「记忆回响」的 `echo` 配置段**已摘除**（与 `lib/echo-index.js` /
    //   `lib/echo-inject.js` / `registerEcho` 一起退役）。那一格（`<memoryEcho>`）改由
    //   `dsh-anima-rag` 的**语义检索**装，预算在那边的 `inject.echo`（topK/maxChars/perHitChars）。
    //   ⛔ 别在这里加回 `echo:` —— 旧配置文件里的这个键现在会被**如实忽略**，
    //      并由 `/config` 的 retired 字段告诉面板"它退役了"（⛔ 不是静默消失）。
...
function sanitizeConfig(parsed) {
  ...
  // ★ 2026-09-26（T3）：旧配置里的 `echo` 段**不再解析**（回响已退役）。
  //   不报 issue：用户没做错什么，只是那个键过时了 —— 面板会显示"已退役"，这里静默忽略键本身。
...
function mergeConfig(current, patch) {
  ...
  // ★ 2026-09-26（T3）：`echo` 段已退役 ⇒ PUT 里**不再接受**它。
  //   ⛔ 不许"收下但不用"（那是静默撒谎）：认得这个键、但明确告诉调用方它退役了。
  if (patch.echo !== undefined) {
    fail('"echo" 已退役（2026-09-26）：<memoryEcho> 那一格改由 dsh-anima-rag 的语义命中装，'
      + '预算在它的 inject.echo 里配（topK / maxChars / perHitChars）')
  }
...
function publicConfig(config, configError) {
  return {
    ...
    // ★★ 2026-09-26（T3）：「记忆回响」已退役 —— 这里**如实说**，⛔ 不是静默消失。
    //   面板读到 `retired:true` 就把那一档显示成退役说明（谁接管、去哪配预算）。
    echo: {
      retired: true,
      retiredAt: '2026-09-26',
      replacedBy: 'dsh-anima-rag',
...
const ENDPOINTS = {
  ...
  //   · `/floors/restore` = 那颗「回到这一楼」（★ 2026-09-24 起**这是那 5 份唯一的写入口**；
  //     ★ 2026-09-25 起回档**提示那一排**上不再有「把档案退回」那颗 —— 那条路从清单行进；
  //     ★★ 2026-09-26 起**弹窗那颗「对齐楼层」也走它**（用户口径「不是不触发弹窗，是弹窗上点同步后，
  //     没有触发真回档」⇒ 那颗改成"先真回档、再记账"）—— 端点本身一个字没改）；
  //   · `/floors/pending/settle` = **仍走"就地登记"**的那两颗「对齐」（★ 2026-09-25 起一个端点带 `mode`
  //     分两种；★★ 2026-09-26 起这一脚只剩**档顶横幅**与**档顶常驻状态行**两处在调 —— 弹窗那颗已改走
  //     `/floors/restore`）：
  //     `mode:'full'`（缺省）＝「**对齐楼层**」，语义 = **就地登记**（★ 2026-09-24 用户口径：跟 Tavern 一致，
  //     回档之后那段历史就是没有了 ⇒ 把**当下盘上那 5 份**的现文记成"剧情现在站的这一楼"那一份快照，
  //     并把 `last` 锚到它）；`mode:'number'`＝「**只对齐楼号**」（★ 2026-09-25 新增：只把 `last` 锚过去，
  //     ⛔ 不记快照、⛔ 不写 blob、⛔ 不新增楼层行）。两种都 ⛔ **那 5 份正文一个字节都不写**；
  //     `mode` 认不出 ⇒ **400**（⛔ 不许悄悄按 full 跑）；
  //   记录与"判回档"是**钩子**做的（轮末记一份、组装时 + 轮末判一次），⛔ 这三条端点都不改判据。
  //   ★ 2026-09-24 改口径（用户原话「**不要做跟随楼层的功能，先做手动挡**，开 fork 时（回档）**前台弹提示，
  //     让用户手动回退剧情档案**」）：判到**开 fork**只在清单里记一条 `pending`（`/floors` 带出去给面板画横幅），
  //     ⛔ 那 5 份一个字节都不写 —— **正文**的写盘只走 `/floors/restore` 那一脚。
  //   ★★ 2026-09-24 收尾（用户原话「回档我没看到有横幅。」「**开 fork 时（回档）前台弹提示**」）：判据换成
  //     "剧情 vs `seen`"（见 `lib/floor-snapshot.js` 文件头 ⑤）⇒ 补两条端点：
  //     · `/floors/scan`          = 面板**开档时**补做一次检测（"开面板也检测一次"；语义是**会写**：
  //       `pending` / 我们那份 `floor-head.json` 可能变 ⇒ 所以是 POST。⛔ 仍然**碰都不碰**那 5 份正文）；
  //     · `/floors/pending/ack`   = 面板那颗「**不处理**」（清掉 `pending` ＋ `seen ← 当前 head`；
  //       ⛔ 不登记、⛔ 不写那 5 份 —— 与两颗 `settle` 的区别就在这儿）。
  '/playthrough/rp-memory/floors': ['GET'],
  '/playthrough/rp-memory/floors/scan': ['POST'],
  // ★★ 2026-09-25（用户口径「**开一个下钻，直接显示具体改动的字段，写改了哪些可以说完全没用**」）：
  //   `/floors/diff` = 清单每一行那颗「**看改动 ▸**」的取数口 —— **只读**：比这一条 vs 时间上紧随其前的
  //   那一条（按 `ordinal` 口径），逐份给出**真正的增行 / 删行**（带上下文；判据全在纯函数
  //   `floorSnapshot.diffFloorFiles` 里）。⛔ 按需取（点了才发，不轮询、不预取）、
  //   ⛔ 一个字节都不写（连检测都不跑）、⛔ 不碰 Tavern。
  '/playthrough/rp-memory/floors/diff': ['GET'],
  // ★★ 2026-09-24（用户口径「**不能在回档操作之后立刻弹吗，检测 tarven**」）：
  //   `/floors/events` = **我们自己的 SSE 长连**（推送面；实现在 `handleRpMemoryFloorsEvents`）。
  //   ⛔ 不走 `send()`（那是"JSON + end"，会立刻把连接关掉）—— 拿**原始 `res`** 自己写头、自己写帧；
  //   连上先发一帧 `hello`（现状，与 `/floors` 同一份拼装），判到**开 fork** 时由 fs.watch 那一脚推一帧；
  //   25 秒一帧注释心跳；订阅者上限 8（超了 503 + 一句人话）。
  //   ⛔ 这条端点**纯推送、一个字节都不写**（连"顺手检测一次"都不做）。
  '/playthrough/rp-memory/floors/events': ['GET'],
  '/playthrough/rp-memory/floors/restore': ['POST'],
  '/playthrough/rp-memory/floors/pending/settle': ['POST'],
  '/playthrough/rp-memory/floors/pending/ack': ['POST'],
...
function createHandler(ctx, log) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      //   ★ 2026-09-24 收尾：`/floors/scan`（开档补一次检测）与 `/floors/pending/ack`（「不处理」）。
      if (rest === '/playthrough/rp-memory/floors') return handleRpMemoryFloors(send)
      if (rest === '/playthrough/rp-memory/floors/scan') return await handleRpMemoryFloorsScan(req, send, log)
      // ★ 2026-09-25：「看改动 ▸」那一次下钻（只读；按需取 —— 面板点一颗才发这一个请求）。
      if (rest === '/playthrough/rp-memory/floors/diff') return handleRpMemoryFloorDiff(send, url)
      // ★★ 2026-09-24：**SSE 长连**（「回档 → 立刻弹」那条推送面）—— 拿原始 res 自己写，⛔ 不走 send()。
      if (rest === '/playthrough/rp-memory/floors/events') return handleRpMemoryFloorsEvents(req, res, send, log)
      ...
      if (rest === '/playthrough/rp-memory/floors/pending/ack') return await handleRpMemoryFloorPendingAck(req, send, log, redact)
...
function registerHttpApi(ctx, scope, log) {
  ...
  run(() => {
    ...
    // ★★ 2026-09-24：随插件卸载/热重载把那条 SSE 长连上的浏览器**全断开**并清空订阅者集合
    //   （⛔ 不留悬挂连接、⛔ 不留幽灵订阅者）；没建过就什么都不做（⛔ 不为关掉而凭空建一个）。
    const disposeSse = () => {
      const hub = floorSseHubIfAny()
      if (hub !== null) {
...
/**
 * 读「现文」给 `planNoteWrite` 用（返回 `null` = 这份文件还**不在**）。
 * ⚠️ 只有**确实不存在**才当 `null`；**在、但读不出来**（权限 / 被占用一类）⇒ **抛** ——
 *   那一下覆盖是**不可逆**的、而我们连备份都拿不到现文 ⇒ ⛔ 不许折成"没有现文"悄悄裸写。
 */
function readNoteCurrent(absPath) {
  try {
    return readFileSync(absPath, 'utf8')
  } catch (e) {
    if (e && (e.code === 'ENOENT' || e.code === 'ENOTDIR')) return null
    throw new Error(`现文读不出来（${e?.code || e?.message || e}）⇒ 拒写：`
      + '读不到就没法判 replace 能不能落地、也没法先备份')
  }
}

/** 一段原文在现文里出现了几次（**只用于把拒收理由说人话**；裁决仍在 `planNoteWrite` 那一处）。 */
function countOccurrences(haystack, needle) {
  if (typeof haystack !== 'string' || typeof needle !== 'string' || needle === '') return 0
  let n = 0
  let at = haystack.indexOf(needle)
  while (at >= 0) { n += 1; at = haystack.indexOf(needle, at + 1) }   // 重叠出现也照数（与 plan 同口径）
  return n
}

/**
 * 把 `planNoteWrite` 的拒收理由翻成**可照做**的人话（★ 20260924；⛔ 不是"拒写：find-ambiguous"这种）。
 * 要点（任务书 §3.2.2）：**找不到就说"先 read 一遍再指"**；**多义就说出现了几次、给长一点**。
 */
function notePlanRejectText(reason, current, find) {
  if (reason === 'no-find') {
    return 'replace 要带 find：**原文里逐字的一小段**（先 read 一遍，把要换掉的那段原样抄进来）'
  }
  if (reason === 'no-file') {
    return 'replace 只能改**已存在**的文件 —— 这一份还不存在 ⇒ 新建请用 overwrite（或 append）'
  }
  if (reason === 'find-missing') {
    return '那段原文没找到（find 与现文对不上）⇒ 先 read 一遍，把要换掉的那段**逐字**抄进 find（⛔ 别凭印象打）'
  }
  if (reason === 'find-ambiguous') {
    const n = countOccurrences(current, find)
    return `这段话在文件里出现了 ${n > 0 ? n : '多'} 次 ⇒ 给长一点、能**唯一确定**的一段`
      + '（连同前后那几行一起指；⛔ 工具不会替你挑一处）'
  }
  if (reason === 'bad-mode') return '不认识的 mode（只有 overwrite / append / replace 三种）'
  return `拒写：${reason}`
}

/**
 * ★ 20260926：**撞单次上限 ⇒ 一次明确的"该优化了"提示**（用户口径：「重新设计下记忆写入功能，
 *   改为**到上限时自动提示需要优化**。同步**轻量化提示词**」）。
 *
 * 上一版的回执只给一个出路 ——「拆成两次写」，那**只会让笔记更长**（笔记长胖正是撞上限的成因）。
 * ⇒ 这一版把三件事说全，顺序也说死：
 *   ① **撞的是哪个上限**（这一段 N 字符 / 单次上限 M —— 数字给全）；
 *   ② **为什么该优化**（笔记在长胖，而 §5 里那几段本该「每轮替换、只留最新」）；
 *   ③ **怎么优化**（可照做的三件：过时条目 `replace` 成空 / 最早场记压成一行 / `index.md` 自己那几段按骨架整段换掉）。
 *   ★ 顺序：**先优化、再写**（⛔ 不是"拆两次写" —— 拆两次只会让文件更长）。
 *   ⛔ 判据一个字没动：`over-budget` 仍是 `decideNoteWrite` 里那条"text 超 `maxChars` ⇒ 拒收"；
 *     这里不改上限、不新造阈值、不自动压缩/自动删除 —— 动手的永远是模型自己。
 * ⚠️ **自记标记的字面只从 `NOTE_MARK` 来**（⛔ 不在本文件重打一遍；`_selftest-memory-write.mjs` 的 ★M⑧b 钉着它）。
 *
 * @param {{ chars?: unknown, maxChars?: unknown }} decided `decideNoteWrite` 判 `over-budget` 时给的那一份
 */
function noteOverBudgetText(decided) {
  const chars = Number.isFinite(decided?.chars) ? decided.chars : 0
  const maxChars = Number.isFinite(decided?.maxChars) ? decided.maxChars : (memoryWrite?.MEMORY_WRITE_MAX_CHARS ?? 0)
  return [
    `这一段 ${chars} 字符，超过单次上限 ${maxChars} ⇒ 拒收（⛔ 不会截断硬写）。`,
    ...
    `③ \`index.md\` 你自己那几段（${NOTE_MARK} 那两段）⇒ 按 §5 的骨架**整段换掉**，只留最新那份。`,
    '三件都做完还超 ⇒ 才拆成两次写（⛔ 不要硬塞）。',
  ].join('\n')
}

/**
 * ★ 20260925：**自记标记** —— **唯一真相在 `lib/memory-write.js`**（`NOTE_SELF_MARK`）。
 * ⛔ 这里**不重打**那个字面（重打一遍迟早两处不一致；`_selftest-memory-write.mjs` 的 ★M⑧b 钉着这件事）。
 * ⚠️ 取不着也不抛：那说明 `memory-write.js` 整块缺席 —— 那种情况下 `memory_write` 工具**根本不会注册**
 *   （见 `registerNoteWriteTool` 开头的降级），⛔ 不让这一句话把整条插件加载链拖下水。
 */
const NOTE_MARK = memoryWrite !== null && typeof memoryWrite.NOTE_SELF_MARK === 'string'
  ? memoryWrite.NOTE_SELF_MARK
  : ''
...
const NOTE_WRITE_DESCRIPTION = [
  ...
  'mode 三选一：`overwrite` = 整份替换（默认；**覆盖已存在的文件会先自动留一份备份**）；',
  '`append` = 追加到末尾；',
  '`replace` = **只替换 `find` 指到的那一小段**：`find` 必须是**原文里逐字的一小段**',
  '（指不准就先 `read`；找不到 / 出现多次会被拒收）。',
  '★ `replace` 时 **`text` 留空 = 把 `find` 指到的那一段整个删掉**（清理过时的条目就用它；'
  + '`find` 仍然要指得准，删除前照样自动留一份备份）。',
  // ★ 20260925：自记标记 ＋ 段模式（字面从 `lib/memory-write.js` 的常量来，⛔ 不在这里重打）
  '★ **你自己写的每一段**，段首**独占一行**写 ' + NOTE_MARK + '（**紧贴**在它那一段的标题**上面**；'
  + '作者预置的部分**不带**它）—— 认这一行＝认**你自己那几段**，不再靠猜文件名。',
  '★ `replace` 的 `find` 给**这一行标记**（' + NOTE_MARK + '）⇒ 工具把那一**整段**换掉'
  + '（**标记行 ＋ 它的标题 ＋ 正文**；这次的 `text` 就是这一整段的新内容，**你自己那一行标记也要写上**）；'
  + '`find` 里**再带上紧跟的那个标题行**（如 `## 最近进展`）⇒ 在多段并存时指定是**哪一段**；'
  + '`text` 留空 ⇒ **整段删**（⛔ 别把上一轮那版留着另开一段）。',
  // ★ 20260925（复核时补）：段模式的**两个已知面** —— 不写出来，模型只会看到"换掉一整段"，
  //   却不知道"段"是按**标题行**切的 ⇒ ① 自己那一段里放个 `### 细节` 就把自己的段切断了
  //   （后半截留在文件里，下一轮它认不出那是自己的）；② 段尾跟着的无标题作者文字会被一起带走。
  //   ⛔ 不加这两句 ⇒ 这两类是**静默**的（回执只报字符数）。
  '⚠️ 一段 = 从那一行标记**到下一个标题行为止** ⇒ ① 你自己那一整段里⛔ **别再起标题行**'
  + '（如 `### 细节` —— 它就是这一段的结尾，会把你的段切断、后半截留在文件里）；'
  + '② 段尾若跟着**作者写的、没有标题的**文字，会被一起删掉 ⇒ 删整段之前先 `read` 一眼那后面是什么。',
  '⛔ 只在真的记东西时调；一次写一段完整的 —— 超单次上限会被**拒收**（那时**该优化笔记了**，回执会给三件事；先优化、再写）。',
].join('\n')
        ...
        toolCtx.tools.register({
          ...
          parameters: {
            ...
            properties: {
              ...
              text: {
                type: 'string',
                description: '要写入的内容（超单次上限会被拒收 ⇒ 那时**该优化笔记了**：先优化、再写）；'
                  + 'mode:replace 时**留空 = 把 `find` 指到的那一段整个删掉**',
              },
              mode: {
                type: 'string', enum: ['overwrite', 'append', 'replace'],
                description: 'overwrite（默认，整份替换；覆盖已存在的文件会先自动留一份备份）／'
                  + 'append（追加到末尾）／replace（只替换 find 指到的那一小段；text 留空则把那一段删掉）',
              },
              find: {
                type: 'string',
                description: 'replace 专用：**原文里逐字的一小段**（先 read 一遍照着抄；'
                  + '找不到 / 出现多次会被拒收 —— 那时给长一点、能唯一确定的一段）；'
                  // ★ 20260925：**段模式**的形状（字面从 lib/memory-write.js 的常量来，⛔ 不在这里重打）
                  + '**也可以给那一行自记标记**（' + NOTE_MARK + '，独占一行）：'
                  + '`find` 的**第一行**是它 ⇒ 工具换掉/删掉那一**整段**（标记行＋它的标题＋正文，'
                  + '⛔ 不含结尾那个标题行）；`find` 里**再带上紧跟的那个标题行**（如 `## 最近进展`）'
                  + '⇒ 在多段并存时指定是哪一段；`mode:\'replace\'` ＋ `text` 留空 ⇒ **整段删**',
              },
            },
          },
          output: {
            schema: {
              ...
              properties: {
                ...
                chars: { type: 'integer' }, bytes: { type: 'integer' }, replacedChars: { type: 'integer' },
                playthroughId: { type: 'string' }, source: { type: 'string' },
                // ★ 2026-09-26（收纳升级）：notes.md 过保养线 ⇒ 这里有**点名催收纳**的一句；没过 ⇒ 空串
                maintenanceHint: { type: 'string' },
              },
            },
            render: (args, value) => [{
              type: 'text',
              // ★ 20260925：`replace` + 空文本 = **纯删除** ⇒ 回执说"删掉 N 字符"。
              //   ⛔ 别拿通用那行套它：那会写成"替换掉 N 字符，0 字符"（读不通，也说不清到底干了什么）。
              // ★ 2026-09-26（收纳升级）：notes.md 过保养线 ⇒ 回执尾上**点名催收纳**的那句
              //   （写完这一刻模型最听得进去；`maintenanceHint` 空串时不加尾巴）。
              text: (value.mode === 'replace' && value.chars === 0
                ? `已在 ${value.path} 里删掉一段（replace，${value.replacedChars} 字符）· 周目 ${value.playthroughId}`
                : `已写入 ${value.path}（${value.mode}`
                  + `${value.mode === 'replace' ? `，替换掉 ${value.replacedChars} 字符` : ''}`
                  + `，${value.chars} 字符）· 周目 ${value.playthroughId}`)
          ...
          async execute(args, exec) {
            ...
            if (!decided.write) {
              ...
              if (decided.reason === 'over-budget') {
                throw new Error(noteOverBudgetText(decided))
              }
              // ★ 20260925：这一道**按 mode 分开**（见 memory-write.js 的 decideNoteWrite）——
              //   `replace` 的空文本是**纯删除**、已经放行，落进这里的是 `overwrite` / `append` 的空文本
              //   （那两种照旧拒收）⇒ 理由得把**出路**指出来（要删那一段就用 replace 留空）。
              if (decided.reason === 'empty-text') {
                throw new Error('text 不能空（`overwrite` / `append` 都要有正文）：想**把 `find` 指到的那一段删掉**，'
                  + '请用 `mode:\'replace\'` 且 `text` 留空')
              }
            ...
            // ★★ 20260924（用户口径：「ai 目前**不会删掉过时的 index 和 note 信息**」⇒「**加手段吧**」）：
            //   写盘这一脚从"直接 writeFileSync/appendFileSync"换成**裁决 → 落盘**两步（都在 lib/memory-write.js）：
            //     ① 先读**现文**（读不到 ⇒ null；在、但读不出来 ⇒ 上面那道直接抛）；
            //     ② `planNoteWrite` 算"到底写什么"：`replace` 只换 `find` 指到的那一小段，
            //        **指不准一律拒收**（找不到 / 出现多次）⇒ ⛔ 绝不猜；
            //        ★ 20260925：`replace` 的 `text` 留空 = **把那一段整个剪掉**（"纯删除"）——
            //        这就是"清理过时条目"那一脚（它走的是同一条路，照样先备份）。
            //     ③ 只有 `plan.ok` 才 `applyNoteWrite` —— 覆盖/替换**已存在**的文件由它先备份成
            //        `<名>.bak-<stamp>` 再写（`deadzone.writeWithBackup`：不可逆这件事在这拆掉）。
            //   ⚠️ `mkdirSync` 也挪到裁决**之后**：被拒的那一次**盘上零变化**（连目录都不建）。
            const currentText = readNoteCurrent(target.absPath)
            const plan = memoryWrite.planNoteWrite({
              current: currentText, mode: decided.mode, find: args?.find, text: args.text,
            })
            if (!plan.ok) throw new Error(notePlanRejectText(plan.reason, currentText, args?.find))
            // ⚠️ 要备份却没有备份实现（deadzone.js 缺席）⇒ **拒写**：宁可这次没记成，
            //   ⛔ 也不拿"覆盖不可逆"去换一次便利（这一脚不许裸写）。
            if (plan.backup === true && (deadzone === null || typeof deadzone.writeWithBackup !== 'function')) {
              throw new Error('备份写盘模块（deadzone.js）不可用 ⇒ 拒写：覆盖/替换已存在的文件是不可逆的，⛔ 不裸写')
            }
            try { mkdirSync(dirname(target.absPath), { recursive: true }) } catch { /* 已存在或并发建，忽略 */ }
            const stamp = plan.backup === true ? deadzone.stampNow() : ''
            const applied = memoryWrite.applyNoteWrite({
              absPath: target.absPath, next: plan.next, backup: plan.backup, stamp,
              io: { writeWithBackup: deadzone?.writeWithBackup, writeFileSync, appendFileSync },
            })
            if (!applied.ok) throw new Error(`写盘失败（${target.relPath}）：${applied.reason}`)
            log.info(`[mt] memory_write：${target.relPath}（${decided.mode}`
              // ★ 20260925：`replace` + 空文本 = 纯删除 ⇒ 日志那行也说"删掉 N 字符"
              //   （⛔ 别写成"替换 N 字符，0 字符"）
              + (decided.mode === 'replace'
                ? (decided.chars === 0 ? `，删掉 ${plan.replacedChars} 字符` : `，替换 ${plan.replacedChars} 字符`)
                : '')
              + (decided.chars === 0 ? '' : `，${decided.chars} 字符`)
              + ` → 周目 ${home.playthroughId}`
              + `${applied.backupPath === null ? '' : `，备份 ${basename(applied.backupPath)}`}）`)
            // ★ 2026-09-26（收纳升级）：写完顺手数一眼 notes.md —— 过保养线 ⇒ 回执里**点名催收纳**
            //   （就在模型刚写完的这一刻说；notes.md 还没建 ⇒ 没什么可催，静默空串）。
            let maintenanceHint = ''
            try {
              maintenanceHint = memoryWrite.notesMaintenanceHint(
                readFileSync(join(home.dir, 'notes.md'), 'utf8').length)
            } catch { /* notes.md 不存在 ⇒ 无收纳可催 */ }
            return {
              path: target.relPath, mode: decided.mode, chars: decided.chars, replacedChars: plan.replacedChars,
...
/**
 * ★ 2026-09-26（收纳升级，用户口径「升级」）：段里那句 **notes.md 超保养线**的点名催收纳
 *   （没过线 / 认不出周目 / 文件还没建 ⇒ 空串）。与 `deadzoneHintFor` 同一落点、同一节奏：
 *   **每轮装配现算**（读一次 notes.md 数字符，⛔ 不落盘、不轮询、不挂定时器）。
 *   为什么数**字符**不数字节：persona §5 那条保养线说的就是「字」，CJK 在 UTF-8 是 3 字节/字。
 */
function notesMaintenanceFor(sessionId) {
  try {
    const home = memoryHomeFor(sessionId)
    if (home === null) return ''
    const hint = memoryWrite.notesMaintenanceHint(readFileSync(join(home.dir, 'notes.md'), 'utf8').length)
    return hint === '' ? '' : hint + '（本行每轮都会出现，直到字数压回线内 —— 现在就做，别等下一轮）'
  } catch {
    return ''
  }
}
...
//   ③ **提示**（★ 改口径）：组装时（每轮至多一次）+ 轮末 + ★ **面板开档时**（`POST …/floors/scan`）
//      各判一次，判到**开 fork**（剧情比"上次认过的那个位置"`seen` 退回去了 / 同一楼换了一支）**只往清单里
//      记一条 `pending`**（幂等：同一次 fork 不刷 `at`）⇒ 面板档顶弹一条**显著**横幅；⛔ **那 5 份一个字节都不写**
//      —— 写盘只在用户点了「回到这一楼」（楼层清单每一行）★ **或弹窗那颗「对齐楼层」（2026-09-26 起）**
//      之后走 `POST …/floors/restore` 那一脚（唯一写入口）；
//      ★ "开 fork"的判据与基准（`seen`）的进出**只有一处**：`lib/floor-snapshot.js` 的 `decideFork` ＋
//      下面 `floorSyncAt` 里那一段（写的是**我们自己的** `floor-head.json`，⛔ 不碰周目目录的账）；
//   ④ ⛔ **不碰别的**：只写那 5 份 + 我们自己的 `.dma-floor-snapshots/` + ★ 我们自己的 `floor-head.json`
//      （"上次认过的剧情位置"那本簿记 —— 本单**唯一**新增的写盘）；★ 死区那几段**按块合并**
...
/**
 * ★★ 2026-09-24：这个周目 `timeline.json` 的**绝对路径**（Tavern 的 `catalog.json` 里那条 `path`）。
 * ⛔ 只拼路径、**不读不解析**它 —— "盯目录"那一脚要的只是"它在哪儿"（内容仍归 `playthroughTimeline`）。
 * 拿不到（没绑工作区根 / 这个周目不在 catalog 里 / 那条没写 path）⇒ `''`（调用方安静跳过）。
 */
function playthroughTimelineFile(characterId, playthroughId) {
  try {
    const rootPath = tavernRootPath()
    const wantId = typeof playthroughId === 'string' ? playthroughId : ''
    if (rootPath === '' || wantId === '' || sessionPlaythrough === null) return ''
    let entries = null
    try { entries = JSON.parse(readFileSync(join(rootPath, 'catalog.json'), 'utf8'))?.playthroughs } catch { return '' }
...
function playthroughTimeline(characterId, playthroughId) {
  try {
    const file = playthroughTimelineFile(characterId, playthroughId)
    if (file === '') return null
    let st = null
...
/**
 * ★★ 「上次认过的**剧情位置**」那份簿记文件的路径（`<storageDir>/floor-head.json`）。
 *
 * 为什么是**我们自己的**文件、而不是别处：
 *   · ⛔ **不许**塞进周目目录的 `index.json`（那是用户的账：Tavern 的视图/别人的预设也在读它）——
 *     本单的纪律是"⛔ 也不许每轮去动它"；
 *   · ⛔ **不许**塞进记忆库（`.roleplay-memory/`）——那是**笔记**的地盘，用户会连目录一起拷走；
 *   · 放在 `storageDir()`（与 `floor-status.json` 同一处，宿主自己的存储目录）⇒ 与"最近一次回档"那本账同源，
 *     拷/删它都不影响用户的内容。
 */
function floorHeadPath() {
  return join(storageDir(), FLOOR_HEAD_FILE)
}

/**
 * 读某个周目的 `seen`（"上次认过的剧情位置"）—— 认不出（不在 / 读不出来 / 这一档没有）⇒ `null`
 * （当作**没有基线**：第一次见 ⇒ 建立基线，⛔ 不算 fork）。
 */
function readSeenHead(playthroughId) {
  const id = typeof playthroughId === 'string' ? playthroughId : ''
  if (id === '' || floorSnapshot === null) return null
  try {
    const doc = JSON.parse(readFileSync(floorHeadPath(), 'utf8'))
    if (!isPlainObject(doc)) return null
    const byPt = isPlainObject(doc.byPlaythrough) ? doc.byPlaythrough : null
    if (byPt === null) return null
    return floorSnapshot.normalizeSeen(byPt[id])
  } catch {
    return null
  }
}
...
/**
 * 写某个周目的 `seen`（整份读改写 + 临时文件 rename；照 `writeFloorStatus` 那几行抄）。
 *
 * ⚠️ **位置一模一样时一个字都不写**（★ 这一条是纪律，不是优化）：宿主每一轮都会拿 `forward` / `no-seen`
 *   去更新 `seen`，而"同一楼同一支"时新值就是旧值 ⇒ 不挡掉的话就是**每轮写盘**（用户口径里最不许的一条）。
 *   相等判据 = `(nodeId, variantId, seq)` 三样一致（`at` 是"第一次认下它的时刻"，⛔ 不跟着刷）。
 * ⚠️ 只动**我们自己的**簿记文件；⛔ 不碰周目目录、⛔ 不碰记忆库、⛔ 不碰 Tavern。
 * @returns {boolean} 真写下去了没有（面板/日志据此如实说"认下了"）
 */
function writeSeenHead(playthroughId, head) {
  const id = typeof playthroughId === 'string' ? playthroughId : ''
  if (id === '' || floorSnapshot === null) return false
  const next = floorSnapshot.normalizeSeen(head)
  if (next === null) return false
  const prev = readSeenHead(id)
  if (prev !== null && prev.nodeId === next.nodeId && prev.variantId === next.variantId && prev.seq === next.seq) return false
  try {
    const dir = storageDir()
    mkdirSync(dir, { recursive: true })
    let doc = null
    try { doc = JSON.parse(readFileSync(floorHeadPath(), 'utf8')) } catch { doc = null }
    const byPt = isPlainObject(doc) && isPlainObject(doc.byPlaythrough) ? doc.byPlaythrough : {}
    byPt[id] = { nodeId: next.nodeId, variantId: next.variantId, seq: next.seq, at: next.at === null ? new Date().toISOString() : next.at }
    const finalPath = floorHeadPath()
    const tmpPath = finalPath + '.tmp'
    writeFileSync(tmpPath, JSON.stringify({ schemaVersion: 1, byPlaythrough: byPt }, null, 2) + '\n', 'utf8')
    renameSync(tmpPath, finalPath)
    return true
  } catch {
    return false   // 写不进去不影响判据本身（下一轮还会再算一次；只是"认过的位置"这一轮没记住）
  }
}

/**
 * 最近一次回档跟随的结果（按周目取；⛔ 别的周目的记录当没有，不拿它冒充本局）。
 * ★ 2026-09-25（用户口径「这一整段删掉」）：面板那块「⇄ 最近一次回档」**整块撤掉了** ⇒ 本函数现在
 *   **没有面板侧的读者**；但**这份账照旧写着**（`writeFloorStatus` 那一脚一个字都没动）——
 *   它是"最近一次回档"的唯一落账，别删；将来要再画出来时它就是现成的取数口。
 */
...
/**
 * 这个周目**当下**的剧情位置（时间线现读；⛔ 只读，⛔ 不缓存 —— 用户在面板上点的那一脚要的是"现在")。
 * @returns {{head:object|null, seq:number|null, seen:object|null}}
 */
function floorHeadNow(home) {
  if (floorSnapshot === null || home === null) return { head: null, seq: null, seen: null }
  const timeline = playthroughTimeline(home.characterId, home.playthroughId)
  const head = floorSnapshot.headOf(timeline)
  if (head === null) return { head: null, seq: null, seen: null }
  const seq = floorSnapshot.seqOfNode(floorSnapshot.seqMapOf(timeline), head.nodeId)
  return { head, seq, seen: { nodeId: head.nodeId, variantId: head.variantId, seq } }
}

/**
 * ★ 用户那一脚**算认过了**（口径见文件头：点了「对齐楼层 / 只对齐楼号 / 不处理」，或清单行的「回到这一楼」⇒ `seen ← head`）。
 * ⚠️ 三条端点（settle / ack / restore）都要跟这一下，否则下一次"楼同步"还会按旧基准判成开 fork、横幅自己弹回来。
 * @returns {boolean} `seen` 真写下去了没有（认不出剧情位置 / 与已记的一模一样 ⇒ false）
 */
function floorAcknowledge(home) {
  const now = floorHeadNow(home)
  if (now.head === null || now.seen === null) return false
  return writeSeenHead(home.playthroughId, now.seen)
}
...
function floorReceiptText(home, receipt) {
  ...
  //   开 fork ⇒ **只提示、⛔ 那 5 份一个字节都不写**（写盘要人在面板的楼层清单里点「回到这一楼」）
  //   ⇒ 回档那一支的回执**没有 `restore`**（`rest === null`），说的是清单里那条 `pending`。
  //   ⚠️ 面板按**纯文本**渲染这一句 ⇒ ⛔ 不用 `**` 那种强调写法（星号会原样显示）。
  //   ★★ 2026-09-24 收尾改口径（用户原话「**开 fork 时（回档）前台弹提示**」）：这一段现在**只在真开了
  //   fork** 时才走到（回执的 kind 按 `fork` 报）—— 文案要说清三件事：**剧情刚从第 X 楼退到第 Y 楼** ＋
  //   **档案停在第 Z 楼** ＋ 要不要把档案也退回去**由你点**。
  if (kind === 'rollback' && rest === null) {
    // ⚠️ 这一支**一定**有那条 `pending`（`fork.kind==='fork'` ⇒ `planPending` 必给一条；没有 target 也给）。
    //   老口径那条 `p === null` 的分支（"这一档你点过「保持现状」⇒ 不再提示"）已随"就地登记"一起作废：
    //   现在点「保持现状 / 对齐」是**把 last 锚到当前这一楼**，之后判据根本不会再认成 fork。
    ...
    const archiveSeq = p === null ? null : p.archiveSeq
    const pre = p !== null && p.why === 'variant'
      ? `检测到${seat(toSeq)}换了变体（swipe 重 roll：${branch(p.fromVariantId)} → ${branch(p.toVariantId)}）`
      : `检测到回档到${seat(toSeq)}`
    const moved = `剧情刚从${seat(fromSeq)}退到${seat(toSeq)}`
      + (p !== null && p.why === 'variant' ? '（同一楼换了支）' : '')
      + `，档案停在${seat(Number.isFinite(archiveSeq) ? archiveSeq : null)}`
    const usedPrev = p === null || p.targetNodeId === ''
      ? ''
      : p.source === 'prev'
        ? `（${seat(p.toSeq)}这一支没记过 ⇒ 要退回的是"进这一楼之前"的样子，用的是${seat(p.targetSeq)}那一份）`
        : ''
    const howTo = p !== null && p.targetNodeId !== ''
      ? `要把剧情档案退回${seat(p.targetSeq)}得由你在面板上点（楼层清单里那一行的「回到这一楼」）`
      : `这一楼没有可退回的那一份（⛔ 不编一个目标出来）⇒ 那排只有「对齐楼层 / 只对齐楼号 / 不处理」三颗`
    return pre + `：` + moved + ` ⇒ ⛔ 这次一个字节都没写（回档不再自动跟随）：要不要把档案也退回去，`
      + howTo + usedPrev + '。⛔ 那 5 份一个字节都没动，⛔ 不影响会话，⛔ 预置那几份一个字都不动。'
  }
  if (kind === 'manual') {
    // ⚠️ 面板按**纯文本**渲染这一句 ⇒ ⛔ 不用 `**` 那种强调写法（星号会原样显示）。
...
function reportFloorReceipt(home, receipt, mode, head, log) {
  ...
  const fp = [
    ...
    // ★ 2026-09-24 收尾：判据那一层的两样也进指纹 —— 否则"**内容变了但指纹没变**"会被指纹门**静默吞掉**
    //   （例：剧情退到第 38 楼、档案从 34 挪到 30 ⇒ 那条 pending 变的可能只是 archiveSeq；或 fork 的
    //    判据从 forward 变成 fork）
    `fork:${floorStr(receipt?.fork?.kind) || '-'}`,
    `archive:${Number.isFinite(receipt?.archiveSeq) ? receipt.archiveSeq : '-'}`,
  ].join('|')
  ...
  if (rollbackFamily) {
    ...
    writeFloorStatus({
      ...
      //   ★ 收尾补：`archiveSeq`（判到那次 fork 时**档案停在第几楼**）与 `fork`（判据本身）也如实带上。
      pending: pend === null ? null : {
        targetKey: pend.targetKey, fromSeq: pend.fromSeq, toSeq: pend.toSeq, targetSeq: pend.targetSeq,
        archiveSeq: pend.archiveSeq ?? null, source: pend.source, why: pend.why, at: pend.at,
      },
      fork: floorStr(receipt?.fork?.kind) === '' ? null : receipt.fork.kind,
    })
  }
  try {
...
/**
 * 跑一次「楼同步」（三个时机共用；异步、**绝不抛**）。
 * @param {object|null} home `floorHomeFor` / `floorHomeBound` 的结果（`null` ⇒ 认不出周目，静默）
 * @param {'watch'|'turn'} mode `watch` = 组装那一脚 + ★ 面板开档那一脚（`POST …/floors/scan`）
 *   （判 fork、只记那条待处理 —— ★ 2026-09-24 起⛔ 不恢复）；`turn` = 轮末（还记一份快照）
 * ★★ `seen`（"上次认过的剧情位置"）**只在这里进出**：
 *   进来先读（`readSeenHead`）喂给 `syncFloor`；跑完按回执更新 ——
 *   `receipt.fork.kind` 是 `'forward'`（剧情往前走）/ `'no-seen'`（第一次见，建立基线）⇒ `seen ← head`；
 *   是 `'same'` / `'fork'` / 其它 ⇒ **不动**。⚠️ `'fork'` **不动**是关键：基准停在"上次认过的位置"，
 *   面板那条横幅才**挂得住**（不然下一轮就被判成 forward、横幅自己消失）。用户点「对齐楼层 / 只对齐楼号 / 不处理」之后
 *   由**那三条端点**把 `seen` 跟到 head（＝"算认过了"）。
 */
async function floorSyncAt(home, mode, log) {
  ...
  try {
    ...
    const headSeq = floorSnapshot.seqOfNode(seqMap, head === null ? '' : head.nodeId)
    const receipt = floorSnapshot.syncFloor({
      memoryDir: home.dir,
      mode,
      head,
      // ★ 判"开 fork"的基准（"上次认过的剧情位置"）；⛔ 认不出就当没有基线（第一次见 ⇒ 建立基线）。
      seen: readSeenHead(home.playthroughId),
      seq: headSeq,
    ...
    // ★ `seen` 只会往前：往前走 / 第一次见 ⇒ 跟上；开 fork 与同一楼 ⇒ **不动**（横幅才挂得住）。
    const forkKind = receipt?.fork?.kind
    if ((forkKind === 'forward' || forkKind === 'no-seen') && head !== null) {
      writeSeenHead(home.playthroughId, { nodeId: head.nodeId, variantId: head.variantId, seq: headSeq, at })
    }
...
// ---------------------------------------------------------------------------
// ★★ 2026-09-24：**主机侧盯时间线**（「回档 → 立刻弹」那条路）
//
// 用户口径：「**不能在回档操作之后立刻弹吗，检测 tarven**」。
// 机制（⛔ 全程不碰上游 Tavern 一行代码，只**读**它写的那个文件）：
//   `fs.watch` 盯住**面板绑定那个周目**的 `timeline.json` 所在**目录**（`./floor-watch.js`，
//   ⛔ 盯目录不盯文件 —— Windows 上"写完改名"很常见）⇒ 去抖后跑一次**与轮末、开面板同一份**检测
//   （`floorSyncAt(home, 'watch')`：判**开 fork**、记那条 `pending`、按回执跟 `seen`）⇒
//   判到开 fork 就**推一帧**给所有连着 SSE 的浏览器（`GET …/floors/events`）。
// 口径（用户拍板，⛔ 谁都不许"顺手做全"）：
//   · 只盯**面板绑定那一个**周目（`floorHomeBound()`，与 `/floors` 同源）——
//     ⛔ 本单**不做"多周目同时盯"**；绑定变了（`ensureFloorWatch` 认到的目录变了）就换盯的那一个；
//   · 回档那一脚**只会**写那条 `pending` 与我们自己的 `floor-head.json` ——
//     ⛔ **那 5 份正文一个字节都不写**（老口径不变；写盘仍只在用户点清单行「回到这一楼」之后）；
//   · ⛔ 不轮询、不定时器（这是**事件**）：`fs.watch` 那一下、外加 SSE 那条 25 秒的**注释**心跳。
// ---------------------------------------------------------------------------

/** 当下盯着的目录 + 拆它的那一脚（⛔ 只有一处；`ensureFloorWatch` 是唯一改它的地方）。 */
let floorWatchState = { dir: '', dispose: null }

/**
 * ★ 把"盯住**当前**面板绑定那个周目的时间线"这一脚对上（**幂等**）。
 *
 * 为什么要幂等且要"能换目标"：面板绑定的周目会变（用户换绑定 / 一开始还没绑），而 `fs.watch`
 * 是**盯着一个目录**的 —— 绑定的周目一变，盯着的那个目录就得跟着换（否则盯的是别人的时间线）。
 *
 * 三个时机调它（都是**事件**，⛔ 不是轮询）：① 接线时（`registerFloorSnapshot` 的 effect）；
 *   ② 有浏览器连上 SSE 时（顺手对一次 —— 用户开了页面说明他要看提示）；③ 换绑之后第一次连上时。
 *
 * ⛔ 认不出周目（还没记忆库 / 还没绑）⇒ **不盯**（安静：拿不到"哪一楼"就没有可判的），
 *   并把**旧的拆掉**；⛔ 盯不住（目录不存在 / 权限）⇒ `watchTimeline` 自己如实 `warn` 一条
 *   （那条日志正是"这一档现在只能靠轮末 / 开面板发现"的如实说明，⛔ 不许静默）。
 */
function ensureFloorWatch(log) {
  if (floorWatch === null || floorSnapshot === null) return { dir: '', watching: false }
  const home = floorHomeBound()
  const file = home === null ? '' : playthroughTimelineFile(home.characterId, home.playthroughId)
  const dir = file === '' ? '' : dirname(file)
  if (dir === '') {
    // 还没有可盯的周目（或时间线路径认不出来）⇒ 把旧的拆掉，⛔ 不盯空。
    if (floorWatchState.dispose !== null) {
      try { floorWatchState.dispose() } catch {}
      floorWatchState = { dir: '', dispose: null }
    }
    return { dir: '', watching: false }
  }
  if (floorWatchState.dir === dir && floorWatchState.dispose !== null) return { dir, watching: true }
  // 换目标了（或第一次）⇒ **先拆旧的再盯新的**（⛔ 不留野 watcher）。
  if (floorWatchState.dispose !== null) {
    try { floorWatchState.dispose() } catch {}
    floorWatchState = { dir: '', dispose: null }
  }
  const dispose = floorWatch.watchTimeline(dir, {
    watch: floorWatchImpl,
    file: basename(file),
    onChange: () => { void Promise.resolve(floorWatchKick(log)).catch(() => {}) },
    log,
  })
  floorWatchState = { dir, dispose }
  return { dir, watching: true }
}

/** 拆掉（随插件卸载 / 热重载）—— 幂等。 */
function disposeFloorWatch() {
  if (floorWatchState.dispose !== null) {
    try { floorWatchState.dispose() } catch {}
  }
  floorWatchState = { dir: '', dispose: null }
...
/**
 * ★★ **时间线一动就现算一次**（那一脚与轮末、开面板**同一份**检测）⇒ 判到**开 fork** 就推一帧。
 *
 * ⚠️ 绑定的周目**现算**（`floorHomeBound()`）—— 上面 `ensureFloorWatch` 盯的目录可能已经换成新的了，
 *   这一脚不能拿"盯的时候那个"接着用。
 * ⛔ 绝不抛（`ensureFloorWatch` 那边已经 `catch` 了一手）；⛔ 一个字节的正文都不写。
 */
async function floorWatchKick(log) {
  if (floorSnapshot === null) return null
  const home = floorHomeBound()
  if (home === null) return null
  const receipt = await floorSyncAt(home, 'watch', log)
  if (receipt === null || receipt === undefined || receipt?.fork?.kind !== 'fork') return receipt
  // ★ 推的那一帧里的 `pending` 与 `/floors` 里那条**同一份拼装**（`floorsView`，⛔ 不许两处各拼一份）。
  //   ★ 2026-09-25：面板要显示的三个**序号**也搭这一帧过去（`pendingOrdinals`；⛔ 不改 `pending` 一个字 ——
  //   它与盘上那条**逐字一致**是硬判据）。
  const view = floorsView(home)
  const frame = {
    type: 'fork',
    pending: isPlainObject(view) && view.pending !== undefined ? view.pending : null,
    pendingOrdinals: isPlainObject(view) && view.pendingOrdinals !== undefined ? view.pendingOrdinals : null,
    at: typeof receipt.at === 'string' && receipt.at !== '' ? receipt.at : new Date().toISOString(),
  }
  floorSseLastFrame = frame      // ★ 自检台的观测口（见 `__floorSseLastFrame`；⛔ 不改推送行为）
  floorSseHub(log).push(frame)
  return receipt
}
...
/**
 * 接线：楼层快照 + **开 fork 提示**（手动挡）。
 *
 * 三条纪律（照死区看守与自动收纳那两条既有的钩子抄，⛔ 不新造调度）：
 *   · **组装那一脚绝不阻塞轮次**：监听里一个字都不 await，装完就 `return next()`；
 *     判 fork（+ 记那条待处理 + 按回执跟 `seen`）排到本轮之外再跑（`setTimeout(…, 0)` + `unref`，
 *     `void … .catch()`，绝不抛），**每轮至多一次**（门 = `turnBoundary.lastTurn`，取不到退到最小间隔）。
 *     ★ 这一脚的意义：用户一开 fork（回档），**本轮之内**那条横幅就已经在清单里了（不等轮末）。⛔ 它不写那 5 份。
 ...
 *   ★ 2026-09-24 收尾：还有**第三脚** —— 面板开档时的 `POST …/floors/scan`（补掉"回档之后没发消息、
 *     直接开面板看"那个时序洞）；它复用同一个 `floorSyncAt(…, 'watch', …)`，⛔ 不另立一套编排。
 */
function registerFloorSnapshot(ctx, log) {
  ...
  ctx.plugin({
    ...
    apply(scope) {
      ...
      // ★★ 2026-09-24：**盯住时间线**（「回档 → 立刻弹」那条路，见上面那一大段口径）。
      //   · 接线这一刻先把"盯住面板绑定那个周目的 timeline.json"对上（认不出周目 ⇒ 不盯，安静）；
      //   · 断线重连/换绑之后，有浏览器连上 SSE 时 `handleRpMemoryFloorsEvents` 会再对一次（幂等）；
      //   · **必须随卸载拆掉**（⛔ 不留野 watcher —— 热重载会重建，不拆就是每热载一次多盯一个）。
      if (floorWatch === null) {
        log.warn(`[mt] 楼层快照：即时检测（盯时间线）未接线（${floorWatchLoadError?.message || floorWatchLoadError}）`
          + ' ⇒ 回档提示只能靠轮末 / 开面板发现（其余功能不受影响）')
      } else {
        scope.effect(() => {
          ensureFloorWatch(log)
          return () => { disposeFloorWatch() }
        }, 'floor-watch.dispose()')
      }
    },
  })
  log.info('[mt] 楼层快照已接线（轮末记一份；组装时 + 轮末 + 面板开档时各判一次**开 fork** —— ★ 判到只提示、⛔ 不自动写；'
    + '写盘只在用户点了「回到这一楼」（楼层清单每一行）或「对齐楼层」（★ 2026-09-26 起弹窗那颗也走它）时走 POST …/floors/restore；'
    + '★ 盯住面板绑定那个周目的 timeline.json（fs.watch，⛔ 不轮询）⇒ 判到开 fork 立刻推一帧给 SSE（GET …/floors/events）；'
    + '只动那 5 份笔记 + .dma-floor-snapshots/ + 我们自己的 floor-head.json，⛔ 预置那几份一个都不碰、死区那几段按块合并（保留盘上现况））')
}
 ...
 *   **判到开 fork 只提示、⛔ 不写盘** ⇒ 响应里多带一条 `pending`（没有就 `null`）—— 面板靠它画档顶那条
 *   **显著**横幅（★ 2026-09-25 起那一排是「对齐楼层」/「只对齐楼号」/「不处理」三颗 ＋ ✕）。⛔ 本端点仍然**纯只读**。
 ...
 *   ★★ 2026-09-25（用户口径「**插件自己按时间编一个序号，不论实际seq，这样反而符合直觉，序号最大的就是
 *   最新生成的**」）：面板上显示的**不再是那个楼号**，而是宿主这里现算的 **`ordinal`** ——
 *   把这一周目**全部**楼层行按记录时间（行上的 `at`）**升序**编号 `1..N`（`N` = 最新），
 *   清单也**按它从大到小**排（最新在最上）。`seq` **照旧照带**（锚、判据、状态文件还用它），
 *   只是**面板不再显示**它；`at` 认不出 ⇒ `ordinal:null`（面板写「序号未知」，⛔ 不编 0）。
...
function handleRpMemoryFloors(send) {
  ...
  return send(200, floorsView(floorHomeBound()))
}

/**
 * ★★ 2026-09-24 收尾：`POST /playthrough/rp-memory/floors/scan` —— 「**开面板就检测一次**」。
 *
 * 为什么要它（真机时序洞）：检测原先只在**轮边界**跑（组装那一脚 / 轮末那一脚）⇒ 回档之后
 * **没发消息、直接开面板看**的那一次检测根本没发生 ⇒ 横幅看不到。面板每次取楼层都走本端点补一脚。
 * ⚠️ 语义是**会写**（那条 `pending` 与**我们自己的** `floor-head.json` 可能变）⇒ 所以是 POST；
 *   ⛔ 但**碰都不碰**那 5 份正文（⛔ 一个字节都不写 —— 写盘仍只在 `…/floors/restore` 那一脚）。
 * ⚠️ 响应体 = **与 `GET /floors` 同一份**（`floorsView`，⛔ 不两处各拼一遍）。
 */
async function handleRpMemoryFloorsScan(req, send, log) {
  if (floorSnapshot === null) {
    return send(200, err('RP_MEMORY_FLOOR_UNAVAILABLE', `楼层快照模块（floor-snapshot.js）不可用：${floorSnapshotLoadError?.message || floorSnapshotLoadError}`))
  }
  const body = await readJsonObjectBody(req, send)
  if (body === null) return undefined
  const home = floorHomeBound()
  // 'watch' 那一脚：判开 fork（+ 按回执把 `seen` 往前跟），⛔ 不记录、⛔ 不写那 5 份。
  if (home !== null) await floorSyncAt(home, 'watch', log)
  return send(200, floorsView(home))
}

/**
 * ★★ 2026-09-25（用户口径，逐字：「**开一个下钻，直接显示具体改动的字段，写改了哪些可以说完全没用**」）：
 * `GET /playthrough/rp-memory/floors/diff?nodeId=&variantId=&against=` —— 楼层清单每一行那颗
 * 「**看改动 ▸**」的取数口。**只读**：⛔ 不写任何文件、⛔ 不跑检测、⛔ 一个字节的正文都不动。
 *
 * 比的是谁：**这一条 vs 时间上紧随其前的那一条**（按 `ordinal` 口径 —— 就是"序号比它小一号"的那一条）。
 *   `against` 缺省 = 前一条；**给了**就比那一条（`against` = 那条的 `nodeId`，`againstVariant` = 它的
 *   `variantId`；不给 `againstVariant` 就取那一楼的**第一条**条目）—— 这是给"跟任意一条比"的将来留的口子，
 *   本单面板只用缺省那种。⚠️ 没有前一条（最早的那一条）⇒ **不是错误**：`ok:true` + `against:null` +
 *   一句"这是最早的一条，没有可比的"（面板照实画，⛔ 不画成"没改动"）。
 *
 * 内容 = **每一份文件真正变了什么**（增行 / 删行，带一点点上下文）—— 判据全在纯函数里
 * （`floorSnapshot.diffFloorFiles`，⛔ 不把 diff 算法写在这里：那样台子只能起服务器测）。
 * 正文从**楼层快照的 blob** 里读（`readBlob`，与 `handleRpMemoryFloorRestore` 那条路**同一处**）。
 *
 * ⛔ 按需取：面板**点了那一颗**才发这一个请求（⛔ 不轮询、⛔ 不挂定时器、⛔ 开档时不预取）。
 * 读侧口径照 `/floors`：HTTP 200 + `ok:false` + 可读原因（⛔ 不许静默、⛔ 不许画成"没改动"）。
 */
function handleRpMemoryFloorDiff(send, url) {
  if (floorSnapshot === null) {
    return send(200, err('RP_MEMORY_FLOOR_UNAVAILABLE', `楼层快照模块（floor-snapshot.js）不可用：${floorSnapshotLoadError?.message || floorSnapshotLoadError}`))
  }
  const home = floorHomeBound()
  if (home === null) {
    return send(200, err('RP_MEMORY_MISSING', '这个周目还没有记忆库（.roleplay-memory/）⇒ 还没有可看的楼层改动'))
  }
  const nodeId = url.searchParams.get('nodeId') || ''
  const variantId = url.searchParams.get('variantId') || ''
  if (nodeId === '') {
    return send(200, err('RP_MEMORY_FLOOR_DIFF_BAD_QUERY', '要看哪一条的改动得带 nodeId（清单里那一行的 id）'))
  }
  const read = floorSnapshot.readIndex(home.dir)
  if (read.error !== null) {
    return send(200, err('RP_MEMORY_FLOOR_INDEX_' + String(read.error).toUpperCase(),
      `楼层快照清单读不出来（${read.error}）⇒ 看不了改动（⛔ 不拿一份读不出来的清单糊弄）`))
  }
  const target = floorSnapshot.floorOfVariant(read.index, nodeId, variantId)
  if (target === null) {
    return send(200, err('RP_MEMORY_FLOOR_DIFF_NO_FLOOR', '这一条不在楼层清单里（可能还没记过）⇒ 没有可看的改动'))
  }
  const order = floorSnapshot.ordinalOrderOf(read.index.floors)
  const ordinals = floorSnapshot.ordinalMapOf(read.index.floors)
  const keyOf = (row) => floorSnapshot.floorKeyOf(row?.nodeId, row?.variantId)
  const at = order.findIndex((x) => keyOf(x.floor) === keyOf(target))
  const targetOrdinal = at === -1 ? null : order[at].ordinal
  // 跟谁比：缺省 = 前一条（升序表里前面那一条）；给了 `against` = 那一楼那一条。
  const wantAgainst = url.searchParams.get('against') || ''
  const wantAgainstVariant = url.searchParams.get('againstVariant') || ''
  let against = null
  let againstSource = 'prev'
  if (wantAgainst === '') {
    against = at > 0 ? order[at - 1].floor : null
  } else {
    againstSource = 'named'
    against = wantAgainstVariant === ''
      ? floorSnapshot.floorOf(read.index, wantAgainst)
      : floorSnapshot.floorOfVariant(read.index, wantAgainst, wantAgainstVariant)
    if (against === null) {
      return send(200, err('RP_MEMORY_FLOOR_DIFF_NO_AGAINST',
        `要跟它比的那一条（${wantAgainst}${wantAgainstVariant === '' ? '' : ' · ' + wantAgainstVariant}）不在楼层清单里 ⇒ 比不了`))
    }
    if (keyOf(against) === keyOf(target)) {
      return send(200, err('RP_MEMORY_FLOOR_DIFF_SELF', '这一条跟它自己比不了（要比的是另一条）'))
    }
  }
  // 死区那一份判据（⛔ 唯一判据仍是 `.dma-deadzones.json`）；读不出来 ⇒ 如实标 `deadzonesReadable:false`。
  const zones = floorDeadZones(home.dir)
  const deadFiles = zones === null ? [] : [...new Set(zones.map((z) => z.file))]
  // 正文：内容寻址的 blob（同一 sha 只读一次 —— 两份楼层里同文的那些份天然共用）。
  const cache = new Map()
  const textOf = (sha) => {
    if (typeof sha !== 'string' || sha === '') return null
    if (!cache.has(sha)) cache.set(sha, floorSnapshot.readBlob(home.dir, sha).text)
    return cache.get(sha)
  }
  const out = against === null
    ? { files: [], truncated: false, added: 0, removed: 0 }
    : floorSnapshot.diffFloorFiles({ target, base: against, textOf, deadFiles })
  const seat = (n) => (Number.isFinite(n) ? `序号 ${n}` : '序号未知')
  const againstOrdinal = against === null ? null : floorSnapshot.ordinalOfFloor(against, ordinals)
  const message = against === null
    ? '这是最早的一条，没有可比的'
    : `${seat(targetOrdinal)} 与 ${seat(againstOrdinal)} 比（${againstSource === 'prev' ? '时间上紧接着它的那一条' : '你指定的那一条'}）`
  const brief = (row) => (row === null ? null : {
    nodeId: row.nodeId, variantId: row.variantId, seq: row.seq ?? null, at: row.at ?? null,
    legacy: floorSnapshot.isLegacyFloor(row),
    ordinal: floorSnapshot.ordinalOfFloor(row, ordinals),
  })
  return send(200, {
    ok: true,
    base: home.base, baseLabel: home.baseLabel, sharedHint: home.base === 'workspace-root',
    dir: floorSnapshot.FLOOR_DIR_NAME,
    // 「这一条是序号几 / 比的是序号几」——面板照着说人话（⛔ 不编：认不出就是 `null`）。
    floor: brief(target),
    against: brief(against),
    againstSource,
    deadzonesReadable: zones !== null, deadFiles,
    files: out.files, truncated: out.truncated, added: out.added, removed: out.removed,
    message,
    checkedAt: new Date().toISOString(),
  })
}
...
/**
 * 楼层清单那一段 body（★ 拼装**只此一处**：`GET /floors` 与 `POST /floors/scan` 共用同一份）。
 * ⛔ 纯读盘 + 纯投影，一个字节都不写。
 * @param {object|null} home `floorHomeBound()` 的结果（`null` = 认不出周目/还没有记忆库 ⇒ 读侧老口径）
 */
function floorsView(home) {
  if (home === null) {
    return {
      ok: false, exists: false, dir: RP_MEMORY_DIR_NAME,
  ...
  // ★★ 2026-09-25（用户口径「**插件自己按时间编一个序号，不论实际seq**」）：面板上那个「序号 N」。
  //   判据**只此一处**（`floorSnapshot.ordinalOrderOf`）：全部楼层行按**记录时间**（行上的 `at`）升序
  //   编号 `1..N`（N = 最新）；并列用既有那个确定性次序破；老记录照样进；`at` 认不出 ⇒ 排最后、序号
  //   如实为 `null`（面板写「序号未知」，⛔ 不编 0）。⚠️ 那个 `seq` **一个字节都不许改语义**（判 fork /
  //   对齐 / 状态文件全还靠它）—— 两个号各管一段，⛔ 谁都别替谁。
  const ordinalOrder = floorSnapshot.ordinalOrderOf(read.index.floors)
  const ordinals = floorSnapshot.ordinalMapOf(read.index.floors)
  const floors = read.index.floors.map((f) => {
    ...
    return {
      ...
      // ★ 2026-09-25：`ordinal` = 面板上那个「序号 N」（认不出时间 ⇒ `null` = 面板写「序号未知」）。
      ordinal: floorSnapshot.ordinalOfFloor(f, ordinals),
      legacy, current,
      // ★ 2026-09-25：**档案指针（`last`）指着的那一行** —— 默认只画最近 10 行时，这一行**必须可见**
      //   （判"是不是它"的活儿在宿主这边做：⛔ 面板不许自己拿 nodeId 去比）。
      //   ⚠️ 老记录（`variantId` 空）**也算**：只要 `last` 指的就是它，它就是"档案停在这一条"。
      pointer: read.index.last !== null
        && f.nodeId === read.index.last.nodeId && f.variantId === read.index.last.variantId,
      // 同一楼、不是这一支（swipe 掉的另一支）—— 面板据此说清"这几条是同一楼的两种分支"。
    ...
    // ★★ 2026-09-25：清单**按 `ordinal` 从大到小**（最新在最上）。判据在宿主：⛔ 面板不排序、不比新旧。
    //   `at` 认不出的那几条没有号（`null`）⇒ **排在最后**（它们连"第几版"都说不清，⛔ 不混进有号的那一段）。
    const oa = Number.isFinite(a.ordinal) ? a.ordinal : null
    const ob = Number.isFinite(b.ordinal) ? b.ordinal : null
    if (oa !== null && ob !== null && oa !== ob) return ob - oa
    if (oa === null && ob !== null) return 1
    if (oa !== null && ob === null) return -1
    // 两个都没号（认不出时间）⇒ 用既有那个确定性次序（nodeId → variantId → at）—— 与 `ordinalOrderOf` 同一份。
    return floorSnapshot.compareFloorOrder(a, b)
  })
  ...
  // ★ 档案停在哪一楼（面板档顶那条**常驻状态行**的「档案 第 Y 楼」）：
  //   优先用记录当时写下的 `seq`；老记录没写 ⇒ 拿它那一楼在**时间线**里的位置兜底（⛔ 不许编一个 0）。
  //   ★ 2026-09-25：面板显示的是它的 **`ordinal`**（「序号 N」）—— 指针指着的那一行**不在清单里**
  //   （或那一行没 `at`）⇒ 如实 `null`（面板写「序号未知」）。
  const lastSeq = read.index.last === null
    ? null
    : (read.index.last.seq === null ? floorSnapshot.seqOfNode(seqMap, read.index.last.nodeId) : read.index.last.seq)
  // ★ 「剧情」那一侧同理：`head` 站的那一行若在清单里 ⇒ 它的 `ordinal`；不在 ⇒ `null`（「序号未知」）。
  const headRow = head === null ? null : floorSnapshot.floorOfVariant(read.index, head.nodeId, head.variantId)
  // ★★ 那条**待处理**（横幅与浮层要说的那三个位置）也换成序号口径 —— ⚠️ **但那条账本身一个字都不许动**：
  //   `pending` 照旧**逐字**投影（SSE 那一帧里的那条与它**逐字一致**是硬判据：
  //   `_selftest-floor-watch.mjs` 的 C7c 就钉着这个）⇒ 序号走**旁边那个新字段** `pendingOrdinals`。
  //   `from*` = 剧情**之前**站的楼（`seen`）、`to*` = 剧情**现在**站的楼、`archive*` = **档案停在哪一楼**；
  //   三个都按 `(nodeId, variantId)` 在清单里查 `ordinal`；查不到（那一楼没有行 / 那行没 `at`）⇒ `null`
  //   （面板照实写「序号未知」，⛔ 不编 0）。
  const pending = read.index.pending === null || read.index.pending === undefined ? null : read.index.pending
  const pendingOrdinals = pending === null ? null : {
    from: floorSnapshot.ordinalOfFloor({ nodeId: pending.fromNodeId, variantId: pending.fromVariantId }, ordinals),
    to: floorSnapshot.ordinalOfFloor({ nodeId: pending.toNodeId, variantId: pending.toVariantId }, ordinals),
    archive: floorSnapshot.ordinalOfFloor({ nodeId: pending.archiveNodeId, variantId: pending.archiveVariantId }, ordinals),
  }
  return {
    ok: true,
    base: home.base, baseLabel: home.baseLabel, sharedHint: home.base === 'workspace-root',
    ...
    // ★ 2026-09-25：`ordinal` = 面板上显示的「序号 N」（认不出 ⇒ `null`）；`seq` 照旧带上（锚与判据还用它）。
    head: head === null ? null : {
      nodeId: head.nodeId, variantId: head.variantId, seq: headSeq,
      ordinal: headRow === null ? null : floorSnapshot.ordinalOfFloor(headRow, ordinals),
    },
    ...
    last: read.index.last === null ? null : {
      ...
      seq: lastSeq, at: read.index.last.at,
      ordinal: lastFloor === null ? null : floorSnapshot.ordinalOfFloor(lastFloor, ordinals),
    },
    ...
    floors,
    // ★★ 2026-09-24：那条**待处理**（判到开 fork 时记的；没有 ⇒ `null`）—— 面板档顶那条显著横幅用。
    //   字段（收尾口径）：`from*` = **剧情之前站在哪一楼**（`seen`）、`to*` = **剧情现在站在哪一楼**（head）、
    //   `archive*` = **档案停在哪一楼**（last）、`target*` = 要写回的那一份快照（**可以没有**：那时三个都空）、
    //   `targetKey` = "同一次 fork"的判据、`source`('self'|'prev')、`why`('back'|'variant')、`at`。
    //   ⚠️ 这一条**逐字**照账投影（⛔ 一个字段都不加、不改）；★ 2026-09-25 面板要显示的那三个**序号**
    //   走旁边那个 **`pendingOrdinals`**（`{from, to, archive}`；认不出 ⇒ `null`，⛔ 不编 0）。
    pending,
    pendingOrdinals,
    checkedAt: new Date().toISOString(),
  }
...
/**
 * ★★ 2026-09-24：`GET /playthrough/rp-memory/floors/events` —— **我们自己的 SSE 长连**（推送面）。
 *
 * 为什么要它（用户口径「**不能在回档操作之后立刻弹吗，检测 tarven**」）：面板档顶那条横幅只在
 * **打开面板**时才看得到，而检测原先只在轮边界跑 ⇒ 回档之后没发消息时那条提示还没被记下来。
 * 现在主机侧盯住 Tavern 写的 `timeline.json`（`./floor-watch.js` + `ensureFloorWatch`），
 * 一判到**开 fork** 就往这条长连上推一帧 —— 浏览器那侧（常驻挂载的侧边栏组件）收到就从**上方**
 * 滑下一条浮层。
 *
 * 为什么是 SSE 而不是别的：DSH 给客户端插件的能力里**没有"服务器推浏览器"的通用通道**
 * （`inject` 只有 attachments/conversation/…/slots/workspaces 那些）⇒ 光靠客户端只能轮询，
 * 而用户口径里**不许轮询**。这台 web 服务本身支持 SSE 长连（DSH 自己的插件事件通道
 * `/plugins/events` 就是一条 SSE），而我们的路由拿到的是**原始 `res`**
 * （`webServer.register({kind:'prefix', …})`）⇒ 我们可以自己写一条，⛔ 不借用 DSH 那条。
 *
 * 纪律（六条，⛔ 一条都不许松）：
 *   · 头：`text/event-stream` + `no-cache` + `keep-alive` + `x-accel-buffering: no`（别让反代攒着）；
 *   · **连上先发一帧 `hello`**（带**现状**：`pending` / `head` / `last`）—— 与 `/floors` **同一份拼装**
 *     （`floorsView`，⛔ 不许两处各拼一份）；EventSource 自己会重连 ⇒ 重连之后自然对齐；
 *   · 帧格式 `data: <JSON>\n\n`（只用默认 message 事件）；
 *   · **心跳**：每 25 秒一帧**注释**（`:\n\n`）防空闲超时（⛔ 不拿 JSON 帧刷屏）；
 *   · `res.on('close')` ⇒ 从订阅者集合里**摘掉**（⛔ 不许越挂越多）；订阅者**设上限**（默认 8），
 *     超了如实回 503 + 一句人话（⛔ 不许静默丢弃）；
 *   · ⛔ **纯推送、不写任何东西**（连"看一眼现状"也不顺手跑检测 —— 写盘只在 fs.watch 那一脚与既有那几条端点）。
 */
const FLOOR_SSE_MAX_SUBSCRIBERS = 8
/** 心跳间隔（毫秒）：只发注释帧，⛔ 不是轮询、⛔ 不发 JSON。 */
const FLOOR_SSE_HEARTBEAT_MS = 25 * 1000

/**
 * 订阅者集合（★ 唯一一处）：`{ res, beat }` 的 Set，上限 `FLOOR_SSE_MAX_SUBSCRIBERS`。
 * dispose 时把每条连接**关掉**并清空 —— 热重载/卸载不留悬挂连接、不留幽灵订阅者。
 */
function createFloorEventHub(log) {
  const subs = new Set()
  const drop = (sub) => {
    if (sub.beat !== null) {
      try { clearInterval(sub.beat) } catch {}
      sub.beat = null
    }
    subs.delete(sub)
  }
  return {
    size: () => subs.size,
    /** 收下一跳：超上限 ⇒ `{ok:false, reason:'full'}`（调用方如实回 503，⛔ 不许静默丢弃）。 */
    add(res) {
      if (subs.size >= FLOOR_SSE_MAX_SUBSCRIBERS) return { ok: false, reason: 'full' }
      const sub = { res, beat: null }
      subs.add(sub)
      const beat = setInterval(() => {
        try {
          if (res.writableEnded === true || res.destroyed === true) return drop(sub)
          res.write(':\n\n')     // ★ 注释帧：Safari 那类"收到非注释帧才认连接活着"的客户端会当真数据
        } catch { drop(sub) }
      }, FLOOR_SSE_HEARTBEAT_MS)
      if (beat && typeof beat.unref === 'function') beat.unref()
      sub.beat = beat
      return { ok: true, sub }
    },
    remove: (sub) => drop(sub),
    /** 推一帧给**所有**连着的人（⛔ 不是"只推第一个"）；推不动的（断了/抛了）当场摘掉。 */
    push(frame) {
      let sent = 0
      let text = ''
      try { text = 'data: ' + JSON.stringify(frame) + '\n\n' } catch { return 0 }
      for (const sub of Array.from(subs)) {
        try {
          const res = sub.res
          if (res.writableEnded === true || res.destroyed === true) { drop(sub); continue }
          res.write(text)
          sent += 1
        } catch { drop(sub) }
      }
    ...
    /** 随插件卸载：把所有连接关掉、清空集合（幂等）。 */
    dispose() {
      for (const sub of Array.from(subs)) {
        try { sub.res.end() } catch { /* 已经断了的就没什么可关的 */ }
        drop(sub)
      }
      subs.clear()
      try { log?.info?.(`[mt] 楼层推送：SSE 端点随插件卸载关掉了全部连接（共 ${FLOOR_SSE_MAX_SUBSCRIBERS} 上限，清空订阅者集合）`) } catch {}
    },
  }
}

/** 订阅者集合的**唯一实例**（随模块实例走：热重载会重建一个新的，旧的在那条路由的 dispose 里关掉）。 */
let floorSseHubInstance = null
function floorSseHub(log) {
  if (floorSseHubInstance === null) floorSseHubInstance = createFloorEventHub(log)
  return floorSseHubInstance
}
/** 已经建过的那个（没建过 ⇒ `null`）—— dispose 用它，⛔ 不为了关掉而凭空建一个。 */
function floorSseHubIfAny() {
  return floorSseHubInstance
}

/** 仅自检台用：那条 SSE 端点当下的订阅者数（"断开的连接真的摘掉了"要有数可数）。 */
export function __floorSseStats() {
  return { subscribers: floorSseHubIfAny() === null ? 0 : floorSseHubIfAny().size() }
}

/**
 * ★ 2026-09-25（复核那一单时加）：**最近推出去的那一帧**（自检台的观测口，与 `__floorSseStats`
 *   同一类：生产里就是记一个变量，⛔ 不影响任何行为）。
 *
 * 为什么要它：`_selftest-floor-diff.mjs` 的 F1 要钉的是「**推给浮层的那一帧里也带着三个序号**」——
 *   那是**宿主侧的拼装**。原先它是靠"台子里真连一条 SSE、等客户端收帧"来验的，实测（2026-09-25）：
 *   宿主确实把它写给了在连的那一个订阅者（探针里 `wrote id=1 bytes=534`、`subscribers:1`），
 *   但**这一条台子的假服务器 + undici 客户端**在那条流上只收到过 hello 那一块（隔离验过：
 *   同样的两段式写、同样的头，在最小例子里两块都收得到 ⇒ 是这条台子的链路的怪癖，不是宿主逻辑）。
 *   ⇒ 这条判据改成读**这一帧本身**（确定性），⛔ 不再依赖跨进程流式的时序。
 */
let floorSseLastFrame = null
export function __floorSseLastFrame() {
  return floorSseLastFrame
}

/**
 * ★★ 2026-09-24：`GET …/floors/events` 的处理函数（SSE）。
 * ⚠️ **不走** `send()` 那条路（它是"JSON + end"，会立刻把连接关掉）—— 这里拿**原始 `res`** 自己写头、
 *   自己写帧；只有"超上限被拒"那一支才用 `send`（那一次本来就该是"回完就关"）。
 */
function handleRpMemoryFloorsEvents(req, res, send, log) {
  if (floorSnapshot === null) {
    // 楼层快照模块缺席 ⇒ 这条端点没有意义：**不接进长连**，走既有那条"可读错误"的口径（HTTP 200 + ok:false）。
    return send(200, err('RP_MEMORY_FLOOR_UNAVAILABLE', `楼层快照模块（floor-snapshot.js）不可用：${floorSnapshotLoadError?.message || floorSnapshotLoadError}`))
  }
  const hub = floorSseHub(log)
  if (hub.size() >= FLOOR_SSE_MAX_SUBSCRIBERS) {
    // ⛔ 不许静默丢弃：如实说清"满了"、"为什么"、"怎么办"。
    return send(503, err('FLOOR_EVENTS_BUSY',
      `楼层推送这条长连已经挂满了（上限 ${FLOOR_SSE_MAX_SUBSCRIBERS} 条）⇒ 这次没接上。`
      + `关掉几个开着的 DSH 页面再刷新即可（这条长连是"回档立刻弹提示"用的，不影响别的功能）。`))
  }
  try {
    res.writeHead(200, {
  ...
  // ★ 先挂进订阅者集合再发 hello：帧是**写序**的，这一整段同步跑完，中间不可能插进别的一帧。
  const added = hub.add(res)
  if (added.ok !== true) {
    // 理论到不了这里（上面已经查过上限）：真到了就如实收尾，⛔ 不留半条连接。
    try { res.end() } catch {}
    return undefined
  }
  const sub = added.sub
  /**
   * 断开（客户端关页/断网/我们被卸载）⇒ **摘掉**（⛔ 不许越挂越多）。
   * ⚠️ 摘的时候也要把那个 25 秒心跳停掉（`remove` 负责）。
   */
  const onClose = () => { hub.remove(sub) }
  try {
    res.on('close', onClose)
    res.on('error', () => { try { res.destroy() } catch {} })
  } catch { /* 挂不上监听也要接着走（至少 hello 得发出去） */ }
  try {
    // ★ hello 那一帧的"现状" = **与 `/floors` 同一份拼装**（`floorsView`，⛔ 不许两处各拼一份）。
    //   ⛔ 这一次**不跑检测**（那会写盘 —— 本端点纯推送、一个字节都不写）。
    const home = floorHomeBound()
    // ★ 顺手把"盯住当前绑定的那个周目"这一脚对上（幂等、只读；⛔ 不是轮询 —— 只在**连上来**这一刻做一次）。
    ensureFloorWatch(log)
    const view = floorsView(home)
    const pending = isPlainObject(view) && view.pending !== undefined ? view.pending : null
    res.write('data: ' + JSON.stringify({
      type: 'hello',
      pending: pending === undefined ? null : pending,
      // ★ 2026-09-25：与 `/floors` 同一份里的那三个**序号**（面板/浮层显示「序号 N」用；⛔ 不编 0）。
      pendingOrdinals: isPlainObject(view) && view.pendingOrdinals !== undefined ? view.pendingOrdinals : null,
      head: isPlainObject(view) && view.head !== undefined ? view.head : null,
      last: isPlainObject(view) && view.last !== undefined ? view.last : null,
    }) + '\n\n')
  } catch {
    try { res.end() } catch {}
    hub.remove(sub)
...
/**
 * ★★ 2026-09-23：`POST /playthrough/rp-memory/floors/restore` —— 「**回到这一楼**」（楼层清单每一行）
 * ★★ 2026-09-26：**弹窗那颗「对齐楼层」也走这一脚**（用户口径「不是不触发弹窗，是弹窗上点同步后，
 *   没有触发真回档」⇒ 那颗改成"先真回档、再记账"：把那条 pending 的 `target*` 当目标发过来）——
 *   ⛔ 本端点**一行都没改**（它本来就顺手锚 `last` ＋ 清 pending，"记账"那一半是现成的）。
...
async function handleRpMemoryFloorRestore(req, send, log, redact) {
  ...
  // ★★ 2026-09-24 收尾：这一脚**真做成了** ⇒ 算用户"认过了当前这个剧情位置"⇒ `seen ← head`
  //   （口径见文件头 ⑤：点了「退回」也算认过；不然下一轮还按旧基准判开 fork、横幅自己弹回来）。
  //   ⛔ 有哪一份没写进去 / 被死区挡住 ⇒ **不认**（那一脚没做成，`seen` 留在原地，下次接着提示）。
  const restoredOk = (rest?.failed ?? []).length === 0 && (rest?.blocked ?? null) === null
  if (restoredOk) floorAcknowledge(home)
  const view = {
    base: home.base, baseLabel: home.baseLabel, sharedHint: home.base === 'workspace-root',
...
/**
 * ★★ 2026-09-24：`POST /playthrough/rp-memory/floors/pending/settle` —— **仍走"就地登记"**的那两颗「对齐」：
 * 「**对齐楼层**」（`mode:'full'`，＝从前的"就地登记 / 保持现状"）与 ★ 2026-09-25 新增的
 * 「**只对齐楼号**」（`mode:'number'`）。⚠️ 名字里的 `dismiss` 是**面板那颗按钮的动作**（把横幅处理掉），
 * ⛔ 不是"什么都不做"—— 具体做什么全在纯函数 `floorSnapshot.settlePending` 里。
 * ★★ 2026-09-26 归属修正（用户口径「不是不触发弹窗，是弹窗上点同步后，没有触发真回档」）：
 *   **弹窗（顶部浮层）那颗「对齐楼层」已改走 `/floors/restore`**（"先真回档、再记账"）
 *   ⇒ 这一脚只剩两处在调：**档顶横幅**那两颗与**档顶常驻状态行**那颗（**那两处仍是"就地登记"**）。
 *   ⚠️ 因此"「对齐楼层」＝就地登记"这句话**只对那两处成立**了（两处的悬停说明也各说各的）。
 *
 * 判据全在 `settlePending`，**按 `mode` 分两种**（其余判据两种模式**一模一样**）：
 *   · `'full'`   ：① 把**当下盘上那 5 份**的现文记成 **(目标那一楼, 那一支)** 的快照；
 *                  ② `last` 锚到那一支（档案就地对齐到当前这一楼）；③ 清掉那条待处理；
 *   · `'number'` ：**只写清单** —— `last ← pending.to*` ＋ `pending: null`；⛔ 不记快照、⛔ 不写 blob、
 *                  ⛔ **不 upsert 楼层行**（那一楼先不长出新行；内容等下一轮轮末自然记上）。
 * 两种都达标同一条纪律：⛔ 那 5 份**正文一个字节都不写**（只读）、⛔ 不碰会话、⛔ 不碰归档、⛔ 不碰 Tavern。
 *
 * 目标那一楼：有待处理那条 ⇒ 用它记下的 `to*`；**没有**（★ 收尾新增：面板档顶那条**常驻状态行**上的
 * 「档案对齐到第 N 楼」—— 用户「不处理」过之后就不是开 fork 了）⇒ 用**宿主现读**的当前 `head`。
 *
 * 体：`{ targetKey?, mode? }` —— 面板把它看到的那条 pending 的 `targetKey` 带上；**跟当下这条不一致**
 * （这段时间里又发生了另一次 fork、目标换了）⇒ 409 `…_PENDING_STALE`，⛔ 绝不"按老目标就地登记"。
 * `mode` 缺省（没带这个键）＝ `'full'`（行为与从前**逐字一样**）；**认不出的值 ⇒ 400**
 * （判据只此一处 = `floorSnapshot.settleModeOf`，⛔ 不许悄悄按 `'full'` 跑）。
 * 状态码口径照写侧那条：体坏 400 / `mode` 认不出 **400** / 认不出落点 409 / 清单读不出来 409。
 */
async function handleRpMemoryFloorPendingSettle(req, send, log, redact) {
  ...
  // ★ 2026-09-25：那一排有两颗「对齐」（对齐楼层 / 只对齐楼号）⇒ 体里带 `mode`。
  //   ⛔ 认不出的值**当场 400** —— 不许"看不懂就当 full 跑"（那会替用户记下一份他没点的快照）。
  const mode = floorSnapshot.settleModeOf(body.mode)
  if (mode === null) {
    return send(400, err('RP_MEMORY_FLOOR_BAD_MODE',
      `认不出的 mode（${JSON.stringify(body.mode)}）⇒ 拒改（只认 'full' 与 'number'；⛔ 没替你按 'full' 跑，⛔ 一个字节都没写）`))
  }
  const home = floorHomeBound()
  if (home === null) {
  ...
  if (cur !== null && wantKey !== '' && wantKey !== cur.targetKey) {
    return send(409, err('RP_MEMORY_FLOOR_PENDING_STALE',
      '这一条回档提示已经换成新的了（那次 fork 的落点变了）⇒ 刷新面板再点（⛔ 没按你看到的那一条动手）'))
  }
  const at = new Date().toISOString()
  const now = floorHeadNow(home)
  const r = floorSnapshot.settlePending(home.dir, { at, head: now.head, seq: now.seq, mode })
  if (r.ok !== true) {
    return send(409, err('RP_MEMORY_FLOOR_INDEX_' + String(r.reason).toUpperCase(),
      `这条待处理登记不掉（${r.reason}）⇒ 拒改（⛔ 一个字节都没写）`))
  }
  // ★★ 收尾：这一脚＝用户"认过了当前这个剧情位置" ⇒ `seen ← head`（不然下一轮还按旧基准判开 fork）。
  //   两种模式都算认过了（「只对齐楼号」也是踩了这一脚、也算"这条提示我处理了"）。
  if (r.changed) floorAcknowledge(home)
  // ★★ 2026-09-25（用户口径，逐字）：「**改成：当前记忆从【序号】改为【序号】**」—— 这两种回执换成
  //   **序号**口径（就是面板上那个「序号 N」＝ `ordinal`，"第几版"，⛔ 不是 Tavern 那个楼号）。
  //   · `A` = 这一脚**之前**档案指针那一行的 `ordinal`（**宿主现读**——就是上面那份刚读的清单，⛔ 面板不算）；
  //     **认不出**（压根没有旧指针 / 那一行不在清单里 / 那行没 `at`）⇒ 如实说
  //     `当前记忆改为【序号 B】`，⛔ **不编 0**、⛔ 不许写成"从【序号 0】"。
  //   · `B` = 这一脚**之后**指针那一行的 `ordinal`（就地登记会当场给这一条记一行 ⇒ 拿得到；
  //     `mode:'number'` 不记行 ⇒ 那一行本来就有才拿得到，拿不到就如实写「序号未知」）。
  //   ⛔ 这一句里**不再出现"楼"**、⛔ 也不再复述"那 5 份一个字节都没写 / 不影响会话"
  //     （用户点名删过这类说明 —— 那些事实留在每颗按钮自己的 `title` 里）。
  //   ⚠️ 判据全在宿主现读的那两份清单上（**只读**：本函数不写任何东西，写盘在 `settlePending` 里）。
  const beforeOrdinal = floorSnapshot.ordinalOfFloor(read.index.last, floorSnapshot.ordinalMapOf(read.index.floors))
  const afterRead = floorSnapshot.readIndex(home.dir)
  const afterOrdinal = r.settled === null
    ? null
    : floorSnapshot.ordinalOfFloor(r.settled, floorSnapshot.ordinalMapOf(afterRead.index.floors))
  const seat = (n) => (Number.isFinite(n) ? `【序号 ${n}】` : '【序号未知】')
  const fromSeat = Number.isFinite(beforeOrdinal) ? seat(beforeOrdinal) : ''
  const toSeat = seat(afterOrdinal)
  const onlyNumber = mode === 'number'
  // ★ 文案**按模式分开说人话**（⛔ 不许拿"对齐楼层"那套去说"只对齐楼号"那一脚 —— 它俩写的东西不一样）。
  const text = r.changed && r.settled !== null
    ? (onlyNumber
      ? (fromSeat === ''
        ? `只把指针改为${toSeat}（这一条先不长快照，下一轮会自然记上）`
        : `只把指针从${fromSeat}改为${toSeat}（这一条先不长快照，下一轮会自然记上）`)
      : (fromSeat === '' ? `当前记忆改为${toSeat}` : `当前记忆从${fromSeat}改为${toSeat}`))
    : (cur === null
      ? (onlyNumber
        ? '指针本来就指着这一条（没有待处理的一条）—— 这一脚不必再走'
        : '档案本来就对齐在这一条上（没有待处理的一条）—— 这一脚不必再走')
      : '没有待处理的一条（本来就没什么可登记的）')
  try {
    log?.info?.(r.changed && r.settled !== null
      ? (onlyNumber
        ? `[mt] 楼层提示「只对齐楼号」：档案指针锚到第 ${String(r.settled.seq ?? '?')} 楼（${String(r.settled.nodeId).slice(0, 18)}）—— ⛔ 没记快照、⛔ 没长新行、那 5 份正文一个字节都没写`
        : `[mt] 楼层提示「档案对齐」＝就地登记：档案锚到第 ${String(r.settled.seq ?? '?')} 楼（${String(r.settled.nodeId).slice(0, 18)}）—— 那 5 份正文一个字节都没写`)
      : '[mt] 楼层提示「档案对齐」：没有待处理的一条（一个字节都没写）')
  } catch {}
  return send(200, {
    ok: true,
    base: home.base, baseLabel: home.baseLabel, sharedHint: home.base === 'workspace-root',
    changed: r.changed === true,
    // ★ 这一脚是哪种模式（面板据此说人话 —— 两颗按钮共用一个端点，回执得能分开）
    mode,
    // ★ 对齐成了哪一支（面板据此说人话）；顺手重取清单就能看到那一行
    settled: r.settled === null ? null : { nodeId: r.settled.nodeId, variantId: r.settled.variantId, seq: r.settled.seq },
    // ★ 2026-09-25：这一脚前后两个**序号**（就是回执里那 A / B；认不出 ⇒ `null` —— ⛔ 不编 0）。
    //   判据全在宿主现读的那两份清单上，⛔ 面板照实画、不许自己算。
    beforeOrdinal: Number.isFinite(beforeOrdinal) ? beforeOrdinal : null,
    settledOrdinal: Number.isFinite(afterOrdinal) ? afterOrdinal : null,
    pending: null,
...
/**
 * ★★ 2026-09-24 收尾：`POST /playthrough/rp-memory/floors/pending/ack` —— 面板横幅上那颗「**不处理**」（★ 2026-09-25 改的名，语义一个字没动）。
 *
 * 只做两件事（⛔ 就这两件，多一件都不做）：
 *   ① **清掉那条 `pending`**（＝把横幅收掉）—— ⚠️ 只写**我们自己的** `.dma-floor-snapshots/index.json`
 *      那一份清单里的这一个字段：⛔ **不登记**（不记盘上现文、⛔ 不动 `last`、⛔ 不落 blob）、
 *      ⛔ **不碰**那 5 份正文（这正是它与 `…/pending/settle` 的区别：那个是"就地登记"，这个是"不处理"）；
 *   ② **`seen ← 当前 head`**（从时间线现读）—— 口径见文件头 ⑤：点了「不处理」＝**算认过了**，
 *      基准往前跟到剧情现在站的那一楼，下一轮不再按旧基准重复弹同一条。
 * ⛔ **不再**引入"记住这一档已忽略"那类字段（上一单刚删掉它 —— 留着的那个字段会静默吞掉**下一次**回档）。
 *
 * 体：`{}`（不带任何参数 —— 判据全在宿主这边，⛔ 面板不自己算楼层）。
 * @returns `{ ok, changed, seen:{nodeId,variantId,seq}|null, seenWritten, message }` —— 如实回"清没清、跟到哪一楼"。
 */
async function handleRpMemoryFloorPendingAck(req, send, log, redact) {
  if (floorSnapshot === null) {
    return send(200, err('RP_MEMORY_FLOOR_UNAVAILABLE', `楼层快照模块（floor-snapshot.js）不可用：${floorSnapshotLoadError?.message || floorSnapshotLoadError}`))
  }
  const body = await readJsonObjectBody(req, send)
  ...
  const home = floorHomeBound()
  if (home === null) {
    return send(409, err('RP_MEMORY_MISSING', '这个周目还没有记忆库（.roleplay-memory/）⇒ 没有可收掉的提示'))
  }
  const read = floorSnapshot.readIndex(home.dir)
  if (read.error !== null) {
    return send(409, err('RP_MEMORY_FLOOR_INDEX_' + String(read.error).toUpperCase(),
      `楼层快照清单读不出来（${read.error}）⇒ 拒改（⛔ 一个字节都没写）`))
  }
  const cur = read.index.pending === null || read.index.pending === undefined ? null : read.index.pending
  const at = new Date().toISOString()
  let changed = false
  if (cur !== null) {
    // ⛔ 整份替换只改这一个字段：`last` / `floors` 原样带过去（⛔ 不登记、⛔ 不动那 5 份）。
    floorSnapshot.writeIndex(home.dir, {
      schemaVersion: floorSnapshot.FLOOR_SCHEMA_VERSION, updatedAt: at,
      last: read.index.last, floors: read.index.floors, pending: null,
    })
    changed = true
  }
  const now = floorHeadNow(home)
  const seenWritten = now.head === null ? false : writeSeenHead(home.playthroughId, now.seen)
  // ★ 2026-09-25：这一句也要在面板上显示 ⇒ 同样换**序号**口径（⛔ 别再让面板出现 Tavern 那个楼号）。
  //   「剧情现在站的那一条」在清单里查 `ordinal`；查不到（那一楼还没记过 / 没 `at`）⇒ 如实说"那一条（序号未知）"。
  const seat = (n) => (Number.isFinite(n) ? `【序号 ${n}】` : '那一条（序号未知）')
  const nowSeat = seat(floorSnapshot.ordinalOfFloor(now.head, floorSnapshot.ordinalMapOf(read.index.floors)))
  const text = changed
    ? `已「不处理」：那条提示收掉了（⛔ 没登记、⛔ 那 5 份笔记一个字节都没动）；认下剧情现在站的${nowSeat}，往后从这儿接着看`
    : `本来就没有待处理的提示（⛔ 一个字节都没动）；认下剧情现在站的${nowSeat}`
  try {
    log?.info?.(changed
      ? `[mt] 楼层提示「不处理」：收掉那条待处理（⛔ 没登记、⛔ 那 5 份一个字节都没动）；seen 跟到第 ${String(now.seq ?? '?')} 楼`
      : '[mt] 楼层提示「不处理」：本来就没有待处理的一条（一个字节都没动）')
  } catch {}
  return send(200, {
    ok: true,
    base: home.base, baseLabel: home.baseLabel, sharedHint: home.base === 'workspace-root',
    changed,
    seen: now.head === null ? null : { nodeId: now.head.nodeId, variantId: now.head.variantId, seq: now.seq },
    seenWritten,
    pending: null,
    message: redact ? redact(text) : text,
  })
 ...
 *   · `action === 'delete-vector'`（★ 2026-09-26 起 `delete-bm25` 已退役、不再认识）—— ★★ 2026-09-22（用户口径「直接冻面板 60s，
...
function registerLastFloors(ctx, log) {
  ...
  if (!archiveFloors || typeof archiveFloors.readArchiveFloors !== 'function') {
    log.warn(`[mt] 归档楼层读取不可用，最近几楼未接线（${archiveFloorsLoadError?.message || archiveFloorsLoadError}）`)
    return
  }
  if (!ctx || typeof ctx.plugin !== 'function') {
  ...
  const floorsFor = (dir, source) => {
    ...
    const floors = archiveFloors.readArchiveFloors(dir)
    cache.source = source
    cache.floors = floors
    cache.at = now
    return floors
      ...
      scope.on('system-prompt/assemble', async (assembly, assembleContext, next) => {
        try {
          ...
          if (lastFloors.readSwitch(cfgOf()).enabled === true
            ...
            const n = await withBudget(sessionContextChars(scope, sid), PREP_BUDGET_MS)
      ...
      scope.effect(() => scope.systemPrompt.section({
        ...
        text: (assembleContext) => {
          try {
            ...
            // ★ 2026-09-27：阈值走 config（`lastFloors.contextChars`，缺省 = 常量 5000）——
            //   迁移局首轮上下文就有 ~25k 字，默认窗永远 context-full。
            if (!lastFloors.contextIsShort({ contextChars, isFirstTurn, contextLimit: sw.contextChars })) return ''
            ...
            const decided = lastFloors.decideInject({ enabled: true, contextChars, isFirstTurn, contextLimit: sw.contextChars, floors, text })
