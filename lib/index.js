...
/** 配置里所有密钥原文（api.key + retrieval.key；redactor 与响应体擦除共用这一份清单）。 */
function secretKeysOf(config) {
  const cfg = config && typeof config === 'object' ? config : {}
  return [cfg.api && cfg.api.key, cfg.retrieval && cfg.retrieval.key].filter((k) => typeof k === 'string' && k !== '')
}
  ...
  return async function handler(req, res) {
    ...
    const send = (status, payload) => {
      ...
      try {
        ...
        // 双保险：响应体里绝不出现 key 原文（api.key 与 retrieval.key 都抹）
        try {
