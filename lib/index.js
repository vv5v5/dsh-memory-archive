...
/** 卡片字段字数：三口径里取「characters」（= 用户界面里最直观的那个）；⛔ 不猜缺的字段。 */
function v3FieldChars(data) {
...
async function handleV3Get(ctx, url, req, send, log) {
  ...
  const caps = await tavernV3Fetch(req, '/pmp-dsh-tavern/api/v3/capabilities')
  if (!caps.ok) {
    return send(200, {
      ok: false,
      error: {
  ...
  const composer = {
    ...
    owner: ours,
    registered: servicePresent && owners.includes(ours),
    servicePresent,
    owners,
    needsHostRestart: composerCfg.enabled === true && !owners.includes(ours),
    note: composerCfg.enabled !== true
  ...
  if (sessionId !== '') {
    const mode = await tavernV3Fetch(req, `/pmp-dsh-tavern/api/v3/sessions/${encodeURIComponent(sessionId)}/prompt-mode`)
    ...
    const src = await tavernV3Fetch(req, `/pmp-dsh-tavern/api/v3/sessions/${encodeURIComponent(sessionId)}/prompt-sources`)
    if (src.ok && isPlainObject(src.json?.sources)) {
      const s = src.json.sources
      const sel = isPlainObject(s.selection) ? s.selection : {}
      const docs = isPlainObject(s.documents) ? s.documents : {}
      const ch = isPlainObject(docs.character) ? docs.character : null
      const data = ch && isPlainObject(ch.data) ? ch.data : {}
      const greeting = isPlainObject(s.greeting) ? s.greeting : {}
      const wbs = isPlainObject(s.worldBookSelection) ? s.worldBookSelection : {}
      out.sources = {
        ...
        rp: isPlainObject(sel.rp) ? sel.rp : null,
        greeting: {
        ...
        suggestedCallConfig: isPlainObject(s.suggestedCallConfig) ? s.suggestedCallConfig : {},
        fieldLengthKeys: isPlainObject(s.fieldLengths) ? Object.keys(s.fieldLengths).length : 0,
        // ⛔ 只回投影：不把 character.data 原文交给面板（要看正文去 Tavern 自己的界面）
      }
    } else if (!src.ok) {
      out.sourcesError = { code: src.json?.code || 'SOURCES_READ_FAILED', status: src.status, message: src.json?.error || src.text.slice(0, 200) }
    }
  }
...
async function handleV3Mode(ctx, url, req, send, log) {
  ...
  const owner = mode === 'external'
    ...
    : undefined
  const cur = await tavernV3Fetch(req, `/pmp-dsh-tavern/api/v3/sessions/${encodeURIComponent(sessionId)}/prompt-mode`)
  if (!cur.ok) {
    return send(200, {
      ok: false,
  ...
  const put = await tavernV3Fetch(req, `/pmp-dsh-tavern/api/v3/sessions/${encodeURIComponent(sessionId)}/prompt-mode`, {
    method: 'PUT',
...
function registerLastFloors(ctx, log) {
  ...
  ctx.plugin({
    ...
    apply(scope) {
      ...
      let warned = false
      const disposeGuard = scope.on('system-prompt/assemble', async (assembly, assembleContext, next) => {
        try {
          if (!warned) {
  ...
  try {
    ...
    try {
      ...
      if (v3Tail && typeof v3Tail.registerV3Tail === 'function') {
        ...
        const t = v3Tail.registerV3Tail(ctx, cfgTail, log)
        v3TailHandle = t
