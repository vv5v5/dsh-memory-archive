...
function defaultConfig() {
  return {
    ...
    retrieval: { url: '', model: '', rerankModel: '', key: '', chatEnabled: true },
...
const ENDPOINTS = {
  ...
  // 「剧情大纲」档的**打开文件夹**（2026-09-20 用户口径）—— 动作型 ⇒ POST。
  // ⛔ 客户端**不传路径**：要开哪个目录由宿主自己按同一套候选链解析（见 resolveRpMemoryDir）。
...
function createHandler(ctx, log) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      if (rest === '/playthrough/reveal') return await handleRevealRpMemory(send, log)
...
const SKILL_DEFS = [
  {
    ...
    description: 'RP 助手 —— 把你这套角色扮演 agent 从头到尾理顺：它以为自己是干什么的、怎么说话、每轮都在的设定各落在哪儿、改哪一块有什么风险，以及这套东西装得对不对、换台机器还能不能用。手上带着这个项目的三份资料（酒馆 / 记忆库 / 翻往事那半边）与平台内部事实，可当场查。先只读体检摆事实，再逐块讲作用并问你的意见，全程你只需要做选择题。',
...
/**
 * 本会话该用的「周目记忆库目录」（★ **会话优先、配置兜底** —— 与面板跟随、检索隔离**同一条链**）。
 * 返回 `{ dir, playthroughId, source }` 或 `null`（没绑工作区根 / 拿不到周目）。
 * ⛔ 只拼路径，不建目录、不写盘。
 */
function memoryHomeFor(sessionId) {
  ...
  if (idx !== null && sessionPlaythrough !== null) {
    const hit = sessionPlaythrough.resolveForSession(idx, sessionId)
    if (hit.source === 'session' && hit.characterId !== '' && hit.playthroughId !== '') {
      return {
  ...
  const bound = rootPlaythroughDir()
  if (bound === null) return null
  return { dir: join(bound.dir, memoryWrite.MEMORY_WRITE_DIR), playthroughId: bound.playthroughId, source: 'config' }
}
      ...
      try {
        ...
        toolCtx.tools.register({
          ...
          async execute(args, exec) {
            ...
            if (home === null) throw new Error('拿不到本会话的周目记忆库目录（没绑工作区根 / 角色-周目？）')
 ...
 * 指的就是这一段。会话优先、配置兜底（与工具、面板同一条链）；拿不到路径 ⇒ **空段**（不注入半句）。
 */
          ...
          text: (assembleContext) => {
            try {
              ...
              return [
                '<memoryHome>',
                `本会话的剧情笔记目录：${home.dir}`,
                '一个周目一份 —— 只往这里写，⛔ 不要写到工作区根、也不要写到别的周目目录。',
                '</memoryHome>',
              ].join('\n')
            } catch { return '' }
          },
 ...
 *
 * 三条纪律：
 *   ① 要开哪个目录**由宿主自己解析**（复用 `resolveRpMemoryDir`，与 `/playthrough/rp-memory` 同一条候选链）
 *      —— ⛔ 请求体里没有路径、也⛔ 不接受任何路径参数 ⇒ 不构成"任意目录打开"入口；
 *   ② ⛔ 不经 shell（`spawn(cmd, [dir])`，没有 `shell: true`）⇒ 路径里的字符不参与命令解析；
 *   ③ ⛔ 响应里**不回绝对路径**（与 rp-memory 同口径：只说命中的是候选链里的哪一处）。
 */
async function handleRevealRpMemory(send, log) {
  const r = resolveRpMemoryDir()
  if (r === null) {
    return send(200, err('RP_MEMORY_MISSING', '这个周目还没有记忆库（.roleplay-memory/）：先选好工作区根，或等预设写出笔记'))
  }
  const okOpened = openDirInFileManager(r.found.dir)
  try {
    log?.info?.(`[mt] 打开记忆库目录：base=${r.found.base} ok=${okOpened}`)
  } catch {}
  if (!okOpened) {
    return send(200, err('REVEAL_FAILED', '没能在系统文件管理器里打开那个目录（宿主进程拉不起来文件管理器）'))
  }
  return send(200, { ok: true, base: r.found.base, baseLabel: r.found.label })
}
...
async function handleRpMemory(send, url) {
  ...
  const candidatesView = rpMemoryCandidates(hit).map((c) => ({ base: c.base, label: c.label, exists: isDirectorySafe(c.dir) }))
