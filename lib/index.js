...
async function readCompositionText(ap, info) {
  ...
  if (info.compositionText) return info.compositionText
  const tryString = (v) => {
...
async function handleAgentGet(ctx, url, req, send) {
  try {
    ...
    if (selected) {
      ...
      compositionText = info ? await readCompositionText(infos.service, info) : null
    ...
    return send(200, {
      ...
      error: reason,
      note: '本接口只读；拿不到的字段一律 null（界面显示未知）。零写入。',
    })
  } catch (e) {
...
/** 「生成 / 修复 RP agent」预览的 6 条事实（规格 §三.6 逐条口径，来自调研第一轮）。 */
function detectPreviewFacts() {
  return [
    '① 生成方式：调官方 agentPresets.copy(\'standard\', <id>, \'<显示名>\')（官方原话「创作即复制」），落到 ~/.dsh/.agent-presets/<id>/；<id> 必须匹配 [a-z0-9][a-z0-9-]*（它同时是目录名）。',
    ...
    '⑤ ★ host 层的工具 preset 删不掉（例：mcp-chrome 在 profile 层，RP 会话里角色会带着 Chrome 截图/CDP/evaluate_script）⇒ 需要我们的 host 守卫兜底，组装层收窄救不了这一条。',
    '⑥ 将删：bash / pwsh / web / subagent* / workflow / ralph / goal / plan / todo；将改：persona 文本（RP 人设 + 注入说明）、fs 换 sandboxed；将保留：compaction 整块必留（漏了长对话必撞窗口）、tool-fs、tool-fs-search 建议还回来。',
  ]
}
...
async function handleAgentDetect(ctx, url, req, send) {
  try {
    ...
    for (const info of infos.items) {
      ...
      if (compositionText == null) compositionText = await readCompositionText(infos.service, info)
    ...
    for (const c of candidates) {
      if (!c.hasCompactionInstruction) {
        missing.push(`「${c.id}」的压缩块没有 customInstruction —— 记忆库的压缩指令模板对它不生效（P1 生成/修复时会写入）`)
      }
      if (c.riskyToolRows === '检出与 RP 无关的工具行（bash/pwsh/web/subagent/goal/plan/todo 等）') {
        missing.push(`「${c.id}」仍挂着与 RP 无关的工具行（生成副本时会删；现存副本要改需等 P2 写入面）`)
      }
    }
    ...
    return send(200, {
      ...
      preview: {
        ...
        backupPlan: '（P1 起才实现写入）写入前把原文件按时间戳备份，面板可列出备份并一键回滚，写后回读校验、不一致自动回滚。本版不创建任何备份目录。',
      },
