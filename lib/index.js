...
// rp-agent（./rp-agent.js，v5 P1a 真落盘写入面）：与上面同样的顶层守卫加载 ——
// 它缺席时 /agent/apply、/agent/backups 两条 rest 降级为可读错误（HTTP 200 + ok:false），
...
function registerHttpApi(ctx, scope, log) {
  ...
  if (rpAgentLoadError) {
    log.warn(
      `[dsh-memory-archive] rp-agent 不可用（${rpAgentLoadError?.message || rpAgentLoadError}），/agent/apply 与 /agent/backups 已降级为可读错误；其余 rest 不受影响`,
    )
  }
