...
// 角色卡「对话后指示」的**末尾段**（./v3-tail.js）：与上面同样的顶层守卫加载。
// 为什么单独一个模块：v3 的外部段全被 Tavern 钉在 profile order 10，谁早谁晚由**段名顺序**定，
// 我们没法让卡里的 postHistoryInstructions 落在全文最后 —— 那必须由**宿主面**一段 order 10203 承担
// （> persona-suffix 10200，> rp:firstRound 10201，> mt:lastFloors 10202）。
// 模块缺席时只是这段不注入，其余一切照旧。
let v3Tail = null
let v3TailLoadError = null
try {
  const mod = await import('./v3-tail.js')
  v3Tail = typeof mod.registerV3Tail === 'function' ? mod : null
  if (v3Tail === null) v3TailLoadError = new Error('v3-tail.js 未导出 registerV3Tail')
} catch (e) {
  v3TailLoadError = e
}
/** 默认段名/order（与 v3-tail.js 的常量一致；模块缺席时端点仍要给个名字）。 */
const V3_TAIL_DEFAULT_NAME = v3Tail?.DEFAULT_TAIL_NAME ?? 'mt:postHistory'
const V3_TAIL_DEFAULT_ORDER = v3Tail?.DEFAULT_TAIL_ORDER ?? 10203
/** 末尾段的接线句柄（apply() 里赋值）：让 /v3 能如实报告"最近一次装配取到了什么、为什么"。 */
let v3TailHandle = null
/** 带外填充器的接线句柄：**同样要能查到** —— 宿主日志在启动它的那个控制台窗口里，我们读不到；
 *  这包零依赖也没有 tsc/eslint 能做作用域分析 ⇒ "接线期静默失败"只能靠可查状态兜（已踩过一次）。 */
let v3SourcesFillerHandle = null
/** PHI 重复修复的接线句柄（同样要能查到）。 */
let v3PhiStripHandle = null
...
// v3 来源快照的带外缓存（./v3-sources-cache.js）：**trace 合同**下 `mt:postHistory` 的唯一来源。
// 为什么需要：段 provider 必须同步、不能联网，而 trace 合同只有 HTTP（且不再提供那个同步服务）。
let v3SourcesCacheMod = null
let v3SourcesCacheLoadError = null
try {
  const mod = await import('./v3-sources-cache.js')
  v3SourcesCacheMod = typeof mod.createSourcesCache === 'function' ? mod : null
  if (v3SourcesCacheMod === null) v3SourcesCacheLoadError = new Error('v3-sources-cache.js 未导出 createSourcesCache')
} catch (e) {
  v3SourcesCacheLoadError = e
}
const v3SourcesCache = (() => { try { return v3SourcesCacheMod === null ? null : v3SourcesCacheMod.createSourcesCache() } catch { return null } })()
/** 同步读带外缓存（供尾部段的 provider 调用；⛔ 永不抛）。 */
function readCachedV3Sources(sessionId) {
  if (v3SourcesCache === null) return null
  try { return v3SourcesCache.get(sessionId) } catch { return null }
}

/**
 * 读"从官方 PHI `:part:` 段摘下来的**已渲染**正文"（尾段的**第二来源**，2026-09-18）。
 * 为什么需要：上游 `b0b0b7b` 删掉了 `GET /sessions/:id/sources`（404、无别名）⇒ trace 合同下
 * `mt:postHistory` 拿不到卡的后处理指令，**该字段整条从请求里消失**（沙箱 A/B：1 次 → 0 次）。
 * 而那段正文就在我们**本来就要摘掉**的官方段里 ⇒ 摘除器交过来，这里同步读。
 */
function readCachedV3RenderedPhi(sessionId) {
  if (v3SourcesCache === null) return null
  try { return v3SourcesCache.getRendered(sessionId) } catch { return null }
}
...
const OUR_SECTION_NAMES = Object.freeze([
  'state:card', 'dma:echo', 'anima:memory', 'rp:storyAnchor', 'rp:firstRound', 'mt:lastFloors', 'mt:postHistory',
])
...
function defaultConfig() {
  return {
    ...
    // 消费方是 dsh-anima-rag（它直接读本文件）；重排地址不单独存 = url 去尾斜杠 + '/rerank'。
    // key 只写不读：GET /config 只回 keySet/keyHint，任何响应/日志不出现原文。
    ...
    // 「最近几楼」（mt:lastFloors @10202，2026-09-17）：**默认关**。
    // 打开后，**空对话的首轮**把当前周目归档里最近的 count 楼原文注入成 <recentFloors>
    // （排在 `mt:postHistory`@10203 之前 = **倒数第二**）—— 新周目开局才有可承接的上下文。
    // ⚠️ 有真历史时**不注入**（不与真历史打架）；判据见 registerLastFloors 的 firstTurnOf。
    lastFloors: { enabled: false, count: 8, maxChars: 3000 },
...
function sanitizeConfig(parsed) {
  ...
  if (parsed.retrieval !== undefined) {
    if (isPlainObject(parsed.retrieval)) {
      for (const k of ['url', 'model', 'rerankModel', 'key']) {
...
function publicConfig(config, configError) {
  return {
    ...
    // 向量检索（20260918）：⛔ 只出 url/model/rerankModel + keySet/keyHint，绝不出 retrieval.key 原文。
    retrieval: {
    ...
    lastFloors: {
      ...
      count: config.lastFloors?.count ?? 8,
    ...
    v3: {
      ...
      // 末尾段（默认关）。它只在 external 模式下才该开：那时组合器被自动加了
      // omitFields=['postHistoryInstructions']，卡里的对话后指示由宿主面这段承担。
      tail: {
        enabled: config.v3?.tail?.enabled === true,
        name: config.v3?.tail?.name ?? V3_TAIL_DEFAULT_NAME,
        order: config.v3?.tail?.order ?? V3_TAIL_DEFAULT_ORDER,
      },
    },
  }
}
...
/** POST /retrieval/test（20260918）：拿当前 retrieval 配置对 embedding 端点打一次最小请求。
 *  回 HTTP 码 + 耗时（成功还给向量维度）；失败给可读原因。密钥只进请求头，绝不回显。
...
async function handleTestRetrieval(send, redact) {
  ...
  const { url, model, key } = r
  if (!url || !model) {
    return send(400, err('CONFIG_INCOMPLETE', '缺少 retrieval.url / retrieval.model，请先在「向量检索 API」里补全'))
  }
  if (!key) {
    return send(400, err('CONFIG_INCOMPLETE', '密钥还没配置：请先在「向量检索 API」卡里保存密钥（环境变量兜底只在 anima 侧生效，本测试只测这里的配置）'))
  }
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return send(
        200,
        Object.assign(
          err('API_ERROR', `embedding 端点返回 HTTP ${resp.status}${detail ? '：' + redact(detail) : ''}`),
    ...
    return send(200, { ok: true, model, endpoint, httpStatus: resp.status, elapsedMs, dims })
  } catch (e) {
    const elapsedMs = Date.now() - started
    const timedOut =
...
const COMPOSITION_PROBES = [
  ...
  },
  {
    ...
    note: '本插件（lib/last-floors.js + registerLastFloors）：**空对话的首轮**把当前周目归档里最近几楼原文注入成 <recentFloors>，让新周目开局有可承接的上下文。位置是**倒数第二** —— 紧随其后的 mt:postHistory@10203（卡的后处理指令）保持绝对最后。有真历史时不注入。默认关。',
  },
  {
...
// ---------------------------------------------------------------------------
// v3 来源「带外填充器」（trace 合同专用，2026-09-17）
//
// 为什么要有它：`mt:postHistory`（order 10203）的段文本要卡里的 `postHistoryInstructions`。
// composer 合同下走进程内同步服务 `pmpDshTavernPrompt.getSources()`；**trace 合同不再提供那个服务**，
// 只剩 HTTP。而段 provider 必须**同步、不能联网**（官方硬要求）⇒ 只能在本轮装配**之前**异步取一次、
// 塞进缓存，段函数再同步读。
//
// 时机：`agent/inbox/spliced`（人类消息进收件箱，**早于本轮装配**）。
// ⛔ **别挪进 `system-prompt/assemble`**：那时段的文本早已结算完（`system-prompt/src/index.ts:595-620`
//   先渲染所有段、再跑瀑布），填了只会**慢一轮**生效 —— 这个坑 `lib/v3-tail.js` 的文件头已经记过一次。
//
// 自打 HTTP 的三个前提（2026-09-17 实测过）：
//   · 端口取自 `webServer.port`（⛔ 不硬编码）；
//   · 地址用回环：`curl` 实测 **不带 Origin 也放行**（200）；
//   · **Host 必须是回环**：伪造成 `evil.example` 直接 **403** ⇒ 不要手动塞 Host。
// ---------------------------------------------------------------------------

/** 自打 HTTP 的超时（回环，短一点；失败只意味着这轮缓存不更新）。 */
const V3_SOURCES_FETCH_TIMEOUT_MS = 4000
/** 合同探测的记忆 + 失败退避：省掉每轮两次往返，也免得 Tavern 不在时每轮白等一个超时。 */
const v3ContractMemo = { contract: null, at: 0, until: 0 }
const V3_CONTRACT_MEMO_MS = 5 * 60 * 1000
const V3_CONTRACT_FAIL_BACKOFF_MS = 60 * 1000

/** 记着这台 Tavern 是哪套合同（带 TTL 与失败退避）。认不出 / 打不通 ⇒ null（⛔ 不猜）。 */
async function probeV3ContractMemo(base) {
  const now = Date.now()
  if (v3ContractMemo.until > now) return null
  if (v3ContractMemo.contract !== null && now - v3ContractMemo.at < V3_CONTRACT_MEMO_MS) return v3ContractMemo.contract
  if (v3Contract === null) return null
  try {
    const res = await fetch(base + v3Contract.v3Paths.capabilities(), { signal: AbortSignal.timeout(V3_SOURCES_FETCH_TIMEOUT_MS), redirect: 'manual' })
    if (!res.ok) { v3ContractMemo.until = now + V3_CONTRACT_FAIL_BACKOFF_MS; return null }
    const det = v3Contract.detectV3Contract(await res.json().catch(() => null))
    v3ContractMemo.contract = det.contract
    v3ContractMemo.at = now
    v3ContractMemo.until = 0
    return det.contract
  } catch {
    v3ContractMemo.until = now + V3_CONTRACT_FAIL_BACKOFF_MS
    return null
  }
}

/**
 * 取一次来源快照塞进缓存。⛔ **只有 trace 合同才做**（composer 合同有同步服务，不需要）。
 *
 * ★ 回**原因串**而不是布尔：`false` 会把"没去取（合同不对）"和"去取了但失败"混成一种，
 *   面板于是把**空转**说成"取数 N 次" —— 而 composer 合同下它本来就该一次都不取（实测踩到）。
 * @returns {Promise<'cached'|'not-trace'|'no-path'|'http-failed'|'shape-bad'|'runtime'>}
 */
async function refreshV3SourcesIntoCache(base, sessionId) {
  try {
    if (v3SourcesCache === null || v3Contract === null) return 'runtime'
    const contract = await probeV3ContractMemo(base)
    if (contract !== v3Contract.CONTRACT_TRACE) return 'not-trace'
    const path = v3Contract.v3Paths.sources(contract, sessionId)
    if (path === null) return 'no-path'
    const res = await fetch(base + path, { signal: AbortSignal.timeout(V3_SOURCES_FETCH_TIMEOUT_MS), redirect: 'manual' })
    if (!res.ok) return 'http-failed'
    const json = await res.json().catch(() => null)
    if (json === null || json.ok !== true) return 'shape-bad'
    return v3SourcesCache.set(sessionId, json.sources) ? 'cached' : 'shape-bad'
  } catch { return 'runtime' } // 填缓存失败绝不影响这一轮
}

/**
 * T7：把 Tavern 展开出来的 **PHI `:part:` 段**从装配里摘掉，只保留我们 `mt:postHistory` 那一份。
 *
 * 为什么：trace 合同下 Tavern 的 builtin 装配**总会**产一份后处理指令
 * （`pmp-dsh-tavern:part:NNNN:character:postHistoryInstructions`），而我们的尾段 @10203 又产一份
 * ⇒ 同一轮系统提示词里出现两次（2026-09-17 沙箱实测：同一份 system 里 @673 与 @7914 各一次，两段内容一致）。
 * composer 合同靠 `omitFields` 让 Tavern 别产；trace 合同没有组合器 ⇒ 改成**在下游把那段摘掉**。
 *
 * ★★ **必须延迟到"第一个会话事件"时才挂这个监听**（2026-09-17 沙箱实测，两版都试过）：
 *    直接挂（无论 `ctx.on` 还是套 `ctx.plugin`）都会**跑在 Tavern 前面** ——
 *    实测那时段表里只有 `pmp-dsh-tavern:profile` **一整段**（28 段、`:part:` 0 个），
 *    Tavern 还没展开，自然一个 PHI 段都摘不到（`strippedTotal:0`，而 PHI 照样重复）。
 *    同一个证据也在**组装捕获**里（另一个监听器，同样是开机即挂的）。
 *    **为什么**：监听按**注册顺序**跑，而 Tavern 顶层是
 *    `inject = ['systemPrompt','sessionController','workspaceController','directoryPickerController']`，
 *    我们只是 `['skills']` ⇒ **Tavern 等服务、激活更晚、注册更晚**，于是排在我们后面。
 *    会话事件一定在插件全部激活之后、又一定在**本轮装配之前**（`agent/inbox/spliced`），
 *    所以"那时才挂"既保证排在 Tavern 之后，又不漏首轮。
 *    ⛔ 这不是"猜"：`handle.lastSeen` 会把每一轮**看见了什么**记下来（段数 / `:part:` 数 / PHI 段数），
 *    一旦又跑回前面，`parts:0` 当场露馅（验收台钉了这条）。
 *
 * ⛔ **安全门**：只在**我们自己的尾段确实已接线**时才摘。否则摘掉 Tavern 那份 = 后处理指令**彻底消失**，
 *    比重复一次糟得多。这个门按**调用时**的句柄判，不是接线时判。
 * ⚠️ 只摘**认得出名字的**那一种；认不出的（例如经预设 `jailbreak` 的 `{{original}}` 注入、段名成了 `preset:…`）
 *    原样留着 —— 宁可重复一次，也不误删别人的段。
 */
function registerPhiStrip(ctx, log, deps = {}) {
  const handle = { installed: false, reason: 'not-attempted', mode: () => 'unavailable', strippedTotal: 0, last: null, lastSeen: null, phiStashed: 0, lastGuard: null }
  if (v3Contract === null) { handle.reason = 'module-missing'; return handle }
  if (deferredInstall === null) { handle.reason = 'install-module-missing'; return handle }
  const writeRenderedPhi = typeof deps?.writeRenderedPhi === 'function' ? deps.writeRenderedPhi : null
  const runStrip = async (assembly, assembleContext, next) => {
    try {
      if (v3TailHandle?.registered === true && assembly !== null && typeof assembly === 'object' && Array.isArray(assembly.sections)) {
        // ★ 摘之前先把这段正文留一份：trace 合同下 `/sources` 被上游删了，尾段的第二来源就是它
        //   （见 v3Contract.phiSectionText 的注释）。留不下**不挡摘除** —— 那只是又回到"没有来源"。
        try {
          const phiText = v3Contract.phiSectionText(assembly.sections)
          const sid = assembleContext?.agent?.session?.id
          if (phiText !== null && writeRenderedPhi !== null && typeof sid === 'string' && sid !== '') {
            if (writeRenderedPhi(sid, phiText) === true) handle.phiStashed += 1
          }
        } catch { /* 留不下不挡摘除 */ }
        // ★ 看守（2026-09-18 改版）：就在这一刻**直接观察**段表 —— 我们自己的段在不在。
        //   为什么在这儿：这是每轮装配**唯一**能看见完整段表的地方（下游监听、段文本已结算）。
        //   为什么不用"读预设 systemPromptMode"：上游把那条端点删了 ⇒ 那条路会瞎；而段表是硬事实。
        try {
          const sid = assembleContext?.agent?.session?.id
          if (typeof sid === 'string' && sid !== '' && v3SourcesCache !== null) {
            const g = v3Contract.guardFromSections(assembly.sections, OUR_SECTION_NAMES)
            v3SourcesCache.setObserved(sid, g)
            handle.lastGuard = { at: Date.now(), ...g }
          }
        } catch { /* 观察失败不影响装配 */ }
        const r = v3Contract.stripPhiParts(assembly.sections)
        // ★ 诊断：如实记下"这一轮**看见了什么**"。没摘时，这就是"没这一段"与
        //   "我们跑在 Tavern 前面（还没展开）"的唯一区分依据 —— ⛔ 不记就只能猜。
        handle.lastSeen = {
          at: Date.now(),
          total: assembly.sections.length,
          parts: assembly.sections.filter((s) => v3Contract.parsePartSectionName(s?.name) !== null).length,
          phiParts: assembly.sections.filter((s) => v3Contract.isPhiPartSection(s)).length,
        }
        if (r.removed.length > 0) {
          assembly.sections = r.kept
          handle.strippedTotal += r.removed.length
          handle.last = { at: Date.now(), removed: r.removed }
        }
      }
    } catch { /* 摘失败绝不影响这一轮 */ }
    return next()
  ...
  if (handle.installed) log.info('[mt] PHI 重复修复已接线（哨兵在位；第一个会话事件后由下游版接管）')
  else log.warn('[mt] PHI 重复修复接线失败（已降级）：监听没挂上')
  return handle
}

/**
 * 接线：装配之前填缓存。⛔ 只 inject systemPrompt（永远在）—— webServer 懒取，免得无 web 的宿主上插件一直 pending。
 *
 * ★ 触发时机（2026-09-17 沙箱实测改过一次）：**不能只认 `agent/inbox/spliced`**。
 *   第一版只挂那一个事件，实测**缓存填晚了** —— 那一轮太快，装配时缓存还是冷的，
 *   尾段照样 `chars=0`（而缓存里明明有那一局）。改成"**任何** `session/event` 都触发"：
 *   会话一建出来就开始暖，等玩家真发消息时缓存早就热了。
 *   用「在飞去重 + 每会话 2 秒节流」兜住开销（回环 HTTP，本来就便宜）。
 */
function registerV3SourcesFiller(ctx, log) {
  /** 句柄：**可变**对象 —— 端口要等真的取数成功才知道（注册那一刻 webServer 可能还没注入）。 */
  const handle = { registered: false, reason: 'not-attempted', port: null, attempts: 0, cached: 0, skipped: 0, failures: 0, lastFetch: null }
  if (v3SourcesCache === null || v3Contract === null) { handle.reason = 'module-missing'; return handle }
  // ⚠️ **自己读配置**，不由调用方传 —— 调用点那个 `cfgTail` 声明在另一个 if 块里，
  //    跨块引用会 ReferenceError，而且会被外层 catch 吞成"填充器静默不接线"（本函数为此改过一次）。
  const enabled = (() => { try { return readConfigFile().config.v3?.tail?.enabled === true } catch { return false } })()
  if (!enabled) { handle.reason = 'tail-disabled'; return handle }
  try {
    ctx.plugin({
      name: 'dsh-memory-archive:v3-sources-filler',
      inject: ['systemPrompt'],
      apply(scope) {
        const portOf = () => {
          try {
            const c2 = typeof scope.get === 'function' ? scope : (typeof ctx.get === 'function' ? ctx : null)
            const p = c2 === null ? null : c2.get('webServer')?.port
        ...
        scope.on('session/event', (session, event) => {
          try {
            ...
            const now = Date.now()
            // ★ `agent/inbox/spliced` = **本轮装配之前**那一刻 ⇒ 免节流，必须拿最新的一份。
            //   （2026-09-17 沙箱实测：只在别的会话事件上取过一次，那次**卡还没绑** ⇒
            //    缓存里 character 是 null ⇒ 尾段判成 no-card、整轮为空。节流害的。）
            const isPreAssembly = type === 'agent/inbox/spliced'
            if (!isPreAssembly && now - (lastAt.get(sid) ?? 0) < MIN_INTERVAL_MS) return
            const port = portOf()
            if (port === null) { handle.failures += 1; return }
            if (lastAt.size > 512) lastAt.clear() // ⛔ 不无界增长（缓存本身封顶 32 个会话）
            inFlight.add(sid)
            lastAt.set(sid, now)
            void refreshV3SourcesIntoCache(`http://127.0.0.1:${port}`, sid)
              .then((why) => {
                // ⛔ 分开记：`not-trace`（composer 合同下**本来就该空转**）不是"取数"，也不是失败。
                handle.attempts += 1
                if (why === 'cached') { handle.cached += 1; handle.port = port }
                else if (why === 'not-trace') handle.skipped += 1
                else handle.failures += 1
                handle.lastFetch = { at: now, event: type, result: why }
              })
              .catch(() => { handle.attempts += 1; handle.failures += 1 })
    ...
    handle.reason = 'ok'
    log.info('[mt] v3 来源带外填充器已接线（trace 合同下 mt:postHistory 靠它兜底）')
  } catch (e) {
    const msg = String((e && e.message) || e)
    handle.registered = false
    handle.reason = 'plugin-failed:' + msg
    log.warn(`[mt] v3 来源带外填充器接线失败（已降级）：${msg}`)
  }
  return handle
}
...
async function handleV3Get(ctx, url, req, send, log) {
  ...
  const out = {
    ...
    composer,
    tail: (() => {
      const tCfg = (cfg.v3 && cfg.v3.tail) || {}
      const st = (() => { try { return v3TailHandle?.status?.() ?? null } catch { return null } })()
      return {
        enabled: tCfg.enabled === true,
        name: typeof tCfg.name === 'string' && tCfg.name !== '' ? tCfg.name : V3_TAIL_DEFAULT_NAME,
        order: Number.isFinite(tCfg.order) ? tCfg.order : V3_TAIL_DEFAULT_ORDER,
        registered: v3TailHandle?.registered === true,
        // ★ 这一段依赖 Tavern 的 Cordis 服务 `pmpDshTavernPrompt.getSources()`；
        //   trace 合同**不再提供**那个服务 ⇒ 这一段的取值路径必须另找（见 /v3/tail 的 last.reason）。
        usable: info.tailServiceUsable,
        // 这一段是"每轮刷新"的：last 说的是**最近一次装配**取到了什么、为什么
        last: st,
        note: info.tailServiceUsable === false
          ? '⚠ 这一段依赖的 Cordis 服务 pmpDshTavernPrompt 在这台 Tavern 上**不存在**（trace 合同不再提供它）⇒ 若不启用带外来源，这一段会**静默变空**。看下面 last.reason 是不是 no-tavern-service。'
          : (info.tailServiceUsable === null
            ? '无法判定：没读到 v3 capabilities ⇒ 不知道那个服务在不在（⛔ 不猜）'
            : (tCfg.enabled !== true
              ? '未启用：卡里的对话后指示仍由组合器在 external 模式下产出（不会排到最后一段）'
              : (v3TailHandle?.registered === true
                ? '已注册：external 模式下组合器自动让出 post-history，这段排到全文最后'
                : '配置说启用，但本进程里没注册上 —— 改完这条配置要**重启宿主**才生效'))),
      }
    })(),
    // 带外填充器：**要能查到**，不能只活在日志里（宿主日志在启动它的控制台窗口，我们读不到）。
    // trace 合同下 `mt:postHistory` 唯一依赖它 —— 它没接线，那一段就会一直是空的。
    sourcesFiller: (() => {
      const h = v3SourcesFillerHandle
      const st = v3SourcesCache === null ? null : (() => { try { return v3SourcesCache.stats() } catch { return null } })()
      return {
        registered: h?.registered === true,
        reason: h?.reason ?? 'not-attempted',
        // 端口是**真的取到数时**才记下来的（注册那一刻 webServer 可能还没注入；composer 合同下
        // 根本不需要端口 ⇒ 长期为 null 是**正常**，面板不该报"⚠ 拿不到端口"）
        port: h?.port ?? null,
        // ⛔ 分开记：`attempts` 是"被叫了几次"，`cached` 才是"真取了并进了缓存"，
        //   `skipped` 是"合同不对、本来就该空转"。混成一个数就是把空转说成取数（实测踩到）。
        attempts: h?.attempts ?? 0,
        cached: h?.cached ?? 0,
        skipped: h?.skipped ?? 0,
        failures: h?.failures ?? 0,
        lastFetch: h?.lastFetch ?? null,
        // ⛔ 只出计数与年龄（缓存模块的 stats 本身就不含正文）
        cache: st,
        note: h?.registered === true
          ? '已接线：每轮装配前取一次 sources 填缓存（**只有 trace 合同真的会取**；composer 合同有同步服务，不需要）'
          : '未接线（' + String(h?.reason ?? '没走到接线那一步') + '）⇒ trace 合同下 mt:postHistory 会保持空',
      }
    })(),
    mode: null,
    sources: null,
    assemblies: null,
    replaceGuard: null,
    // T7：摘重复 PHI 的状态（"到底摘没摘、摘了几次"必须能查到，⛔ 不能只活在日志里）
    phiStrip: (() => {
      const h = v3PhiStripHandle
      ...
      const registered = installed && mode === 'downstream'
      const armed = installed && mode === 'upstream-sentinel'
      return {
        registered,
        armed,
        mode,
        reason,
        tailGated: v3TailHandle?.registered === true,
        strippedTotal: h?.strippedTotal ?? 0,
        last: h?.last ?? null,
        ...
        note: registered
          ? (v3TailHandle?.registered === true
            ? '已接线：装配下游把 Tavern 的 postHistoryInstructions :part: 段摘掉，只留我们 @10203 那一份'
            : '已接线但**安全门是关的**（我们的尾段未接线）⇒ 不摘 —— 否则后处理指令会彻底消失')
          : (armed
            ? '已挂载、待启用：刻意等到**第一个会话事件**才真正挂上（那之前挂会排在 Tavern 前面，看不见 :part: 段）'
            : '未接线（' + reason + '）⇒ trace 合同下 PHI 会重复注入'),
      }
    })(),
  }
  if (sessionId !== '') {
  ...
  const obs = (() => { try { return v3SourcesCache === null ? null : v3SourcesCache.getObserved(sessionId) } catch { return null } })()
    ...
    : (presetSaysReplace
      ...
      : (observedOk && obs.partial
        ? `这一轮装配（共 ${obs.total} 段）里我们缺了 ${obs.missing.join('、')}`
          + `（在的有 ${obs.present.length} 段）—— 可能只是某个插件没启用（例如 state:card 属 state-bridge），`
...
function rootFloorsDir() {
  try {
    ...
    return { dir: join(rootPath, ch, pt, 'archive', 'floors'), source: ch + '/' + pt }
  } catch {
    return null
  }
}
...
//   · 只在**空对话的首轮**注入 —— 有真历史时不注入，免得同一段历史进两次；
//   · ★ 只在**绑定本局周目的会话**里注入（复用 autoCollectScope 那份判据）——
//     否则你在编程会话里的第一句话也会被塞进这个周目最近几楼（那是错的）。
//
// 接线纪律（照抄 echo）：段 provider **同步**、只返回算好的字符串；绝不抛；模块/服务缺席只降级。
// ---------------------------------------------------------------------------

/** 接线：注册 `mt:lastFloors` 段 + 首轮判据 + 越权看守。绝不抛（调用方还会再包一层）。 */
function registerLastFloors(ctx, log) {
  ...
  /** 「绑定周目的会话」缓存（与 autoCollectScope 同一判据，同一个 TTL）。 */
  const boundCache = { ids: null, at: 0 }
  const cfgOf = () => { try { return readConfigFile().config } catch { return {} } }
  /**
   * ★ **绑定门**（必须有）：本段注的是**这个周目**的归档楼层 —— 不绑定周目的会话
   * （例如你正在跟 AI 干活的编程会话）⛔ 一个字都不许注。判据复用自动收纳那份
   * （catalog 的 rootSessionId ∪ config.root.sessionId），**宁可漏，也不猜**。
   */
  const boundSessions = () => {
    const now = Date.now()
    if (boundCache.ids !== null && now - boundCache.at < LAST_FLOORS_CACHE_TTL_MS) return boundCache.ids
    let ids
    try { ids = autoCollectScope().ids } catch { ids = new Set() }
    boundCache.ids = ids
    boundCache.at = now
    return ids
  }
  ...
  /** 我们自己的段名（看守豁免 —— mt:postHistory 预期就排在 10203，比我们高，不该报）。 */
      ...
      scope.effect(() => scope.systemPrompt.section({
        ...
        text: (assembleContext) => {
          try {
            ...
            const sid = assembleContext?.agent?.session?.id
            if (typeof sid !== 'string' || sid === '') return ''
            if (!boundSessions().has(sid)) return '' // ★ 绑定门：非本局会话一个字不注
            if (firstTurnOf(scope, assembleContext?.agent) !== true) return ''
            const hit = rootFloorsDir()
            if (hit === null) return ''
            ...
            const decided = lastFloors.decideInject({ enabled: true, isFirstTurn: true, floors, text })
...
export function apply(ctx) {
  ...
  try {
    ...
    // v3 消费端（默认关）：软注入 pmpDshTavernPrompt —— 服务出现才注册组合器。
    // ⛔ 绝不硬 inject：2026-09-16 沙箱实测，硬 inject 这个服务的插件在 Tavern 被卸载后
    ...
    try {
      if (v3Composer && typeof v3Composer.registerV3Composer === 'function') {
        const cfgAllV3 = readConfigFile().config.v3 ?? {}
        const cfgV3 = cfgAllV3.composer ?? {}
        const cfgTail = cfgAllV3.tail ?? {}
        // 末尾段开着时，组合器必须**让出** post-history，否则同一段文字进两次（卡字段 + 宿主面段）。
        // 只在真的会注册时加 omit：tail 模块缺席 / enabled=false 时组合器照常带 post-history。
        const tailWillRegister = cfgTail.enabled === true && v3Tail !== null
        const cfgV3Effective = tailWillRegister
          ? { ...cfgV3, omitFields: [...new Set([...(Array.isArray(cfgV3.omitFields) ? cfgV3.omitFields : []), 'postHistoryInstructions'])] }
          : cfgV3
        const r = v3Composer.registerV3Composer(ctx, cfgV3Effective, log)
      ...
      // 末尾段：与组合器**各自 try/catch**（一个坏不拖另一个）。
      if (v3Tail && typeof v3Tail.registerV3Tail === 'function') {
        const cfgTail = readConfigFile().config.v3?.tail ?? {}
        // 第 4 个参数是**带外来源**读取器：trace 合同下那个同步服务不存在，尾部段改读它。
        const t = v3Tail.registerV3Tail(ctx, cfgTail, log, { readSources: readCachedV3Sources, readRenderedPhi: readCachedV3RenderedPhi })
        v3TailHandle = t
        if (t.registered) log.info(`[mt] v3 末尾段已接线：name=${t.name} order=${t.order}（默认关的开关见 config.v3.tail.enabled）`)
        else log.info(`[mt] v3 末尾段未启用（${t.reason || 'disabled'}）—— 卡里的对话后指示不进宿主面`)
      } else {
        log.warn(`[mt] v3-tail 不可用，末尾段未接线（${v3TailLoadError?.message || v3TailLoadError}）`)
      }
      // 带外填充器：与上面各自 try/catch（一个坏不拖另一个）。
      // ⛔ 这里**不要**读 cfgTail —— 它在上面那个 if 块里，跨块引用会 ReferenceError（已踩过一次）。
      //    开关由 registerV3SourcesFiller 自己读配置判断。
      try {
        v3SourcesFillerHandle = registerV3SourcesFiller(ctx, log)
      } catch (e) {
        const msg = String((e && e.message) || e)
        v3SourcesFillerHandle = { registered: false, reason: 'threw:' + msg, port: null }
        log.warn(`[mt] v3 来源带外填充器接线失败（已降级）：${msg}`)
      }
      // T7：摘掉 Tavern 那份重复的 PHI（只在我们的尾段确实接线时才摘 —— 安全门在监听器里）。
      try {
        v3PhiStripHandle = registerPhiStrip(ctx, log, {
          writeRenderedPhi: (sid, text) => (v3SourcesCache === null ? false : (() => { try { return v3SourcesCache.setRendered(sid, text) } catch { return false } })()),
        })
      } catch (e) {
        const msg = String((e && e.message) || e)
        v3PhiStripHandle = { registered: false, reason: 'threw:' + msg, strippedTotal: 0, last: null }
        log.warn(`[mt] PHI 重复修复接线失败（已降级）：${msg}`)
      }
    } catch (e) {
