...
function registerHttpApi(ctx, scope, log) {
  ...
  if (rpAgentLoadError) {
    log.warn(
      `[dsh-memory-archive] rp-agent 不可用（${rpAgentLoadError?.message || rpAgentLoadError}），/agent/apply、/agent/backups、/agent/rollback、/agent/cards、/agent/card 与 /agent/pack/apply 已降级为可读错误；其余 rest 不受影响`,
    )
  }
