  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      if (rest === '/sections/text') {
        ...
        // 内部走 sessionQuery.readSession（记忆库同款路径，⛔ 不直读会话文件），
...
function registerHttpApi(ctx, scope, log) {
  ...
  if (promptViewerFactory === null) {
    ...
    try {
      ...
      const pvHandler = promptViewerFactory({ archivedProvider: makeArchivedProvider(ctx) })
      run(() => {
...
const SKILL_DEFS = [
  ...
  },
  {
    ...
    description: '配置知识库（只给模型）：官方源码原文与实测结论，带行号与出处，并说明怎么自己再验一遍。',
    whenToUse: null,
