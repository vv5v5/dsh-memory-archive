...
function registerPhiStrip(ctx, log, deps = {}) {
  const handle = { installed: false, reason: 'not-attempted', mode: () => 'unavailable', strippedTotal: 0, last: null, lastSeen: null, phiStashed: 0 }
...
async function handleV3Get(ctx, url, req, send, log) {
  ...
  out.replaceGuard = {
    presetMode: out.sources?.presetMode ?? null,
    ourSections: [...OUR_SECTION_NAMES],
    atRisk: out.sources?.presetMode === 'replace',
    note: out.sources?.presetMode === 'replace'
      ...
      : (out.sources?.presetMode == null
        ? '读不到预设的 systemPromptMode（要走 v3 sources）⇒ 无法判定，⛔ 不猜'
        : '预设是 append 模式 ⇒ Tavern 不会滤掉我们的段'),
  }
