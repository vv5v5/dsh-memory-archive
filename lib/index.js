 ...
 */
function registerPhiStrip(ctx, log) {
  const handle = { installed: false, reason: 'not-attempted', mode: () => 'unavailable', strippedTotal: 0, last: null, lastSeen: null }
...
const ENDPOINTS = {
  ...
  '/collect/scan': ['POST'],
  '/collect/auto': ['POST'],
  // 手动「扫归档原文 → 总结」（2026-09-16）：plan 零 LLM、apply 才调模型；两步走（预览即合同）
  '/summarize/plan': ['POST'],
  '/summarize/apply': ['POST'],
  // 清空总结（补救：预设没调好就重来）—— 备份正文 → 清空 → 重置 index（保留原文与批次清单）
...
function createHandler(ctx, log) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      // 手动「扫归档原文 → 总结」：plan 零 LLM，apply 才调模型（两步走：预览即合同）。
      // 指令与破限头**沿用同一份配置**（`prompts.compaction` / `prompts.compactionJailbreak`），
      // 保证「压缩用的指令」与「归档总结用的指令」是同一段文字，不出现第二个口径。
      ...
      if (rest === '/summarize/plan') {
        // ⛔ 配置必须**在本块里现读**：上面第 3054 行那个 `const { config }` 是 `/config` GET 分支的
        //    块级作用域，在这里引用会 `config is not defined`（真机 500 实测踩过）。
        const cfg = readConfigFile().config
        return await import('./summarize.js').then((m) => m.handleSummarizePlan(ctx, url, req, send, { config: { ...cfg.summarize } }))
      }
      if (rest === '/summarize/apply') {
        const cfg = readConfigFile().config
        const t = templatesResponse().templates
        // ★ key 来源：配置里填了就用配置的；没填则回落到环境变量（ST 侧那四个渠道本来就是同一把
        //   SiliconFlow key，本机已有 ANIMA_RAG_* 两份拷贝，别再让用户贴第三遍）。
        //   ⛔ 只在这里做请求头，绝不回显（`/config` 仍只给 keySet/keyHint）。
        const envKey = process.env.SILICONFLOW_API_KEY || process.env.ANIMA_RAG_EMBED_KEY || ''
        return await import('./summarize.js').then((m) => m.handleSummarizeApply(ctx, url, req, send, {
          config: { ...cfg.summarize },
          api: { ...cfg.api, key: cfg.api.key || envKey, maxTokens: cfg.summarize?.maxTokens },
          instruction: t.compaction.current,
          jailbreak: t.compactionJailbreak.current,
        }))
      }
  ...
  try {
    ...
    try {
      ...
      if (v3Tail && typeof v3Tail.registerV3Tail === 'function') {
        ...
        const t = v3Tail.registerV3Tail(ctx, cfgTail, log, { readSources: readCachedV3Sources })
        v3TailHandle = t
