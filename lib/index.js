...
const ENDPOINTS = {
  ...
  //   · `/floors`         = 楼层清单（只读：第 N 楼 · 时间 · 改了哪几份（+几字节）· 当前那一楼 · 最近一次自动回档）；
  //   · `/floors/restore` = 面板「回到这一楼」（**手动**触同一个恢复函数；恢复前先存一份"当下"，可撤销）。
  //   记录与跟随是**钩子**做的（轮末记一份、组装时 + 轮末判回档），⛔ 这两条端点都不改判据。
...
function createHandler(ctx, log) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      // ★ 2026-09-23：按楼层绑定的笔记快照（只读清单 + 面板「回到这一楼」）。
...
// ---------------------------------------------------------------------------
// 「**按楼层绑定**的笔记快照 + 回档跟随」的接线（2026-09-23）
//
// 用户口径：「**能不能做按楼层绑定管理的功能，也就是每轮模型修改都记录，能自动跟随回档**」。
...
//   ③ **跟**：组装时（每轮至多一次）+ 轮末各判一次回档，是回档就**把笔记恢复到那一支**；
...
function floorReceiptText(home, receipt) {
  ...
  const at0 = kind === 'rollback' ? (rest?.headSeq ?? to) : to
  ...
  if (kind === 'rollback' || kind === 'manual') {
    ...
    const targetVariant = typeof rest?.target?.variantId === 'string' && rest.target.variantId !== ''
      ...
      : ''
    const pre = kind === 'rollback'
      ? (rest?.why === 'variant'
        ? `检测到${seat(at0)}换了变体（swipe 重 roll：${branch(rest?.from?.variantId)} → ${branch(rest?.headVariantId)}）`
        : `检测到回档到${seat(at0)}（从${seat(from)}）`)
      : `面板点了「回到这一楼」（${seat(to)}${targetVariant}）`
...
function reportFloorReceipt(home, receipt, mode, head, log) {
  ...
  const always = (kind === 'rollback' || kind === 'manual') && failed.length === 0 && (rest === null || rest.blocked === null)
  if (!always) {
  ...
  if (rollbackFamily) {
    writeFloorStatus({
      ...
      from: rest?.from === undefined || rest?.from === null ? null : { nodeId: rest.from.nodeId, variantId: floorStr(rest.from.variantId), seq: rest.from.seq ?? null },
      to: rest?.target === undefined || rest?.target === null ? null : { nodeId: rest.target.nodeId, variantId: floorStr(rest.target.variantId), seq: rest.seq ?? null },
      source: rest?.source ?? null, why: rest?.why ?? null,
 ...
 * @param {'watch'|'turn'} mode `watch` = 组装那一脚（只判回档、只做恢复）；`turn` = 轮末（还记一份）
 */
 ...
 *     判回档 + 恢复排到本轮之外再跑（`setTimeout(…, 0)` + `unref`，`void … .catch()`，绝不抛），
 *     **每轮至多一次**（门 = `turnBoundary.lastTurn`，取不到退到最小间隔）。★ 这一脚的意义：
 *     让模型**本轮**读到的笔记就是那一楼的（回档之后不等下一轮才跟上）。
...
function registerFloorSnapshot(ctx, log) {
  ...
  ctx.plugin({
    ...
    apply(scope) {
      ...
      // 轮末那一脚：判回档（若真回档，先存当下再恢复）+ 记一份（有变化才写盘）。
  ...
  log.info('[mt] 楼层快照已接线（轮末记一份；组装时 + 轮末各判一次回档并自动跟随；'
 ...
 * （+几字节）· [回到这一楼]`）、当前站的那一支高亮、档顶一句「最近一次自动回档跟随」。
