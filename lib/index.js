...
const RP_MEMORY_PREFERRED_ORDER = Object.freeze(['index.md', 'story.md', 'characters.md', 'world.md'])
 ...
 * `<rootPath>/<charId>/<playthroughId>`（**只读**，与"角色-周目"这条唯一的组织轴一致）。
 *
 ...
 *
 * ★ 20260919 提函数：原来这一个函数直接拼到 archive/floors；现在"拼周目目录"归
 *   rootPlaythroughDir()，rootFloorsDir() 只在其上加 `archive/floors` 一层。
 *   判定逻辑与返回值语义**一字未改**（检查顺序、null 条件、source 拼法全部原样；
 *   path.join 多段拼接与逐段 join 等价 ⇒ dir 字符串逐字节相同），echoArchiveSource()
 *   等调用方行为不变。
 */
...
/** 归档楼层目录 = 周目目录下的 `archive/floors`（判定逻辑原样，只是把"拼周目目录"提成了上面的函数）。 */
...
async function handleRpMemory(send, url) {
  ...
  const candidatesView = candidates.map((c) => ({ base: c.base, label: c.label, exists: isDirectorySafe(c.dir) }))
      ...
      scope.on('system-prompt/assemble', async (assembly, assembleContext, next) => {
        try {
          ...
          if (lastFloors.readSwitch(cfgOf()).enabled === true
            && typeof sid === 'string' && sid !== '' && boundSessions().has(sid)) {
