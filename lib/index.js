...
function registerHttpApi(ctx, scope, log) {
  ...
  if (rpAgentLoadError) {
    log.warn(
      `[dsh-memory-archive] rp-agent 不可用（${rpAgentLoadError?.message || rpAgentLoadError}），/agent/apply、/agent/backups、/agent/rollback、/agent/cards 与 /agent/card 已降级为可读错误；其余 rest 不受影响`,
    )
  }
