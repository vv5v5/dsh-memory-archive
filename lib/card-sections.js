 ...
 *
 * ★ 2026-09-20：order 50（原 `state:card`）**已不保留** —— 状态子系统整个剥离了
 *   （state-bridge 子包归档、story-anchor 也不再挂载；状态改由周目笔记 `.roleplay-memory/state.md` 维护）。
 * ★ 2026-09-26（T3）：order 54 的原主 `dma:echo`（记忆回响 D2）**已退役** —— 本插件不再往 54 注册任何段
 *   （`<memoryEcho>` 那一格改由 `dsh-anima-rag` 的语义命中装，画在它自己的 55 段里）。
 *   54 这一位**继续保留**：它是"回响那一格"的历史位次，留个带名字的空位比让别的插件无意占进去更好查。
 */
export const HARD_RESERVED_ORDERS = Object.freeze([
  { order: 45, owner: 'rp:policy（上游）' },
  { order: 54, owner: '（空位：dma:echo 已于 2026-09-26 退役）' },
