...
/** deps.tavern.read 的宽容消费：裸字符串 / `{ok,text}` / A1 的 `{content}`（无 ok）都认；失败 → false。 */
async function readExists(tavern, rel) {
  try {
    const r = await tavern.read(rel)
    if (typeof r === 'string') return true
    if (!isRecord(r) || r.ok === false) return false
    return typeof r.text === 'string' || typeof r.content === 'string'
  } catch {
    return false
  }
}
...
export function floorBodyText(floorNo, turn) {
  ...
  // ⛔ 不加尾换行：同一个 archive/floors 目录只有**一套字节约定**，而 A1 的写入器
  // （collect.js:71 floorDocText）是 `JSON.stringify({...})` 原样。两个写手不一致的后果是
  // 「预览里的 sha256 与真落盘的字节对不上」——落库单正是靠这两个数对账的。
 ...
 */
/**
 * 内部计划器（零落库）：文本（直传或 deps.tavern.read 读来）→ NormalizedImport →
 * willWrite（每轮一楼 + 一条批次摘要）。楼号从「现存最大楼号 + 1」起；floors 目录
 * 列不到（不存在/网络失败）⇒ 按空目录从 0000 起 + warning，绝不抛。
 * ⛔ 本函数【只调 deps.tavern 的 list 与 read】，绝不调 write/mkdir。
 * @returns `{ plan, internal }`；`plan` 是可外传的那半，`internal` 带正文（只给落库模块）。
 */
export async function buildImportPlan(input, deps) {
  ...
  if (typeof source.text === 'string') {
    ...
    try {
      ...
      const err = importError('IMPORT_SOURCE_UNREADABLE', '读取源文件失败：' + String((e && e.message) || e))
      // 把底层 code 带出来：落库端点据此把"Tavern 不可达 / 5xx"报成 IMPORT_TAVERN_UNREACHABLE，
      // 而不是含糊的"源文件读不到"（用户分不清是网络问题还是文件问题）。
      if (e && typeof e.code === 'string') err.causeCode = e.code
      throw err
    }
    // ★ 读取结果**三种形状都要认**：裸字符串 / 导入自己的只读客户端 `{ok:true, text}` /
    //   A1（collect.js createTavernClient）`{content}`（**没有 ok 字段**）。落库单
    //   （import-apply.js）必须用 A1 那个客户端的同一个实例（它才有 write/mkdir），于是
    //   只认 `{ok:true,text}` 会让"导入落库"永远读不到源文件（20260915 自检台当场抓到的互操作缺口）。
    const t = typeof r === 'string'
      ? r
      : isRecord(r) && r.ok !== false
  ...
  const warnings = []
  // ★ 归档路径口径**必须与 A1（collect.js）逐字一致**：`catalogTarget` 取的 playthroughId 是
  //   `catalog.json` 里那条 path 的**目录名**（`segs[len-2]`，真机形如 `playthrough-938a0d26-…`，
  //   自带 `playthrough-` 前缀），collect.js 就是 `characterId + '/' + playthroughId + '/archive'`。
  //   ⛔ 本模块原来在这里又拼了一次 `'playthrough-' + id` ⇒ 真机会写成
  //   `<char>/playthrough-playthrough-938a…/archive`，而收纳与回响读的是
  //   `<char>/playthrough-938a…/archive` —— 导入「成功」但**归档里永远看不到**（自检台当年
  //   用带前缀的常量算 floorsRel，于是把这个 bug 一起抄成了"期望值"，两边一起错）。
  const archiveRel = target.characterId + '/' + target.playthroughId + '/archive'
  const floorsRel = archiveRel + '/floors'

  // ★ 覆盖重导（input.overwrite）：楼号必须**从 0000 重新开始**，否则所谓"覆盖同号楼层"会变成
  //   "把同一份正文再追加一遍到 0254+"（2026-09-16 实测：勾了覆盖，计划却是 254…507）。
  ...
  if (listed.ok) {
    ...
    existingNums = nums
    if (nums.length === 0) {
      warnings.push('IMPORT_FLOORS_EMPTY：' + floorsRel + ' 下没有可识别的 NNNN.json 楼层，楼号从 0000 起')
    } else if (overwrite) {
      start = 0
      warnings.push('IMPORT_OVERWRITE_FROM_ZERO：覆盖重导：楼号从 0000 重新开始，归档里同号楼层将被覆盖')
    } else {
  ...
  // 覆盖重导只能覆盖到「本次源有多少楼」为止：归档里更高号的那些楼层文件**删不掉**
  // （工作区文件面只有 GET/PUT）—— 如实报出来，别让人以为归档被清干净了。
  if (overwrite && existingNums.length > 0) {
    const tail = existingNums.filter((n) => n >= Math.max(kept.length, 1)).sort((a, b) => a - b)
    if (tail.length > 0) {
      warnings.push(
        'IMPORT_OVERWRITE_TAIL：归档里还有 ' + tail.length + ' 个更高楼号（' + tail.slice(0, 3).join(', ')
        + (tail.length > 3 ? ' …' : '') + '）本次源覆盖不到；楼层文件删不掉（文件面没有删除接口），它们会留着',
      )
    }
  }
  ...
  const summaryText = kept.length > 0 || typeof normalized.greeting === 'string'
    ? summaryBodyText({ planId, normalized, kept, keepGreeting })
    : null
  if (summaryText !== null) {
    willWrite.push({ path: summaryRel, bytes: byteLen(summaryText), sha256: sha256Hex(summaryText) })
  }
  ...
  // ⛔ internal 带正文，**只给服务端落库模块**（import-apply.js）用；端点只 send(plan)，
  //   正文绝不出 HTTP（隐私铁律）。floors 用 A1（collect.js planCollect）认的形状。
  const internal = {
    archiveRel,
    floorsRel,
    summaryRel,
    start,
    endFloor: kept.length > 0 ? start + kept.length - 1 : null,
    summaryText,
    target: { characterId: target.characterId, playthroughId: target.playthroughId },
    floors: kept.map((turn, i) => ({
      floor: start + i,
      isUser: turn.role === 'user',
      isSystem: turn.hidden === true,
      mes: turn.text,
      name: typeof turn.name === 'string' ? turn.name : '',
    })),
  }
...
/**
 * 公开计划器（**只返回可外传的那半**）。落库单要正文请用 buildImportPlan，⛔ 别把 internal 外传。
 */
export async function planImport(input, deps) {
  return (await buildImportPlan(input, deps)).plan
}
...
/**
 * 读 + 解析请求体（**导入系唯一口径**）：超限 ⇒ IMPORT_TOO_LARGE，其余 ⇒ IMPORT_INVALID。
 * 自检台注入口 `deps.rawBody`（生产路径永远从 req 读，注入只为行为级自检）。
 */
export async function readImportJsonBody(req, deps = {}) {
  let raw
  if (deps && typeof deps.rawBody === 'string') {
    raw = Buffer.from(deps.rawBody, 'utf8')
  } else {
    try {
      raw = await readRequestBody(req)
    } catch (e) {
      const code = e && e.code === 'IMPORT_TOO_LARGE' ? 'IMPORT_TOO_LARGE' : 'IMPORT_INVALID'
      throw importError(code, '请求体读不到：' + String((e && e.message) || e))
    }
  }
  try {
    const body = JSON.parse(raw.toString('utf8') || '{}')
    if (!isRecord(body)) throw new Error('不是 JSON 对象')
    return body
  } catch (e) {
    throw importError('IMPORT_INVALID', '请求体不可用：' + String((e && e.message) || e))
  }
}
...
export async function handleImportPlan(ctx, req, send, log) {
  let body
  try {
    body = await readImportJsonBody(req)
  } catch (e) {
    return send(400, errBody(e.code || 'IMPORT_INVALID', String(e.message || e)))
  }
  try {
