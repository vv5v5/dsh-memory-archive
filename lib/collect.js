...
export function planCollect(input, deps = {}) {
  ...
  const summaryPreExists = summaryShas.has(summaryPath) && summaryShas.get(summaryPath) !== null
  if (summaryPreExists && !overwrite) {
    throw failCode('COLLECT_SUMMARY_EXISTS', '摘要文件 ' + summaryPath + ' 已存在；确认重写请带 overwrite:true')
  }
  if (summaryPreExists) warnings.push('overwrite:true：摘要 ' + summaryId + '.md 已存在，将被覆盖')
  expectedRevisions[summaryPath] = overwrite && summaryShas.has(summaryPath) ? summaryShas.get(summaryPath) : null
...
export async function handleCollectPlan(ctx, url, req, send, deps = {}) {
  ...
  try {
    ...
    const summaryTarget = summaryPathOf(body.target, body.summary, body.range)
    let observed
    try {
      observed = await observeArchive(tavern, body.target, { overwrite: body.overwrite === true, floors: body.floors, summaryPaths: [summaryTarget.path] })
    } catch (e) {
