  ...
  try {
    ...
    try {
      ...
      targetKnown = pts.some((pt) => {
        const t = catalogTarget(pt)
        return t !== null && t.characterId === body.target.characterId && t.playthroughId === body.target.playthroughId
      })
    } catch (e) {
      if (isMissingErr(e)) targetKnown = false
      else if (e.code === 'TAVERN_UNREACHABLE') return send(400, errBody('COLLECT_TAVERN_UNREACHABLE', e.message))
      else return send(400, errBody('COLLECT_CATALOG_BROKEN', e.message))
    }

    // 观察归档现状：目录 / 已有楼号 / 撞楼文件的当前指纹 / 索引 / manifest。
    const archiveRel = body.target.characterId + '/' + body.target.playthroughId + '/archive'
    ...
    for (const f of body.floors) {
      ...
      if (!overwrite) continue // 默认拒绝路径下不需要指纹（会先抛 COLLECT_FLOOR_CONFLICT）
    ...
    if (archiveExists) {
      ...
      if (ix !== null) {
        observed.index = {
          exists: true,
          doc: parseJsonOrNull(ix.content),
          sha256: sha256Hex(ix.content),
        }
      }
