...
// ★★ 2026-09-19 **砍掉预设白名单**。用户口径：「白名单砍掉，**不存在库中**，我们本地有就行」。
//    本插件是通用工具，**不预设"我们只管哪些 preset"** —— 一个 preset 能不能改，由官方语义判定就够：
//    `trust === 'user'` ⇒ 可写；随部署附带的（DEPLOYMENT_IDS）⇒ 只读。
//    ⛔ 别再把它加回来：一份硬编码的 id 名单既是**本地事实**（不该进库），
//       又会随预设换代**立刻过期**（本机今天刚把 rp 换成社区版，名单当场就错了）。
...
/**
 * 注释行计数 —— ★ **必须跳过 YAML 块标量的正文**。
 *
 * 为什么（2026-09-16 沙箱实测出来的真 bug，别再改回去）：RP 归档指令正文里本来就带
 * `## 时间跨度` / `## 地点` / … / `## 标签` 这类**以 `#` 开头**的行 —— 它们在块标量（`|-`）里是
 * **内容**，不是注释。旧实现按「trim 后以 # 开头」数注释，于是：真机预设 117 条 `#` 行里有 6 条是
 * 块内容 ⇒ 只要 `customInstruction` 整块被替换（正常情况下必然如此），就"丢"6 条 ⇒
 * `commentsPreserved` 判 false ⇒ `VERIFY_FAILED` ⇒ **自动回滚**。
 * 后果：**"应用"只能成功一次**（空 → 第一次写入），之后每次修改都被自己的校验拦下 ——
 * 用户看到的现象正是"应用没用/没有实现"。
 */
function commentCounts(text) {
  const counts = new Map()
  let blockIndent = null // 非 null = 正在某个块标量里；值是头行的缩进
  for (const raw of String(text).split('\n')) {
    ...
    if (blockIndent !== null) {
      ...
      if (indent > blockIndent) continue // 仍是块内容（`## …` 那类行就在这儿被跳过）
      blockIndent = null // 块结束：这一行按普通行继续判
    }
    // 块标量头：`key: |` / `|-` / `>+` / `|2` / `|-  # 尾注释`（键里不允许出现 `:`）
    if (/^[ \t]*[^\s#][^:]*:[ \t]*[|>][-+0-9]*([ \t]+#.*)?$/.test(line)) {
      blockIndent = line.match(/^[ \t]*/)[0].length
      continue
    }
    if (trimmed.startsWith('#')) counts.set(trimmed, (counts.get(trimmed) || 0) + 1)
  }
  return counts
}

/** 注释保全检查：原文每条注释行在结果里逐条还在（次数只多不少）。块标量正文不算注释（见上）。 */
export function commentsPreserved(originalText, newText) {
  const a = commentCounts(originalText)
  const b = commentCounts(newText)
