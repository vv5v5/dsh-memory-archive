 ...
 *   订阅 tools/change 重算（mcp 工具晚注册；重算前先撤上一次的限制，避免按层越堆越多）；
 *   重入保护（restrict() 自身会 emit tools/change ⇒ applying 标志挡递归）；
 *   绝不抛（整段 try/catch，出错只降级为不收窄并大声写日志 —— 这一行抛 = preset 挂不上）。
...
function denyAllButTransport(ctx) {
  ...
  try {
    ...
    // 重算前先撤上一次的限制：restrict() 的官方契约是「返回值 = 撤掉这一层的 disposer」
    ...
    try {
      liftLast = tools.restrict({ deny })
      lastKey = key
    } catch (e) {
      liftLast = null
      lastKey = null
      const say = ctx && ctx.log && typeof ctx.log.warn === 'function' ? ctx.log.warn.bind(ctx.log) : null
      if (say) say('[rp-tool-scope] 收窄失败，降级为不收窄：' + String((e && e.message) || e))
    }
  } catch (e) {
    const say = ctx && ctx.log && typeof ctx.log.warn === 'function' ? ctx.log.warn.bind(ctx.log) : null
