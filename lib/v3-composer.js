...
/** 组合器产出的段 id（最终段名后缀），**数组顺序即注入顺序**。
 *  顺序口径（用户 2026-09-16 定稿）：
 *    ① 只注入"控制角色"的内容（主要提示词 / 描述 / 性格 / 场景 / 示例）；作者署名一类**不注入**；
 *    ② 主要提示词**紧跟上面的预设段**（= 我们这一块的第一段）→ 描述/性格/场景 → 世界书命中
 *       → 我们那行来源标记 → **后处理指令放最后**。
 *       ⚠️ "最后"只能是**我们这一块内的最后**：外部段整体被 Tavern 放在 profile 槽位（order 10），
 *       想真排到全文末尾得靠宿主槽位（例如 `deployment:persona-suffix` @10200），那是另一条路。
 *  ⛔ **开场白不在这里注入**（用户 2026-09-16 明确）：它属于"开场那一刻"的消息，不在这一段里。
 */
...
export function buildComposition(args) {
  ...
  // 1) 主要提示词**紧跟上面的预设段**（我们这一块的第一段），再描述/性格/场景/示例
  //    （口径：只注入"控制角色"的内容；作者署名一类不注入）
  ...
  // 3) 一行来源标记（默认开，放在**后处理之前**，好让后处理真排到最后）：
  //    让"这段是谁装的"在**看请求原文**时一眼可辨；不写正文、只有版本号
  if (cfg.marker !== false) {
    ...
    if (rt.greetingReferenceApplies === true) bits.push('greeting=first-turn(本块不注入开场)')
  ...
  // 4) **后处理指令放最后**（用户口径：DSH 只允许"在我们这一块里尽可能靠后"）
