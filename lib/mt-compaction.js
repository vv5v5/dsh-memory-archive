...
if (basicLoad.ok && llmLoad.ok) {
  ...
  RpCompactionEngine = class RpCompactionEngine extends BasicCompactionEngine {
    constructor(ctx, config = {}) {
      ...
      /**
       * ★ 2026-09-26（收纳升级，用户口径「在 rp 压缩（压正文）之外，还有压工具的每轮压缩」）：
       * **每轮强制修剪工具结果**（不等压缩触发）。官方「compaction-basic」只在「触发确认后」调
       * 「ctx.get('toolResultPruner').pruneSession(...)」⇒ 阈值之下老的大份工具结果永远不动
       * （真机十周目：整份 notes.md 被重读 8 次、8 份快照全躺在上下文里）。默认开；
       * 预设配置给「toolPrunePerTurn: false」可关回官方行为。公开属性（⛔ 不用 「#」，Proxy 纪律）。
       */
      this.toolPrunePerTurn = toolPrunePerTurn
      /** 每轮修剪的失败告警也只喊一声（公开属性）。 */
      this.toolPruneWarned = ''
    }
    ...
    async compactIfNeeded(agent, trigger, signal) {
      ...
      // ★★ 2026-09-26（收纳升级）：**每轮强制修剪工具结果** —— 在 super 的阈值判定**之前**无条件跑。
      //   为什么：官方只在「触发确认后」调「ctx.get('toolResultPruner').pruneSession(...)」，阈值之下
      //   老的大份工具结果（真机十周目：整份 notes.md 的 8 次重读快照，占整个上下文 ~26%）永远不清。
      //   这里每轮先剪一次：超 pruner 自己阈值（thresholdChars=8192）的结果被缩成「头＋标记＋尾」
      //   并落盘记录（幂等；原始全文仍在会话日志），然后 super 的阈值判定照旧走官方。
      //   ⛔ 异常绝不外冒（与面板现读同一纪律）：剪失败只 warn 一次，本轮照常走官方压缩判定。
      if (this.toolPrunePerTurn) {
        try {
          const pruner = this.ctx && typeof this.ctx.get === 'function' ? this.ctx.get('toolResultPruner') : undefined
          if (!pruner || typeof pruner.pruneSession !== 'function') {
            // ⛔ 静默跳过会让"以为在剪"骗过所有人 ⇒ 每轮明说拿没拿到服务（验收期日志；确认后可静音）。
            const message = TAG + ': 每轮工具修剪未执行：realm 里拿不到 toolResultPruner 服务（' + typeof (pruner && pruner.pruneSession) + '）'
            if (this.toolPruneWarned !== message) {
              this.toolPruneWarned = message
              try {
                if (this.ctx && this.ctx.logger && typeof this.ctx.logger.warn === 'function') this.ctx.logger.warn(message)
                else console.warn(message)
              } catch {}
            ...
            // ★ 验收期诊断（2026-09-26）：先自己走一遍 surface，数 tool/result 与超预算数——
            //   与 pruner 的内部扫描同一条路（surface.nodes + eventAt + deriveEventMessage），
            //   日志能直接回答「扫描盲区」还是「剪了没写回」。
            ...
            try {
              for (const seq of [...session.surface.nodes]) {
                ...
                toolCount++
                const msg = session.deriveEventMessage(ev)
                let chars = 0
                for (const blk of (msg?.content ?? [])) if (blk?.type === 'text') chars += [...blk.text].length
                if (chars > 8192) overCount++
              }
            } catch { /* 诊断失败不影响主流程 */ }
            const pruned = pruner.pruneSession(session)
            const entries = pruned && Array.isArray(pruned.pruned) ? pruned.pruned.length : 0
            const saved = pruned && Number.isFinite(pruned.charsRemoved) ? pruned.charsRemoved : 0
            console.log('[mt-compaction] 每轮工具修剪：surface 可见 tool/result ' + toolCount
              + ' 个（超预算 ' + overCount + '）⇒ 剪 ' + entries + ' 处 / 约 ' + saved + ' 字符')
          }
        } catch (error) {
          const message = TAG + ': 每轮工具结果修剪失败（照常走压缩判定）：' + String((error && error.message) || error)
          if (this.toolPruneWarned !== message) {
            this.toolPruneWarned = message
            try {
              if (this.ctx && this.ctx.logger && typeof this.ctx.logger.warn === 'function') this.ctx.logger.warn(message)
