...
export function createSourcesCache(opts = {}) {
  ...
  /**
   * @type {Map<string, {sources:object|null, rendered:string|null, observed:object|null, at:number}>}
   * 插入序 = LRU 序（命中时重新插入以刷新）。
   * ★ 一行三个槽：`sources`（composer 合同那套来源快照）、`rendered`（trace 合同下从官方
   *   `:part:` 段摘到的**已渲染**正文）、`observed`（这一轮段表里我们自己的段在不在 = replace 看守的直接观察）。
   *   ⛔ 共用同一行/TTL/LRU —— 不做第二套生命周期。
   */
  const rows = new Map()
  ...
  return {
    ...
    set(sessionId, sources) {
      ...
      rows.delete(sid)
      rows.set(sid, { sources, rendered: prev?.rendered ?? null, observed: prev?.observed ?? null, at: clock() })
      while (rows.size > max) {
    ...
    setRendered(sessionId, text) {
      ...
      const prev = rows.get(sid)
      rows.delete(sid)
      rows.set(sid, { sources: prev?.sources ?? null, rendered: t, observed: prev?.observed ?? null, at: clock() })
      while (rows.size > max) {
        const oldest = rows.keys().next().value
        if (oldest === undefined) break
        rows.delete(oldest)
    ...
    /**
     * 存"这一轮段表里我们自己的段在不在"的**直接观察**（2026-09-18 看守改版）。
     * 与另外两槽共用同一行/TTL/LRU。形状不对/空对象不存（宁可不存，也不存半个）。
     * @returns {boolean} 是否真的存了
     */
    setObserved(sessionId, observed) {
      ...
      const prev = rows.get(sid)
      rows.delete(sid)
      rows.set(sid, { sources: prev?.sources ?? null, rendered: prev?.rendered ?? null, observed, at: clock() })
      while (rows.size > max) {
        const oldest = rows.keys().next().value
        if (oldest === undefined) break
        rows.delete(oldest)
    ...
    /**
     * 取那次观察（未过期、是对象才给）。⛔ 与 `get` 同理：不因为别的槽空就删行。
     * @returns {object|null}
     */
