...
export function createSourcesCache(opts = {}) {
  ...
  /** @type {Map<string, {sources:object, at:number}>} 插入序 = LRU 序（命中时重新插入以刷新） */
  const rows = new Map()
  ...
  return {
    ...
    set(sessionId, sources) {
      ...
      // 命中已存在的键时先删再插 ⇒ Map 的插入序即"最近使用序"
      rows.delete(sid)
      rows.set(sid, { sources, at: clock() })
      while (rows.size > max) {
    ...
    get(sessionId) {
      ...
      if (!isObj(row.sources)) { rows.delete(sid); return null }
      rows.delete(sid)
      rows.set(sid, row)
      return row.sources
    },
