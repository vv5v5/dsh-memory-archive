...
export function registerSectionsCapture(context, options = {}) {
  ...
  const plugin = {
    ...
    apply: (scope) => {
      ...
      const disposeListen = scope.on('system-prompt/assemble', async (assembly, assembleContext, next) => {
        try {
      ...
      scope.effect(() => disposeListen, 'dsh-memory-archive-sections-capture: assemble listener')
      try {
        mkdirSync(opts.dir, { recursive: true })
      } catch {}
      scope.logger?.info?.(`[mt-sections] 组装捕获已挂载（dir=${basename(opts.dir)}，fullText=${opts.fullText ? 'on' : 'off'}）`)
    },
  }
...
// 正文来源只走记忆库同款路径 —— 宿主 sessionQuery.readSession()（全量事件，含
// request/header 的 data.header.system）；⛔ 绝不直读 ~/.dsh/sessions 下的会话文件，
...
export async function handleSectionsTextGet(ctx, url, send, redact, log, options = {}) {
  ...
  try {
    ...
    let snapshot
    try {
      snapshot = await sq.readSession(found.sessionId)
    } catch (e) {
      const msg = String(e?.message || e)
      return send(500, { ok: false, error: { code: 'SESSION_READ_FAILED', message: typeof redact === 'function' ? redact(msg) : msg } })
    }
    const events = eventsOfSnapshot(snapshot)
    if (!events) return send(500, { ok: false, error: { code: 'SESSION_READ_FAILED', message: 'readSession 返回了无法识别的数据形状' } })
    const system = pickSystemForTurn(events, found.turn)
    if (typeof system !== 'string') return respondUnavailable({ ...found, unavailable: 'turn-not-found' })
    const sliced = sliceSectionByHeader(found.section, system)
...
export async function handleEditorDiagnosticsGet(ctx, url, send, redact, log, options = {}) {
  ...
  if (!sq || typeof sq.readSession !== 'function') {
    ...
    try {
      await sq.readSession(sessionId)
    } catch (error) {
...
export async function handleSectionsRawGet(ctx, url, send, log, options = {}) {
  ...
  if (!sq || typeof sq.readSession !== 'function') {
    ...
    let events = null
    try {
      events = eventsOfSnapshot(await sq.readSession(sid))
    } catch (error) {
    ...
    if (events === null) {
      ...
      if (cred.offset === null || cred.chars === null) textUnavailable = 'no-offset'
        ...
        const system = pickSystemForTurn(events, record.turn)
        if (typeof system !== 'string') textUnavailable = 'turn-not-found'
        else {
          const sliced = sliceSectionByHeader(cred, system)
